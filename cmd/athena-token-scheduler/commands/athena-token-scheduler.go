...
func NewCommand() *cobra.Command {
	...
	var chainStateInterval, walletAssetInterval, simulationInterval, walletFundingSourceHistoryRetryInterval, walletSwapTransactionHistoryRetryInterval, aveInterval, contractSourceInterval, researchTTL time.Duration
	...
	command.Flags().DurationVar(&walletFundingSourceHistoryRetryInterval, "wallet-funding-source-history-retry-interval", env.ParseDurationFromEnv("ATHENA_TOKEN_WALLET_FUNDING_SOURCE_HISTORY_RETRY_INTERVAL", time.Minute, time.Second, 24*time.Hour), "Wallet funding source history retry interval")
	command.Flags().DurationVar(&walletSwapTransactionHistoryRetryInterval, "wallet-swap-transaction-history-retry-interval", env.ParseDurationFromEnv("ATHENA_TOKEN_WALLET_SWAP_TRANSACTION_HISTORY_RETRY_INTERVAL", time.Minute, time.Second, 24*time.Hour), "Wallet swap transaction history retry interval")
