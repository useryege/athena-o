...
func NewCommand() *cobra.Command {
	command := &cobra.Command{
		Use:               cliName,
		Short:             "athena-dex tools used by Athena",
		Long:              "athena-dex has internal utility tools used by Athena",
...
func NewRunDexCommand() *cobra.Command {
	...
	command := cobra.Command{
		Use:   "rundex",
		Short: "Runs dex generating a config using settings from the Athena configmap and secret",
		RunE: func(c *cobra.Command, _ []string) error {
			...
			errors.CheckError(err)
			config.UserAgent = fmt.Sprintf("athena-dex/%s (%s)", vers.Version, vers.Platform)
	...
	command.Flags().StringVar(&cmdutil.LogFormat, "logformat", env.StringFromEnv("ATHENA_DEX_SERVER_LOGFORMAT", "json"), "Set the logging format. One of: json|text")
	command.Flags().StringVar(&cmdutil.LogLevel, "loglevel", env.StringFromEnv("ATHENA_DEX_SERVER_LOGLEVEL", "info"), "Set the logging level. One of: debug|info|warn|error")
	command.Flags().BoolVar(&disableTLS, "disable-tls", env.ParseBoolFromEnv("ATHENA_DEX_SERVER_DISABLE_TLS", false), "Disable TLS on the HTTP endpoint")
	return &command
}
...
func NewGenDexConfigCommand() *cobra.Command {
	...
	command := cobra.Command{
		Use:   "gendexcfg",
		Short: "Generates a dex config from Athena settings",
		RunE: func(c *cobra.Command, _ []string) error {
			ctx := c.Context()
