...
func selectPodForPortForward(clientSet kubernetes.Interface, namespace string, podSelectors ...string) (*corev1.Pod, error) {
	...
	return nil, fmt.Errorf("cannot find ready pod with selector: %v - use the --{component}-name flag in this command or set the environmental variable (Refer to https://athena.readthedocs.io/en/stable/user-guide/environment-variables), to change the Athena CD component name in the CLI", podSelectors)
}
