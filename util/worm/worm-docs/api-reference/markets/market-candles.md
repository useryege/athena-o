...
<ParamField query="end_time" type="integer" required>
  Exclusive end timestamp (unix seconds). Must be greater than `start_time`. The range `end_time - start_time` must not exceed **30 days** (2,592,000 seconds).
</ParamField>
...
<Note>
  The API enforces a **30-day** maximum window per request (`end_time - start_time` ≤ 2,592,000 seconds). Split larger backfills into multiple requests.
</Note>
