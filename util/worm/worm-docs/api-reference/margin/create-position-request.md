...
<ParamField body="funds" type="string" required>
  Position collateral (decimal string). Do not send `price` or `shares`. Minimum depends on the market backend: **$1.00** for Hyperliquid-routed markets, **$5.00** for Polymarket-routed markets. Requests below the minimum return `400` with a field error on `funds`.
</ParamField>
...
<Note>
  ...
  On success the API returns **`201 Created`** with a **`Location`** header pointing to `GET /margin/positions/requests/{pubkey}/`.
</Note>
