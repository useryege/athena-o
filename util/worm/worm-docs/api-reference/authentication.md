...
<Tip>
  Python developers can use the official **[Python SDK](/api-reference/clients-sdks)** (`worm-sdk`) for HMAC signing and API key bootstrap instead of implementing the flow manually.
</Tip>
...
| Endpoint Type                                                                                                                                                                   | Auth                                                    |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------- |
| Markets, events, sports catalog, search, orderbook, prices, candles, market trades, market margin activity, [margin position estimate](/api-reference/margin/estimate-position) | None                                                    |
| `POST /auth/keys/challenge/`, `POST /auth/keys/create/`                                                                                                                         | None (bootstrap; `api_auth_bootstrap` throttle, 10/min) |
| Orders, trades, account, margin (except estimate), redeems, `GET /auth/keys/`, `DELETE /auth/keys/{key_id}/`                                                                    | HMAC headers (`api_authenticated`, 300/min)             |
...
API_SECRET = creds["secret"]  # HMAC signing key — use this string as the HMAC secret (UTF-8 bytes)
...
<Warning>
  **Storage** — Keep `WORM-API-KEY` and `WORM_API_SECRET` in environment variables or a dedicated secret manager. Do not commit `.env` or similar files.
