...
Source: https://docs.worm.wtf/api-reference/margin/list-settlements

GET /margin/positions/settlements/
...
<ParamField type="string">
  Comma-separated settlement states (case-insensitive). Filters apply across the full list; unsupported combinations may return a validation error.
</ParamField>

<ParamField type="string">
  Market lifecycle filter (case-insensitive). Allowed values depend on the markets in the result set; unsupported combinations may return a validation error (for example when using `OPEN` or `RESOLVED`).
</ParamField>
