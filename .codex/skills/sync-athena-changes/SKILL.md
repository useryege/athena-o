...
Run the matching generator once after a coherent source-edit batch and before adapting consumers that require the new generated shape.

| Changed source | Generator |
| --- | --- |
| `internal/<module>/store/migrations/*.sql` or `internal/<module>/store/queries/*.sql` | `make sqlc-local` |
...
- Treat `hack/postgres/init/*.sql` as database bootstrap configuration, not an automatic sqlc trigger unless the same change also updates module migration or query sources.
...
- Use the generation command and generated diff as the default validation for generated boundaries.
