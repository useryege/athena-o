...
if [ "$ATHENA_REDIS_LOCAL" = 'true' ]; then
    ...
    if [ -z "$REDIS_PASSWORD" ]; then
        ...
        docker run --rm --name athena-redis -i -p "$REDIS_PORT:$REDIS_PORT" docker.io/library/redis:"$REDIS_IMAGE_TAG" --save '' --appendonly no --port "$REDIS_PORT"
    else
        echo "Starting Docker container with password."
        docker run --rm --name athena-redis -i -p "$REDIS_PORT:$REDIS_PORT" -e REDIS_PASSWORD="$REDIS_PASSWORD" docker.io/library/redis:"$REDIS_IMAGE_TAG" redis-server --save '' --requirepass "$REDIS_PASSWORD" --appendonly no --port "$REDIS_PORT"
    fi
fi