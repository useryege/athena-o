...
docker compose version >/dev/null
mkdir -p '${BSC_INDEXER_REMOTE_APP_DIR}'"
...
ssh "${remote}" "set -euo pipefail
install -m 0644 '${remote_tmp_compose}' '${BSC_INDEXER_REMOTE_APP_DIR}/docker-compose.yml'
install -m 0600 '${remote_tmp_env}' '${BSC_INDEXER_REMOTE_APP_DIR}/.env'
rm -f '${remote_tmp_compose}' '${remote_tmp_env}'"

echo "Streaming ${BSC_INDEXER_IMAGE} to ${remote}..."
docker save "${BSC_INDEXER_IMAGE}" | ssh "${remote}" "docker load"

echo "Starting PostgreSQL 18 and the BSC indexer..."
if ! ssh "${remote}" "set -euo pipefail
cd '${BSC_INDEXER_REMOTE_APP_DIR}'
BSC_INDEXER_IMAGE='${BSC_INDEXER_IMAGE}' docker compose -f docker-compose.yml --env-file .env up -d --force-recreate --remove-orphans --wait --wait-timeout 180"; then
  echo "Remote deployment failed. Recent service logs follow." >&2
  ssh "${remote}" "cd '${BSC_INDEXER_REMOTE_APP_DIR}' && BSC_INDEXER_IMAGE='${BSC_INDEXER_IMAGE}' docker compose -f docker-compose.yml --env-file .env ps" >&2 || true
  ssh "${remote}" "cd '${BSC_INDEXER_REMOTE_APP_DIR}' && BSC_INDEXER_IMAGE='${BSC_INDEXER_IMAGE}' docker compose -f docker-compose.yml --env-file .env logs --tail=120" >&2 || true
  exit 1
fi

echo "Remote deployment status:"
ssh "${remote}" "cd '${BSC_INDEXER_REMOTE_APP_DIR}' && BSC_INDEXER_IMAGE='${BSC_INDEXER_IMAGE}' docker compose -f docker-compose.yml --env-file .env ps"
