...
if [ -f '${REMOTE_APP_DIR}/docker-compose.prod.yml' ]; then
  ...
  POSTGRES_PASSWORD=dummy REDIS_PASSWORD=dummy ATHENA_WALLET_ENCRYPTION_KEY=dummy MINIO_ROOT_PASSWORD=dummy-root-password ATHENA_ACCOUNT_AVATAR_S3_ACCESS_KEY_ID=dummy-access-key ATHENA_ACCOUNT_AVATAR_S3_SECRET_ACCESS_KEY=dummy-secret-key PROD_IMAGE='${IMAGE}' MINIO_IMAGE='${MINIO_IMAGE}' MINIO_MC_IMAGE='${MINIO_MC_IMAGE}' PROD_POSTGRES_VOLUME='${POSTGRES_VOLUME}' PROD_MINIO_VOLUME='${MINIO_VOLUME}' docker compose -f docker-compose.prod.yml --env-file \"\${compose_env_file}\" down --remove-orphans
fi
...
cd '${REMOTE_APP_DIR}'
compose() {
  PROD_IMAGE='${IMAGE}' MINIO_IMAGE='${MINIO_IMAGE}' MINIO_MC_IMAGE='${MINIO_MC_IMAGE}' PROD_POSTGRES_VOLUME='${POSTGRES_VOLUME}' PROD_MINIO_VOLUME='${MINIO_VOLUME}' docker compose -f docker-compose.prod.yml --env-file .env \"\$@\"
}
...
compose run --rm --no-deps minio-init"
  ...
  ssh "${REMOTE}" "cd '${REMOTE_APP_DIR}' && PROD_IMAGE='${IMAGE}' MINIO_IMAGE='${MINIO_IMAGE}' MINIO_MC_IMAGE='${MINIO_MC_IMAGE}' PROD_POSTGRES_VOLUME='${POSTGRES_VOLUME}' PROD_MINIO_VOLUME='${MINIO_VOLUME}' docker compose -f docker-compose.prod.yml --env-file .env --profile tools run --rm athena-migrate athena up --module '${MIGRATE_MODULE}'"
...
compose ps"
  ...
  echo "Remote hot deployment completed. PostgreSQL, MinIO, and Redis were preserved."
  exit 0
fi
...
ssh "${REMOTE}" "cd '${REMOTE_APP_DIR}' && PROD_IMAGE='${IMAGE}' MINIO_IMAGE='${MINIO_IMAGE}' MINIO_MC_IMAGE='${MINIO_MC_IMAGE}' PROD_POSTGRES_VOLUME='${POSTGRES_VOLUME}' PROD_MINIO_VOLUME='${MINIO_VOLUME}' docker compose -f docker-compose.prod.yml --env-file .env up -d postgres"

echo "Running Athena migrations on ${REMOTE}..."
ssh "${REMOTE}" "cd '${REMOTE_APP_DIR}' && PROD_IMAGE='${IMAGE}' MINIO_IMAGE='${MINIO_IMAGE}' MINIO_MC_IMAGE='${MINIO_MC_IMAGE}' PROD_POSTGRES_VOLUME='${POSTGRES_VOLUME}' PROD_MINIO_VOLUME='${MINIO_VOLUME}' docker compose -f docker-compose.prod.yml --env-file .env --profile tools run --rm athena-migrate athena up --module '${MIGRATE_MODULE}'"

echo "Starting Athena on ${REMOTE}..."
ssh "${REMOTE}" "cd '${REMOTE_APP_DIR}' && PROD_IMAGE='${IMAGE}' MINIO_IMAGE='${MINIO_IMAGE}' MINIO_MC_IMAGE='${MINIO_MC_IMAGE}' PROD_POSTGRES_VOLUME='${POSTGRES_VOLUME}' PROD_MINIO_VOLUME='${MINIO_VOLUME}' docker compose -f docker-compose.prod.yml --env-file .env up -d"

echo "Remote deployment status:"
ssh "${REMOTE}" "cd '${REMOTE_APP_DIR}' && PROD_IMAGE='${IMAGE}' MINIO_IMAGE='${MINIO_IMAGE}' MINIO_MC_IMAGE='${MINIO_MC_IMAGE}' PROD_POSTGRES_VOLUME='${POSTGRES_VOLUME}' PROD_MINIO_VOLUME='${MINIO_VOLUME}' docker compose -f docker-compose.prod.yml --env-file .env ps"
