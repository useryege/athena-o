...
application: [ "$BIN_MODE" = 'true' ] && COMMAND=./dist/athena || COMMAND='go run ./cmd/main.go' && sh -c "GOCOVERDIR=${ATHENA_COVERAGE_DIR:-/tmp/coverage/athena-application} FORCE_LOG_COLORS=1 ATHENA_FAKE_IN_CLUSTER=true ATHENA_BINARY_NAME=athena-application $COMMAND --loglevel debug --port ${ATHENA_APPLICATION_PORT:-8082}"
redis: hack/start-redis-with-password.sh
postgres: hack/start-postgres-with-password.sh
ui: sh -c 'cd ui && ${ATHENA_E2E_YARN_CMD:-yarn} start'
