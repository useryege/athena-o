...
- `ARGOCD_E2E_APISERVER_PORT`: Listener port for `athena-server` (default: `8080`)
- `ARGOCD_E2E_REPOSERVER_PORT`: Listener port for `athena-reposerver` (default: `8081`)
...
If you have changed the port for `athena-server`, be sure to also set `ARGOCD_SERVER` environment variable to point to that port, e.g. `export ARGOCD_SERVER=localhost:8888` before running `make test-e2e` so that the test will communicate to the correct server component.
