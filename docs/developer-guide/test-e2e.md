...
The test [directory](https://github.com/useryege/athena/tree/master/test) contains E2E tests and test applications. The tests assume that Athena services are installed into `athena-e2e` namespace or cluster in current context. A throw-away
...
The [/test/e2e/testdata](https://github.com/useryege/athena/tree/master/test/e2e/testdata) directory contains various Athena applications. Before test execution, the directory is copied into `/tmp/argo-e2e***` temp directory (configurable by `ATHENA_E2E_DIR`) and used in tests as a
...
> This is often caused by `/tmp` directoring sharing protection. You can configure a different directory with `ATHENA_E2E_DIR`, or disable the directory sharing protection.
>
...
- `ATHENA_E2E_DIR`: Local path to the repository to use for ephemeral test data

If you have changed the port for `athena-server`, be sure to also set `ATHENA_SERVER` environment variable to point to that port, e.g. `export ATHENA_SERVER=localhost:8888` before running `make test-e2e` so that the test will communicate to the correct server component.
...
- A namespace `athena-e2e-ns-${id}`.
- A primary name for the app `athena-e2e-${id}`.
...
**Tests fails to delete `athena-e2e-ns-*` namespaces.**
