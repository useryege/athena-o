...
L1 为第一层直接关联钱包，包括部署者、最多 5 个初始接收钱包及已取得的 owner。初始接收钱包的选择沿用 [Token 目标设计](token.md#研究证据与完成条件)；owner 仍按 [owner 获取流程](token-owner-wallet-flow.md)尽力采集，未取得时继续研究其他已知钱包。
