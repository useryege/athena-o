...
flowchart TD
    ...
    related --> history["按地址去重，通过 Etherscan 采集普通交易<br/>区块 0 至部署区块减 1<br/>从新到旧取第 1 页，最多 300 笔"]
...
| 关联钱包关系 | 取得 owner 后将其纳入项目关联钱包研究，保留 owner 角色，不默认 owner 与部署者或初始接收钱包相同；历史交易按钱包地址去重查询。 |
...
例如项目部署在第 `30000` 区块，即使 owner 在之后才取得，其历史交易仍查询 `0..29999` 区块内最近最多 300 笔普通交易。该数量上限按每个去重钱包计算，属于已确认需要原样保留的规则，不代表完整钱包历史。
