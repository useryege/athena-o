...
从开发阶段纳入范围的顶层合约创建交易发现项目，识别 Token 后取得或复用源码质检报告及风险等级。高风险项目直接排除、不再研究；非高风险的项目继续其他研究，符合筛选规则后交接给交易监控与执行板块，在交易开放且满足买入条件时及时买入。
...
- 源码质检报告及风险等级按源码共享，按 `code_hash` 查库命中后直接复用，无报告且取得源码时才由 AI 静态分析并评级。报告评为高风险时立即排除当前项目，不等待其他资料齐备；非高风险继续其他研究。
- AI 超时、请求失败或返回内容不符合报告要求属于技术异常，单独记录，不作为业务判断分支。
...
高风险质检直接排除项目及同源码共享报告已经确定；风险评级细则、未取得源码时的处理、其余研究规则与买入条件、研究完成前已开放交易的处理、买入条件不满足后的处理、交接后继续研究或撤销关注、买入结果确认和防重复执行仍待设计。第二板块将扩展协议、报价资产及底池类型，具体范围待定；持仓和卖出尚未纳入确定范围。
