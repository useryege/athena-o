...
> 文档状态：目标设计草案，尚未实现。本文件说明已确认的 L1–L5 分层展示、共享来源与交易证据交互。当前阶段只采集、解析并展示钱包资料，不据此筛选过滤项目。具体流程见 [多层关联钱包研究](token-wallet-research-flow.md)。
...
flowchart LR
    ...
    E --> C["L2：来源钱包 C"]
    E --> D["L2：来源钱包 D"]
    C --> A["L1：钱包 A<br/>标签：部署者 · owner · 初始接收钱包"]
...
钱包 A 同时是当前项目的部署者、owner 和初始接收钱包，只显示一个节点和三个角色标签。C 向 A、B 两个钱包转账，因此显示为一个来源节点、两条连线；不会因为 A 有三个标签而复制 A 或 C 到 A 的连线。C 与 D 又共享 E 这一上游来源。E 是值得继续查看历史行为的关联线索，共同来源本身不证明这些钱包属于同一个团队。

## 节点、连线与层级

| 对象 | 已确认的设计 |
| --- | --- |
| 钱包节点 | 按链和地址唯一保存和计数。一个节点可同时显示部署者、owner、初始接收钱包等多个角色标签，已有标签不被新角色覆盖。 |
...
- 默认先展示 L1、L2；支持选择一个钱包继续展开其上游来源，也支持一键展开至 L5。
- 展开时优先复用已取得的历史与关系；未采集的钱包按统一窗口取数并分析。L5 仍展示自身部署与代币交互结果，仅停止继续发现 L6。
...
- 区分尚未展开、已折叠、达到 L5 边界，以及已查询样本内没有来源；达到每钱包 300 笔采样上限时保留该事实。任何一种显示状态都不代表已取得完整钱包历史。

五层只限制深度，来源分支仍可能很多。当前不增加额外金额门槛或每层来源数量上限，使用按需展开、折叠和共享节点减少重复显示；具体执行调度后续细化。
