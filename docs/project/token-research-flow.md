...
“整理研究资料”环节的独立展开图见 [研究资料整理流程](token-research-materials-flow.md)，包含基础信息、源码、owner 读取、关联钱包采集及结果汇集。

## 流程图

```mermaid
flowchart TD
    token["已判定为 Token 的项目"] --> materials["整理研究资料（详见独立流程）<br/>基础信息、源码与 owner<br/>部署者、初始接收钱包和 owner 的历史与代币余额"]
...
owner 缺失时继续研究的原则已明确；具体研究规则、阈值、规则组合方式、人工与 AI 的分工、其他资料缺失或矛盾时的处理，以及交接后是否持续研究、更新或撤销关注，均留待后续设计。本文件不补充这些规则，也不沿用当前六类任务全部结束后构建不可变画像的流程作为固定完成条件。
