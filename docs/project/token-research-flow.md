...
本流程从已经判定为 Token 的项目开始。先取得或复用源码质检报告及风险等级，高风险项目直接排除，不再继续研究；明确非高风险的项目继续其他研究和规则筛选。筛选结论、原因和依据关联项目保存，供后续人工排查。完整业务背景见 [Token 目标设计](token.md)。
...
flowchart TD
    ...
    risk -->|明确非高风险| materials["继续整理其他研究资料（详见独立流程）<br/>基础信息、owner 及关联钱包资料"]
    risk -->|未取得报告或无法评级| unresolved["记录实际原因<br/>后续研究处理待明确"]
...
图中明确高风险质检的排除关口：生成或命中高风险报告后立即结束当前项目研究，不等待其他研究材料齐备。其他资料的采集顺序、并发方式和已启动任务的结束处理另行设计。owner 明确尽力采集，未取得本身不构成排除理由，也不使已因高风险排除的项目恢复研究。没有报告或无法评级不属于“明确非高风险”；其后续研究处理以及其他资料缺失、冲突或采集失败的处理仍待设计。
...
| AI 静态质检报告 | 按 `code_hash` 匹配源码，复用其已完成报告与风险等级；没有报告且已有源码时才由 AI 静态分析并评级，同一源码只生成一份共享报告。保存代码依据、触发条件、潜在影响、评级依据及无法判断的部分，不核实链上状态。高风险直接排除当前项目，不再研究；明确非高风险才继续其他研究。详见 [合约源码 AI 静态审阅流程](token-contract-review-flow.md)。 |
...
筛选结论关联对应链和项目持久保存。高风险报告直接触发排除，保存当前项目引用的共享报告、风险等级与排除原因；相同源码的新部署项目同样应用该报告，无需重复质检。明确非高风险只表示可以继续研究，符合其余规则后才交接；其他不符合规则的项目也要能查到具体原因和依据。具体存储结构和人工查询入口后续设计。
...
owner 缺失本身不排除项目、同一源码共享质检报告及风险等级、报告高风险时直接排除且不再研究，均已明确。风险评级细则、其余研究规则与分工、没有有效报告或其他资料缺失时的处理，以及交接后的更新与撤销关注，仍待设计。本文件不增加数值阈值，也不沿用当前六类任务全部结束后构建不可变画像的流程作为固定完成条件。
