...
图中明确高风险质检的排除关口：生成或命中高风险报告后立即结束当前项目研究，不等待其他研究材料齐备。钱包分支到保存展示为止；虚线表示仍待设计的其他项目研究筛选，不以钱包展示结果触发通过或排除。其他资料的采集顺序、并发方式和已启动任务的结束处理另行设计。owner 由 AI 分析源码或复用已有结论，再由 Go 按可用方式读取当前合约；未取得本身不构成排除理由，也不使已因高风险排除的项目恢复研究。其他资料缺失、冲突或采集失败的处理仍待设计。
