...
该板块只采集、解析、保存和展示资料，不增加风险评级、项目筛选或排除规则。源码 AI 当前只生成事实报告和依据，后续由程序按管理员维护的规则评级与筛选，见 [AI 静态质检流程](token-contract-review-flow.md)。
...
本流程以项目首次真实 Swap 为研究截止，识别范围不限定 V2、V3 或 V4；未取得全部税费接收方或五层钱包资料不延后截止。首次 Swap 时尚未交接的项目结束研究并放弃本轮参与；结束研究不代表资料齐备、高风险或可以买入。管理员手动重检共享报告不自动恢复项目，具体衔接见 [项目研究与筛选流程](token-research-flow.md)。
...
- 税费接收钱包属于 L1，读取余额采集时最新区块的 WETH、USDT 等已追踪代币余额，保留区块与时间。不采集原生币余额，本轮不将余额采集扩展到 L2–L5。
