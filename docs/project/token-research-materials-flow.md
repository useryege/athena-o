...
flowchart TD
    ...
    subgraph source["合约源码与 AI 静态报告"]
        ...
        SR -->|取得报告| ST["关联同一源码的共享报告及风险等级<br/>保留代码依据与评级原因"]
        SR -->|AI 失败或未完成| UN["记录实际原因<br/>后续研究处理待明确"]
        ST --> RISK{"质检报告风险结论"}
...
| AI 静态质检报告 | 通过 `code_hash` 定位源码及已完成报告，有报告就复用风险等级；没有报告且已有源码时才由 AI 静态分析并评级，同一源码只生成一份报告。报告保存代码依据、触发条件、影响及评级原因，不核实链上状态或依赖钱包资料。高风险直接排除项目，不再研究。完整流程见 [合约源码 AI 静态审阅](token-contract-review-flow.md)。 |
...
AI 静态质检只依赖已取得的源码；同一源码的报告与 owner 读取方式共享复用，owner 地址仍按当前合约读取。owner 和钱包采集不进入静态报告的核实过程。报告描述源码中的逻辑与条件，不将其表述为已核实的当前链上状态。没有报告、无法评级、AI 失败或未完成时记录实际原因，不能作为非高风险结论；此类情况如何影响后续研究仍待明确。
