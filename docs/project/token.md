...
| 合约源码 `contract_code_source` | 继续采集，按字节码哈希复用已有源码，取得后交给 AI 静态审阅 | 先查询数据库，相同字节码哈希已有关联源码时直接复用，避免重复调用 Etherscan API；保留源码、验证状态和代码哈希，生成静态审阅报告，识别源码中可见的漏洞或危险逻辑 |
...
复用范围是已有源码资料，不据此将新地址标记为已经在 Etherscan 完成独立源码验证，也不直接复用其他项目的筛选结论。运行时字节码相同仍可能存在不同的初始化参数和链上状态；Etherscan 的 [相似字节码匹配说明](https://info.etherscan.com/what-is-similar-match-bytecode/) 也区分了代码匹配与部署行为。源码来源、复用关系与新地址验证状态的具体表达后续设计。
...
首版在取得或复用非空源码后，直接交给 AI 阅读并生成一份静态审阅报告。目标是识别源码中已经可见的明显漏洞、后门或危险逻辑，为筛选这部分项目提供依据。流程为：取得源码 → AI 静态审阅 → 生成报告 → 保存并关联源码及项目。图示与报告说明见 [合约源码 AI 静态审阅流程](token-contract-review-flow.md)。
...
报告说明审阅范围、发现的问题、源码位置或片段、触发条件及潜在影响。AI 可以指出“源码允许某角色执行危险操作”，但不把它写成已经确认当前链上角色或参数的事实；缺少实现或依赖源码时，说明无法判断的部分。未发现明显问题仅表示本次源码审阅的结果，AI 失败或未完成也不能记录为未发现风险。

报告作为项目研究与筛选依据。具体问题清单、提示词和报告格式，以及哪些发现直接触发项目过滤，后续再细化；本轮只确定 AI 静态审阅与报告产出的范围，不新增自动过滤阈值、模型选型或数据结构。
...
通过 Token 识别后，项目研究的目的是依据研究筛选规则评估项目，保留符合规则的项目，过滤不符合规则的项目，并留下可供后续人工排查的结论、原因和依据。资料采集为筛选判断提供事实材料；取得源码后的首版研究已明确采用 AI 静态审阅并生成报告，其余研究方式和具体筛选规则仍待设计。
...
Token 识别结果与项目研究筛选结论分别表达不同判断：一个项目可以是 Token，但不符合后续项目筛选规则；研究阶段过滤项目不将其重新标记为不是 Token。研究规则、阈值、规则组合方式，以及资料缺失或矛盾时如何判定，仍需逐项明确；本轮不自行补充筛选条件，也不将缺失资料直接等同于劣质项目。
...
后续增加 AI 板块后，将 AI 分析得到的 owner 字段读取方式与所依据的源码资料、对应的运行时字节码 `code_hash` 关联保存，供相同代码的合约复用。这里的 `code_hash` 沿用运行时字节码哈希，不是新增的源码文本哈希。
...
| 研究标准与结果记录 | 已明确按研究规则筛选项目，符合的交接、不符合的过滤，并持久保存结论、未通过规则、原因及依据供人工排查；具体规则、阈值、组合方式、信息缺失或矛盾的处理，以及存储结构和人工查询入口后续设计 |
| 研究方式 | 取得源码后先由 AI 静态审阅并生成报告，不核实链上状态；报告格式、问题清单、失败处理，以及其余研究方式、研究触发和信息更新规则仍待细化 |
