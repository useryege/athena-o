...
| 合约源码 `contract_code_source` | 继续采集，按字节码哈希复用已有源码 | 先查询数据库，相同字节码哈希已有关联源码时直接复用，避免重复调用 Etherscan API；保留源码、验证状态和代码哈希，为后续合约逻辑与权限研究提供材料 |
...
通过 Token 识别后，项目研究的目的是依据研究筛选规则评估项目，保留符合规则的项目，过滤不符合规则的项目，并留下可供后续人工排查的结论、原因和依据。资料采集为筛选判断提供事实材料，具体研究方式和筛选规则仍待设计。
