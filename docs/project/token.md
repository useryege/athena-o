...
| 钱包历史交易 `wallet_normal_transactions` | 原样保留部署前最近最多 300 笔普通交易的查询规则，关联钱包范围纳入已取得的 owner | 每个去重钱包分别查询区块 `0` 至 `部署区块 - 1`，倒序取第一页最多 300 笔，为历史项目、资金往来与关联行为分析提供线索；owner 首版通过合约读函数获取 |
...
部署交易、部署者、代码哈希、初始接收钱包及分配线索等发现阶段已经取得的信息，应一并作为研究材料。目标设计的关联钱包包括部署者、最多 5 个初始接收钱包和采集到的合约 owner，三类地址均纳入普通交易与代币余额采集。初始接收钱包沿用部署交易内该代币的转账日志，按净接收量从高到低排序，在净接收量为正且查询时无合约代码的地址中最多选取 5 个。该上限仅针对初始接收钱包，部署者与已取得的 owner 按各自角色纳入关联钱包。

钱包历史交易的查询规则已确认原样保留：对每个去重后的关联钱包，通过 Etherscan 普通交易接口 `account/txlist` 获取项目部署区块之前、距离部署时点最近的最多 300 笔交易。部署者、所选初始接收钱包和已取得的 owner 均沿用这一规则；300 笔是每个钱包的上限。
...
例如项目部署在第 `30000` 区块，则查询范围固定为 `0–29999`，倒序取每个钱包最近的最多 300 笔普通交易。即使实际采集时链已推进到更高区块，截止高度仍为 `29999`。该范围是普通交易的历史样本，不包含内部交易或完整代币转账事件历史。

这些交易用于追溯关联钱包参与过的历史项目、资金来源及地址间的交易联系，为项目筛选积累依据；具体分析方法和筛选规则后续细化。

当前代码已经实现上述区块范围、排序、页码及数量规则，本轮原样保留。当前钱包范围仍只覆盖部署者和最多 10 个初始接收钱包；初始接收钱包缩减为最多 5 个、取得 owner 后纳入采集的调整仍待实现。详见 [钱包历史交易现状](../design/token-intelligence/wallet-normal-transactions.md)及对应的[截止区块计算](../../internal/token/collection/application/processors.go)与[Etherscan 查询参数](../../internal/token/adapters/normaltransactions/provider.go)。
