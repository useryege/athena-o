...
缓存键是运行时字节码哈希，不是源码文本哈希，也不是 `code.length`。有哈希记录不等于已经取得源码；某个地址的空结果不会使同哈希的其他地址永久停止源码查询。按哈希匹配到同一份源码后，共享其质检报告及风险等级、AI 已完成的 owner 分析结论与读取方式，以及[税费接收钱包分析](token-fee-recipient-wallet-flow.md)的识别结论、固定地址与读取方式。依赖链上状态的 owner 和税费接收地址，由 Go 程序按当前链、当前合约分别读取。当前项目应用共享报告的风险结论并保存自己的筛选记录，不复制其他项目的链上状态；复用源码也不等于当前地址已在 Etherscan 完成独立源码验证。
