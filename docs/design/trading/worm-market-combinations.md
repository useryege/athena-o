...
selection and display order, responsive presentation, and unsaved-draft state.
...
interactive browser
  -> native API Server facade
       ...
       -> Worm Markets internal gRPC
            ...
            -> stable market/outcome selectability projection
...
once and preserves the Event's upstream child order.
...
6. The browser may add multiple unique Events. A Market Condition ID may occur
   ...
   ordinals.
7. Desktop renders Event explorers in the main column and a sticky Current
   combination summary in the second column. At 900 px and below, markets render
   as cards and a sticky selected-count control opens the same summary in a
   Drawer. Labeled YES/NO controls, text reasons, visible focus, keyboard actions,
   and touch-sized controls preserve non-color and non-pointer operation.
...
11. Edit loads `GET /api/v1/worm-trading/combinations/{id}` and then refreshes
    ...
    validation at the server. Dirty browser state installs both in-application
    navigation and browser-unload protection.
12. PUT sends the full name and item replacement plus `expectedRevision`. The
...
13. DELETE requires `expectedRevision` in the query. It locks and verifies the
...
revision, a dirty baseline, and transient errors in React memory. It stores no
combination draft in localStorage or sessionStorage. The server response omits
the owner account UUID and always materializes arrays and pagination fields.
...
- The browser cannot select an owner or persist its own title, logo, outcome
  label, selectability, backend, leverage, or ordinal snapshot.
...
An edit-time refresh failure retains saved snapshots for inspection. It does not
authorize a save: the API Server refetches every Event and revalidates every item
before a PUT. Account, access-revision, or route changes abort scoped browser
requests and discard late results.

## Observability

The native responses use `Cache-Control: no-store, private` and vary by Cookie
and Authorization. Event catalogs expose `fetchedAt`; saved templates expose
revision and create/update Unix seconds. Stable unavailable codes diagnose
...
- [ ] Catalog completeness, bounded concurrency, stable unavailable codes, and no-estimate boundary remain current.
...
- [ ] Desktop two-column, compact card/Drawer, keyboard, focus, and touch behavior remain current.
