...
> 实现状态（2026-09-16）：Worm Markets 服务与权限已经退役，执行所需 fresh catalog 已内聚到 Worm Trading；本文件的 proof、串行、幂等、未知结果、恢复和历史规则保持有效。原 main 的专属数据库与旧通知来源已经精确收尾，正常重启未重建 Markets。
...
| Process construction and secrets | [cmd/athena-worm-trading/commands/athena-worm-trading.go](../../../cmd/athena-worm-trading/commands/athena-worm-trading.go), [cmd/athena-wallet/commands/athena_wallet.go](../../../cmd/athena-wallet/commands/athena_wallet.go), [docker-compose.prod.yml](../../../docker-compose.prod.yml) | fixed Web client, signer-only Wallet clientset, dedicated signer token, service lifecycle |
