...
| Service Operations | 两个 BSC 索引器与 Sports 的代码和契约删除、失效配置／队列清理、精确部署退役及专属数据直接删除 | [删除与清理配套设计](../superpowers/specs/2026-09-16-module-removal-cleanup-design.md) | `2026-09-16 已实施并验收`；该任务当时保留 Worm 两服务，现场状态见[验收记录](../testing/module-removal-cleanup-acceptance.md)；随后 Markets 由独立任务退役，Trading 与共用 Worm 能力保留，不能把后续 Markets 结果计入原 BSC／Sports 验收 |
| Development Runtime | 十一个本地应用、独立入口、阶段进度、就绪与失败范围、精确停止 | [2026-09-17 全栈修订方案](../superpowers/specs/2026-09-17-full-stack-access-reassessment-design.md) | `已实现`；五核心／六业务、Token 延期、五远端 Gateway 只读健康、分阶段就绪与 180 秒共享停止预算均已落地；[全栈验收](../testing/full-stack-access-acceptance.md) |
| Development Runtime | 按实例正向选择独立服务、持久基础设施、独立 schema owner、精确归属和有界停止 | [本地运行编排](development-runtime/local-runtime-orchestration.md) | `已实现`；根路径／前缀十一应用、局部选择、五库、失败隔离、保存运行器和最终资源审计通过，[全栈验收](../testing/full-stack-access-acceptance.md) |
| Developer Experience | Public LLM discovery documents, one-time Connect AI instructions, Swagger generation and embedding, unauthenticated documentation delivery, and root-path and safety boundaries | [AI Discovery Documentation](developer-experience/ai-discovery-documentation.md) | `已实现，尚未退役`；[退役需求](../requirements/developer-experience/api-documentation-removal.md)与[详细设计](../superpowers/specs/2026-09-18-swagger-ai-discovery-removal-design.md)已审阅通过，转交新的 chat 在新 Git 分支实施 |
...
| Identity and Access | 同一账号只保留一个有效客户端登录，新登录成功后旧登录立即失效 | [单客户端登录目标设计](identity-access/single-client-login.md) | `设计中`；业务规则已确认，[完整规格第 4 节](../superpowers/specs/2026-09-15-trader-sync-manual-trading-design.md#4-单客户端登录与权限)已整理当前会话、交换恢复及交易接收顺序，待整体审阅，代码未实现 |
...
| Identity and Access | Realm-to-persisted-role session binding, credential-specific Wallet and Worm-selection rules, login, API Key, Profit Sharing, seven-module access, Pending state, and transactional administrator control | [Account Access Control](identity-access/account-access-control.md) | `已实现` |
...
| Web UI | 会员登录、Pending access、七模块导航、Solana 列表、Telegram 绑定、Trader Sync 六路由与可见页单飞刷新、Account Center、API Keys、Profit Sharing 和 Wallet | [会员应用壳](web-ui/member-application-shell.md) | `已实现` |
| Web UI | 管理员登录/角色复查、账户授权、Trader Sync 安全概要、三状态来源与 Module Access 第四页签、系统通知和管理员自助 | [管理员应用壳](web-ui/administrator-application-shell.md) | `已实现`；访问设置 GUI 与审计持久化见[全栈验收](../testing/full-stack-access-acceptance.md) |
...
| Market Intelligence | Current Polymarket sports synchronization, price history, and price/score alerts | [Sports Live](market-intelligence/sports-live.md) | [代码删除已实施，现场状态见验收](../requirements/development-runtime/sports-removal.md)；源码和入口已移除 |
| Market Intelligence | Completed ATP/WTA event synchronization, price history, status, and manual refresh | [Sports History](market-intelligence/sports-history.md) | [代码删除已实施，现场状态见验收](../requirements/development-runtime/sports-removal.md)；源码和入口已移除 |
...
| Market Intelligence | Retired Worm sports-market synchronization, rules, live state, history, and alerts | [Worm Markets 历史设计](market-intelligence/worm-markets.md) | `源码、运行入口与原 main 专属数据已退役`；目录归 Trading；五来源待发通知已核对收尾，正常重启未重建 Markets |
...
| Trading | Trader Sync 手动交易的账户接入、执行与占用、领取与完整资产历史 | [总体设计](trading/polymarket-manual-trading.md) | `设计中`；三段详细设计及八视图桌面／手机方案已确认，[完整开发规格](../superpowers/specs/2026-09-15-trader-sync-manual-trading-design.md)已整合接口、会话及服务边界和验收矩阵，待整体审阅；真实链路验证与正式实现未完成 |
| Web UI | Trader Sync 账户设置、买卖、领取、持仓、历史双视图与记录详情 | [手动交易页面设计](web-ui/trader-sync-manual-trading.md) | `页面方案已确认`；2026-09-15 用户认可[八视图桌面／手机方案](../requirements/web-ui/trader-sync-manual-trading-proposal.md)，必要辅助状态继续覆盖与验证；静态检查已完成，正式交易 UI 未实现 |
...
| Trading | Revisioned owner-scoped selection of up to 20 Solana Wallets, removal-first official-HMAC connection/activity flows, saved combinations and previews, Cash Out, and official Web JWT live-Run orchestration | [Worm Trading](trading/worm-trading.md) | `已实现`；Trading 独立拥有目录、账户复核和既有交易数据，Markets 运行依赖已移除 |
| Trading | Provider-backed Worm event catalogs and owner-scoped, revisioned market-combination CRUD with trusted display snapshots | [Worm Market Combinations](trading/worm-market-combinations.md) | `已实现`；Trading 进程内目录和保存时权威复核已落地 |
| Trading | Durable asynchronous read-only execution previews limited to the current selected Wallet revision, with frozen Wallet and market order, authoritative Worm exposure and estimates, cumulative USDC simulation, and expiring owner-scoped review | [Worm Execution Preview](trading/worm-execution-preview.md) | `已实现`；Preview worker 使用同进程 Trading 目录 |
| Trading | Current-selection-gated Run admission, Run-bound Worm Web JWT execution, purpose-bound custodial signing, browser-led Wallet-major coordination, durable mutation attempts, reconciliation, and permanent owner-scoped history | [Worm Order Execution](trading/worm-order-execution.md) | `已实现`；Markets 退役不改变执行、恢复或历史规则 |
| Trading | Current-selection exact-position, fresh-proof-authorized Worm HMAC Cash Out with whole-position market Close, at-most-once dispatch, read-only recovery, and durable historical detail | [Worm Position Cash Out](trading/worm-position-cash-out.md) | `已实现`；Markets 退役不改变平仓规则 |
| Trading | Up-to-20 selected-Wallet, Wallet-major serial Cash Out batches with complete position freezing, single-operation reuse, confirmed-USDC advancement gates, durable Wallet locks, and manual recovery controls | [Worm Position Cash Out Batches](trading/worm-position-cash-out-batches.md) | `已实现`；Markets 退役不改变批量串行、锁或恢复规则 |
...
| Blockchain Data | Finalized inbound BSC transaction indexing and lookup | [BSC Inbound Normal Transactions](blockchain-data/bsc-inbound-normal-transactions.md) | [代码删除已实施，现场状态见验收](../requirements/blockchain-data/bsc-indexer-removal.md)；源码和入口已移除 |
| Blockchain Data | Finalized BSC V2 Swap-topic transaction indexing and lookup | [BSC V2 Swap Transactions](blockchain-data/bsc-v2-swap-transactions.md) | [代码删除已实施，现场状态见验收](../requirements/blockchain-data/bsc-indexer-removal.md)；源码和入口已移除 |
...
| Web UI | Market Radar 三页、Sports 三页与 Managed OO 两页的集中视觉批次 | [v21 视觉提案](../requirements/web-ui/market-intelligence-batch-proposal.md) | `展示视觉已确认`，八页十六张主图已整批确认，见 [v21 确认记录](../requirements/web-ui/previews/theme-market-intelligence-v21-approval.json)；辅助状态继续覆盖，批准时正式 UI 实施与真实业务验收尚未完成；[Sports 代码删除、Worm 保留已实施](../requirements/development-runtime/sports-removal.md)，其余页面与共用规则保留 |
| Web UI | Worm 资产、组合与执行流程及管理员自助／Help 的集中视觉批次 | [v22 视觉提案](../requirements/web-ui/worm-and-common-batch-proposal.md) | `展示视觉已确认`，六个业务页面及六项共用界面差异的 24 张主图已整批确认，见 [v22 确认记录](../requirements/web-ui/previews/theme-worm-and-common-v22-approval.json)；反馈属于状态覆盖，不追加逐图审批。总覆盖与实施计划已整理，批准时正式重构及真实交易尚未验收；[Sports 代码删除、Worm 保留已实施](../requirements/development-runtime/sports-removal.md)，其余页面与共用规则保留 |
