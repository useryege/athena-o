...
| --- | --- | --- |
| Development Runtime | Local process supervision, persistent infrastructure, provider and scoped step-up state, Worm credential database, role-selected disabled-auth identities, dual frontend entry points, and full current-state reset lifecycle | [Local Runtime Orchestration](development-runtime/local-runtime-orchestration.md) |
...
| Identity and Access | Independent Wallet-reveal, Worm-credential, and exact-Run proofs; rate-limited Google/Solana reauthentication; fixed five-minute leases; and durable plan-bound execution authorization | [Wallet Secret and Worm Credential Reauthentication](identity-access/wallet-secret-reauthentication.md) |
