...
| Trading | Trader Sync Activity Alerts：10 人共享 WSS、全部通知同库、发送许可与撤权边界、分批摘要、未知终态与安全运行指标 | [Activity Alerts 后端技术设计](trading/trader-sync-activity-alerts.md) | 现有功能 `已实现`；独立服务的[职责、接口、事务及构建运行目标](trading/trader-sync-activity-alerts.md#已确认的独立服务目标) `已确认待实现`；[字段契约](../superpowers/specs/2026-09-13-trader-sync-grpc-contract-design.md)与[实施计划](../superpowers/plans/2026-09-13-trader-sync-independent-grpc-service.md)已确认并开始实施；[验收与外部限制](../testing/trader-sync-activity-alerts-acceptance.md) |
