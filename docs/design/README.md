...
`docs/design/` 是 ATHENA 面向开发者和 AI 代理的后端技术设计根目录。技术设计在需求确认后、编码前产生，并在实现完成后继续维护；它既能表达已确认的目标方案，也能说明当前已经运行的设计，二者必须通过状态明确区分。

目录中既有的 `web-ui/` 文档继续作为已实现界面架构说明维护；新的前端 UI 变更仍按根 `AGENTS.md` 的 UI 设计与布局确认门禁执行，不由后端工作流技能路由。

聊天记录不是事实来源。会约束实现的技术决定必须写入对应设计文档，并与 [`docs/requirements/`](../requirements/README.md) 中的已确认需求双向链接。

## 设计状态

| 状态 | 含义 | 允许的动作 |
| --- | --- | --- |
| `设计中` | 正在形成或修改目标技术方案，尚未确认 | 可以分析现状和编辑设计；不得修改依赖该方案的后端源码 |
| `已确认待实现` | 用户已确认目标技术方案，但代码尚未完全实现 | 只有收到另行明确的实现任务后才可编码 |
| `已实现` | 文档描述的设计已经由当前代码实现 | 作为当前实现说明维护；设计语义变化时同步更新 |

正常流转为 `设计中` → `已确认待实现` → `已实现`。组件边界、接口或数据契约、数据模型、事务与并发、核心流程或基础设施发生实质变化时，先退回 `设计中` 并重新确认。命名、辅助函数拆分和局部代码组织等不改变方案的实现选择无需回退。

可执行代码始终是“当前运行行为”的事实来源。`设计中` 和 `已确认待实现` 文档必须在“现状与目标差距”中清楚区分当前实现与目标方案；达到 `已实现` 后，正文只保留当前有效设计，直接替换过时内容，不保存迁移历史、废弃方案或未来计划。

## 阅读顺序

1. 阅读本索引、[设计模板](template.md)和[设计主导后端开发工作流](../developer-guide/design-led-backend-development.md)。
2. 阅读本次能力的已确认需求，以及所有覆盖受影响组件的设计文档。
3. 沿源码链接检查真实入口、边界和依赖，再开始设计或实现。
4. 完成实现前按需求逐项审查设计一致性，并将相关文档更新为当前代码事实。
...
| Trading | Trader Sync Activity Alerts：10 人链上实时订阅、结算时间边界、账户同库事务、分批摘要与未知投递终态；修订需求待整体确认 | [Activity Alerts 后端技术设计](trading/trader-sync-activity-alerts.md) | `设计中` |
...
新增子系统或可独立理解的能力时使用[设计模板](template.md)。
...
- 按稳定子系统和能力组织文档，不按日期、任务、Issue、PR、需求编号或发布版本组织。
- 每份设计都要链接对应需求；需求文档也要链接回技术设计。没有独立历史需求文档的既有能力无需虚构链接。
...
- 纯格式化、注释、文案，以及生成源、契约和设计语义均未变化的生成文件刷新等机械修改，不要求更新设计文档。修改 SQL、Proto、ABI 等生成源不属于豁免。

## 新增文档

1. 选择稳定的子系统目录和能力导向文件名。
2. 复制[设计模板](template.md)，设置为 `设计中`，并链接已确认需求。
3. 检查当前代码，在文档中区分现状、目标和差距。
4. 将文档登记到上方地图；用户确认后改为 `已确认待实现`。
5. 明确派发实现后按设计编码，同步文档和源码链接，完成一致性审查后改为 `已实现`。
6. 检查相对链接并删除所有占位说明。
