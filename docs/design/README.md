...
| Identity and Access | Independent login-only Wallet-reveal and Worm-credential proofs, rate-limited Google/Solana reauthentication, fixed five-minute scope-bound Redis leases, and resumable full-account Worm authorization | [Wallet Secret and Worm Credential Reauthentication](identity-access/wallet-secret-reauthentication.md) |
| Web UI | Google and Phantom login, responsive shell, Pending access, custodial Wallet management, Worm Assets and Combinations navigation, Connect AI, UUID-scoped caches, and administrator directory | [Application Shell](web-ui/application-shell.md) |
...
| Trading | Owner-scoped Solana balances and connection inventory, paced official-HMAC wallet bootstrap, encrypted Worm credentials, open positions and in-flight requests, and partial-failure semantics | [Worm Trading](trading/worm-trading.md) |
