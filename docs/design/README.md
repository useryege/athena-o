...
| --- | --- | --- |
| Development Runtime | Local process supervision, persistent infrastructure, provider and wallet-secret authentication state, isolated disabled-auth identity, and full current-state reset lifecycle | [Local Runtime Orchestration](development-runtime/local-runtime-orchestration.md) |
...
| Identity and Access | Login-only wallet private-key reveal, rate-limited Google or Solana reauthentication, and fixed five-minute Redis leases | [Wallet Secret Reauthentication](identity-access/wallet-secret-reauthentication.md) |
| Web UI | Google and Phantom login, responsive shell, Pending access, custodial Wallet management, Connect AI, UUID-scoped caches, and administrator directory | [Application Shell](web-ui/application-shell.md) |
