...
| Scanner application flow | [internal/token/discovery/application/scanner.go](../../../internal/token/discovery/application/scanner.go) | `Scanner.RunOnce`, `initialScanStartBlock`, `StartChain`, `StopChain` |
| EVM block source | [internal/token/adapters/evm/block_source.go](../../../internal/token/adapters/evm/block_source.go) | `LatestBlockNumber`, `DiscoverProjectCandidates` |
...
The EVM client registry caches one selected WebSocket client per chain in memory. A latest-block or block-fetch error closes and removes that cached client so a later loop can probe configured endpoints again.
