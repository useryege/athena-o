...
> 2026-09-17 重构后复核：[全栈启动与访问控制修订技术方案](../../superpowers/specs/2026-09-17-full-stack-access-reassessment-design.md)已核对当前源码，区分已完成的 Trading／Markets 重构与尚缺的十一应用编排、访问开关，并记录内部鉴权、数据库结构准备及有界停止差距。用户明确外部用户业务请求统一经过 API、服务器拒绝外部访问业务端口，并要求“先不要过重设计，这里聚焦当前目标”：本轮只做十一应用启动、API 开关、管理员页面及必要配套，三个服务内部鉴权重构不纳入，已有鉴权保留。该范围决定已确认；配置、schema 准备、接口、生命周期和验证安排已整理完成，待整体审阅，未实施。旧技术方案不应直接作为当前实施清单。
...
- **部署网络边界已确认（2026-09-17）**：服务器访问规则禁止外部用户直连业务服务端口，外部业务请求统一经过 API Server；因此本期在 API 入口控制用户访问。后续设计沿用[长期部署边界](../../developer-guide/service-development-standards.md#deployment-network-boundary)，不以默认外部端口可达为由扩大内部服务重构范围。该决定不表示网络配置已经实测。
