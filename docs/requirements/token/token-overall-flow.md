...
各环节已确认的时效及无统一完成期限的后台范围见[时效目标](token.md#已明确的正常运行时效目标尚未验证)。组件、存储、接口、并发、限流、配置与验收见[主 spec 及五份子系统设计](../../superpowers/specs/2026-09-10-token-first-block-design.md)，实际容量仍待验证。筛选、移交／撤回、买入与交易执行属于未来第二板块，本阶段不展开。
