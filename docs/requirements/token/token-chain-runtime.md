...
首版只运行 ETH（chain ID `1`）实例，不要求提供 BSC 配置。后续接入 BSC（chain ID `56`）时，使用同一程序增加 BSC 实例，并配置该链节点、聚合合约及已接入协议的部署信息；扩链前将现有免费 Etherscan Key 全部换成付费 Key，并核对接口权限。BSC 的资产范围、资产地址和估值路径留到接入时确定，当前仅记录默认原生币资金来源门槛 `0.01 BNB`。持币地址总数与前 100 名分布改由 Ave 获取，仍按项目所属链隔离。

按链拆分适用于扫描、项目事件监控和外部数据采集。每条链使用一套共享事件监控能力动态覆盖本链研究项目，不为每个项目复制业务代码或建立独立监控系统；具体进程数量和组件边界见关联目标技术设计。资料汇总、AI 静态分析、源码与报告缓存、查询 API 可以共享；它们仍须保留各项目的所属链和数据来源。本设计不要求把扫描器、事件监控、采集器和资料汇总合并成一个进程，也不确定第二板块交易执行内部的服务数量和通信方式。
...
| 观察与刷新策略 | 每条链分别配置观察时长（默认 72 小时）、仅针对 `Transfer` 的资料刷新映射和每项目每数据组冷却时间（默认 10 分钟）；`Approval` 只记录续期，部署者／当前 owner 主动交易只续期并在缺源码时重查，两者均不能配置其他资料刷新。一个实例修改或加载本链策略不得覆盖其他链 |
| L1 资产估值配置 | 按链、按资产配置直接或最多两跳的固定 DEX 现货路径；ETH 首版限于七协议可信部署，BSC 资产及具体路径在接入时确定。每次余额、池状态和价格使用同一观察区块，配置修改只影响之后刷新，不改写历史快照 |
| 资金来源门槛 | 按链配置原生币单笔最小金额，项目开始时保存快照并用于该项目后续追溯；ETH 默认 `0.01 ETH`，未来 BSC 默认 `0.01 BNB`，不将不同链数额或资产混用 |
| 交易所／跨链地址库 | 管理员按链维护地址、类型、名称和依据；同一地址在不同链的标签不能直接互用，命中节点的扩展和标签变更遵守钱包研究规则 |
...
| 项目活动 | 共享监控覆盖本链目标 Token `Approval` / `Transfer` 和部署者／当前 owner 的成功顶层主动交易。实时接收、历史补查、活动摘要及期限均关联链与项目；`Approval` 仅记录续期，`Transfer` 续期、缺源码重查并按配置刷新，监控地址主动交易仅续期和缺源码重查。所有动作先满足期限资格 |
...
项目与研究任务保存后，由本链共享事件监控独立接管动态目标。目标 Token `Approval` / `Transfer`、部署者／当前 owner 主动交易与 Swap 均以实时接收降低延迟、以历史补查保证启动、断线、重启、动态目标变化及登记时差期间不漏事实。Token 日志和 Swap 从部署区块开始补查；部署者从部署交易之后开始，当前 owner 从成功读取它的观察区块开始，不能将顶层交易覆盖等同于日志覆盖。

部署者／当前 owner 的主动交易必须成功、属于顶层且 `from` 为监控地址，零原生币合约调用仍有效；入账、失败交易和内部调用无效，也不使用资金来源金额门槛。owner 更换后切换 owner 监控目标，部署者始终保留；地址或角色重合时，同一项目同一交易只处理一次。活动区块时间不晚于处理前当前期限才允许续期及其对应动作；迟到活动只保留依据，不续期、重查源码或刷新。`Transfer` 默认刷新 Token 当前状态和 Ave 持币资料，`Approval` 没有源码重查和资料刷新，主动交易没有其他资料刷新。

新增目标先开始实时接收，再固定补查终点，重叠范围去重；动态目标变化不得形成未覆盖空档，断线或重启后从持久覆盖进度恢复，内存接收缓冲不作为完成依据。确认不活跃超时前，`Approval`、`Transfer`、部署者交易、各有效监控区间的当前 owner 交易及 Swap 必须全部完整覆盖越过当前期限，且期限内没有待处理的有效活动或 Swap。地址分片、订阅重建、请求并发和持久化结构见关联目标技术设计。
...
本轮同步需求与五份目标技术设计，未修改程序或部署配置。单链实例边界保持已确认，Token 整体及书面规格仍待整包审阅。当前源码行为见[现有扫描器](../../design/token-intelligence/chain-processor.md)和[六源画像](../../design/token-intelligence/collection-profile.md)；目标架构见[主 spec](../../superpowers/specs/2026-09-10-token-first-block-design.md)，已确认时效见[目标表](token.md#已明确的正常运行时效目标尚未验证)。
