...
首版只运行 ETH（chain ID `1`）实例，不要求提供 BSC 配置。后续接入 BSC（chain ID `56`）时，使用同一程序增加 BSC 实例，并配置该链节点、聚合合约及已接入协议的部署信息；扩链前将现有免费 Etherscan Key 全部换成付费 Key，并核对接口权限。持币数据继续使用专用 PRO Key。

按链拆分适用于扫描、活动监控和外部数据采集。资料汇总、AI 静态分析、源码与报告缓存、查询 API 可以共享；它们仍须保留各项目的所属链和数据来源。本设计不要求把扫描器、采集器和资料汇总合并成一个进程，也不确定第二板块交易执行内部的服务数量和通信方式。
...
```mermaid
flowchart TD
    code["同一套代码与构建产物<br/>同类程序按启动配置选择唯一链"] --> eth["ETH 单链任务组 · chain_id = 1<br/>扫描、活动和 Swap 监控、外部采集"]
    code -.-> bsc["后续 BSC 单链任务组 · chain_id = 56<br/>扫描、活动和 Swap 监控、外部采集"]
...
| 部分 | 目标责任与隔离方式 |
| --- | --- |
| 区块扫描与 Token 识别 | 只处理启动链的区块，按该链恢复和保存扫描进度；每条链内部仍按区块高度顺序执行 |
| 项目活动 | 仅匹配启动链内目标 Token 的 Approval/Transfer；活动依据和产生的源码更新任务关联实际链及项目 |
| 首次 Swap 监控 | 订阅、补查、池或管理合约身份、代币映射及日志覆盖进度均有明确链范围；不能将 ETH 的协议地址直接当作 BSC 部署使用 |
...
每个扫描实例完成当前链某块的全部候选 Token 识别，并可靠保存识别结果、项目资料和研究任务后，才推进该链进度。没有部署候选的区块也保存完成进度；任务保存与游标推进应保持一致。源码、AI、五层钱包及其他研究后台独立执行，不阻塞后续区块扫描。

项目首次实际 Swap 仍结束研究，未交接项目放弃本轮参与；拆分进程不改变截止条件或在途任务收尾仍待细化的边界。扫描失败重试、通知和 `H < C` 人工处理继续按 [区块扫描设计](token-block-scan-flow.md) 执行；本阶段仍不考虑区块替换或链重组。
