...
新增目标先开始实时接收，再固定补查终点，重叠范围去重；动态目标变化不得形成未覆盖空档，断线或重启后从持久覆盖进度恢复，内存接收缓冲不作为完成依据。确认不活跃超时前，`Approval`、`Transfer`、部署者交易、各有效监控区间的当前 owner 交易及 Swap 必须全部完整覆盖越过当前期限，且期限内没有待处理的有效活动或 Swap。地址分片、订阅重建、请求并发和持久化结构后续技术设计。
