...
| 项目活动 | 共享监控为本链所有研究中项目维护目标 Token Approval/Transfer 动态目标；实时订阅、历史补查、活动摘要、观察期限及配置刷新任务均关联实际链及项目；无非空源码时强制触发源码查询 |
...
项目与研究任务保存后，由本链共享事件监控独立接管动态目标。Approval/Transfer 与 Swap 都以实时订阅降低延迟、以历史日志补查保证启动、断线、重启、动态目标变化及登记时差期间不漏事件；新增目标先建立实时接收，再固定终点并从部署区块补查，重叠范围去重。Approval/Transfer 覆盖所有研究中项目，用于期限续期和按链配置刷新；动态目标变化不得形成未覆盖空档，断线或重启后从持久覆盖进度恢复；内存接收缓冲不作为完成依据。地址分片、订阅重建、请求并发和持久化结构后续技术设计。
