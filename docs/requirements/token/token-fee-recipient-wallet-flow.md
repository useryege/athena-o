...
取得的地址作为 L1 直接关联钱包，追加“税费接收钱包”标签，采集历史交易、分析部署与代币交互、在项目门槛和终止节点边界内追溯资金来源，并读取六项资产余额与 USDT 估值。同一地址还可以是部署者、owner 或初始接收钱包，各角色和依据同时保留。税费接收钱包可以是合约地址，数量不受“最多 5 个初始接收钱包”限制。
...
flowchart TD
    ...
    related --> history["每个去重钱包部署前最近最多 300 笔<br/>分析直接部署、代币交互<br/>单笔成功直接入账达到项目门槛才追溯 L2–L5<br/>交易所 / 跨链节点完成本层分析后停止上溯"]
...
图中的固定地址与链上读取分支可以同时存在，分别处理每个接收方；不将整个合约强制归为二选一。部分方式读取失败时保留其他成功结果。流程图表达后台研究任务的数据依赖，不要求税费接收方、owner、静态报告或其他资料共同完成才继续研究，也不阻塞该块识别结果、项目资料及研究任务保存后的区块推进。

税费接收地址只可被管理员配置为具有续期资格的目标 Token `Transfer` 所触发的刷新组；`Approval` 和部署者／当前 owner 主动交易不得触发该资料刷新。刷新仍使用执行时最新区块，只保留最近成功的当前地址集合，失败另存最近时间与原因。每次成功结果替换对应税费用途的当前地址集合；新增地址自动成为 L1，执行其固定 `0..B-1` 历史、六项资产余额与 USDT 估值及 L2–L5 研究。被移出的旧地址失去相应当前税费角色，但保留仍然成立的其他角色与已有事实。
...
- 补充同批交易回执与日志，分析成功直接部署过的合约和代币交互；明确买卖首版只识别 Uniswap V2/V3/V4、Sushi V2/V3、PancakeSwap V2/V3，未支持路径保留未分类事实。
- 使用项目开始时保存的原生币门槛快照，从单笔成功、顶层、直接转入且达到门槛的 ETH 交易提取发送地址；ETH 默认 `0.01 ETH`，多笔小额不累计，小额交易仍保留在原始历史中。后台自动逐层追溯 L2–L5，复用各钱包的同范围历史分析并保留共同来源和循环关系。首版不扩展内部转账或代币入账来源；L5 不继续扩展 L6。
- 任一节点命中管理员按链维护的交易所／跨链地址库时，仍完成本层最多 300 笔历史与行为分析并保留已有资金边，但不再发现更上游来源。标签新增、移除和项目停止后的处理沿用统一资金来源规则。
...
读取描述的具体数据结构、不同入口结果冲突处理、零地址等特殊返回值的解释、技术失败重试、刷新版本与任务协调仍待技术设计。新地址替换当前角色并自动完成其 L1 初始研究、旧地址保留其他角色的业务语义已经确定。当前不添加地址数量上限、特殊返回值排除条件或新的研究完成门槛。
