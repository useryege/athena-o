...
税费接收地址可被管理员配置为具有续期资格的 `Approval` 或 `Transfer` 活动触发的刷新组。刷新仍使用执行时最新区块，只保留最近成功的当前地址集合，失败另存最近时间与原因。每次成功结果替换对应税费用途的当前地址集合；新增地址自动成为 L1，执行其固定 `0..B-1` 历史、五项资产余额及 L2–L5 研究。被移出的旧地址失去相应当前税费角色，但保留仍然成立的其他角色与已有事实。
...
- 按已有规则从成功、金额大于零的直接 ETH 入账提取发送地址，后台自动逐层追溯 L2–L5，复用各钱包的同范围历史分析并保留共同来源和循环关系。首版不扩展内部转账或代币入账来源；L5 不继续扩展 L6。展示五层实际结果，展开与折叠不触发取数。
- 税费接收钱包属于 L1，读取余额采集时最新区块的 WETH、USDT、USDC、DAI、WBTC，保留区块与时间。不采集原生 ETH，L2–L5 不采集余额。
