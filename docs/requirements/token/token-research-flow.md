...
flowchart TD
    ...
    activityEligible -->|是| renew["重置观察期限"]
    activityEligible -->|是| refresh["按事件类型配置刷新资料"]
...
| 源码 | 只复用非空源码；成功空内容与技术失败分开。研究中无非空源码时，每个具有续期资格的活动都可触发或合并重查；不定时轮询。 |
...
- 所有研究中项目持续接收合法 `Approval` / `Transfer`。事件时间处于当前期限内（含期限时点）时，零值、撤销授权、mint、burn、自转都续期，使用事件区块时间，次数不限；晚于当前期限的活动不续期。
- `Approval` 和 `Transfer` 分别配置资料刷新组。对具有续期资格的活动，续期和缺少非空源码时重查源码不可关闭；默认 `Approval` 无其他刷新，默认 `Transfer` 刷新 Token 当前状态和 Ave 持币资料。
