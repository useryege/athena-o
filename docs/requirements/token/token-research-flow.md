...
flowchart TD
    ...
    materials --> wallets["L1–L5 固定历史与关系研究<br/>按项目门槛发现资金来源，交易所 / 跨链节点停止上溯<br/>仅 L1 读取六项资产并逐项估值为 USDT"]
...
| L1–L5 钱包历史与资金边 | 每钱包固定查询部署前 `0..B-1`、desc、第一页最多 300 笔。单笔成功、直接原生币顶层入账达到项目开始时保存的门槛快照才形成资金边；ETH 默认 `0.01 ETH`，未来 BSC 默认 `0.01 BNB`。小额保留原始历史但不形成资金边或上游节点，多笔不累计。L5 不扩展 L6；普通活动不整体刷新固定历史或关系图。 |
| 交易所／跨链终止节点 | 管理员按链维护地址、类型、名称和依据。L1–L5 任一节点命中后仍完成本层最多 300 笔历史及行为分析，但不发现上游。研究中新增标签停止后续扩展、保留事实且允许在途任务收尾而不继续派生；移除后可恢复尚未执行的上溯，已停止项目不重开。 |
