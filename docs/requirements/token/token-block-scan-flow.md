...
项目开始时另保存本链原生币资金来源门槛快照，ETH 默认 `0.01 ETH`，未来 BSC 默认 `0.01 BNB`。该门槛只用于后台钱包历史的单笔资金来源识别，不影响区块发现、Token 身份或零金额主动交易续期。L1 六项资产同区块 USDT 估值、L1–L5 历史分析及交易所／跨链终止节点规则由 [钱包研究流程](token-wallet-research-flow.md)承接，不等待这些研究完成才推进扫描。
