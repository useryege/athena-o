...
- 本图的区块高度、扫描游标与任务始终属于实例选择的链；未来 BSC 实例独立运行本流程，不共用 ETH 游标。研究期活动与 Swap 日志由同一链的共享事件监控独立处理，也不得跨链共用覆盖进度或协议部署地址。
...
采用方案 A：当前区块的候选发现完成后，汇总该块全部候选并使用聚合合约批量判断；完成全部候选的 Token 识别，可靠保存识别结果、项目资料和对应研究任务后，才推进区块进度。没有部署候选的区块也保存完成进度。源码、AI、五层钱包和其他研究资料由后台独立执行，不等待这些任务结束，也不等待项目首次 Swap 才推进区块。
...
区块扫描不处理研究期目标 Token 的 Approval/Transfer 或 Swap 日志。可靠保存项目与研究任务后，按链共享事件监控独立接管动态目标：对仍在研究且尚无非空源码的项目接收 Approval/Transfer，对所有仍在研究的项目匹配可信 Swap；两类日志都采用实时订阅与历史补查防漏。项目登记时从部署区块开始补查，事件监控积压或短暂故障不回退已经成功完成的扫描区块，见 [项目事件监控](token-project-activity-flow.md)与[Swap 事件调研](token-swap-events-research.md)。
