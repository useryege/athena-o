...
区块扫描不处理研究期目标 Token 的 Approval/Transfer 或 Swap 日志。可靠保存项目与研究任务后，按链共享事件监控独立接管：为所有研究中项目接收 Approval/Transfer，其中事件区块时间不晚于当前期限的活动用于续期和按配置刷新，迟到活动只保留依据；同时匹配首版七协议 Swap。两类日志都采用实时订阅与历史补查防漏。项目登记时从部署区块开始补查，事件监控积压或短暂故障不回退已经成功完成的扫描区块，见 [项目事件监控](token-project-activity-flow.md)与[Swap 事件调研](token-swap-events-research.md)。
