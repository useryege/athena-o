...
三天起点定位算法、实际并行获取数、节点请求限流与背压、缓冲数据的具体形式与字节级资源保护、重试间隔、通知提交失败处理、人工恢复见关联目标技术设计，尚未实现；已确认的需求是本轮最多 100 块、并行预取与严格串行处理同时运行。研究期事件监控的连续性与业务边界见独立文档，不纳入扫描器内部。当前不考虑区块替换或链重组，不增加回滚修复。图中异常通知属于未来程序行为，本次文档整理不会触发这些运行告警。
...
扫描同步保存部署身份、Token 识别结果、实际观察区块和代码哈希；部署回执解析及最多 5 个初始接收钱包的提取改由已可靠登记的后台任务完成，不阻塞发现游标。该拆分不改变初始接收钱包的选取条件或研究范围。

每次真实扫描失败各自可靠保存通知记录，再异步交入系统通知组件；传输重试复用同一次失败身份，不合并不同失败。扫描责任以可靠交入通知组件为界，最终投递使用该组件现有机制，通知服务暂不可用不阻止对应区块重试。
...
当前不考虑区块被替换时的处理。断线恢复补查用于防止日志和主动交易漏数据，不属于链重组回滚设计。正常追平的新 Token 发现已确认采用 [P95 ≤ 15 秒目标](token.md#已明确的正常运行时效目标尚未验证)，首次三天发现补扫以 [2 小时内追平](token.md#首次三天发现补扫)为目标；其他时效与恢复范围见[活动刷新、恢复与停止](token.md#活动刷新恢复与停止)。本次不新增 API、数据库字段或内部组件设计。
