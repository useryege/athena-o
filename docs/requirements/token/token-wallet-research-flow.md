...
>
> 关联技术设计：完整目标未创建；普通交易采集现状见文末链接。已确认单项决定不等于整份需求确认或后端实现授权。
...
跨项目复用钱包分析时，需匹配链、钱包地址、截止区块、历史类型和采样规则。钱包历史会随时间变化，不能只按地址永久复用某次 300 条结果，也不能用普通交易资料代替内部交易资料。具体缓存结构及分析规则变化后的更新方式后续设计。
