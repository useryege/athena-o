...
图中的手动补采针对研究中的项目；已经结束项目的独立源码维护入口另行设计，不自动恢复研究。尚未取得源码时支持管理员手动补采。项目研究期间，按链共享事件监控通过实时订阅与从部署区块开始的历史补查接收目标 Token 新的 `Approval` 授权事件或 `Transfer` 代币转账事件，去重后发出项目更新事件；处理时若该项目仍在研究且仍无非空源码，再次尝试获取。活动检测针对当前目标 Token 的事件，不修改关联钱包历史固定的部署前 300 笔范围，也不把其他代币或其他钱包活动自动当作该项目的更新依据。已取得非空源码时退出这类动态监控目标并继续复用，不因每个新事件重复请求源码或重新生成 AI 报告；Swap 监控仍持续到研究结束。
