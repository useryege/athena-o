...
> 需求状态：讨论中
>
> 细分状态：目标设计草案，尚未完整实现。当前已有按代码哈希复用资料的机制；本图还包含待实施的手动补采、指定活动触发补采和请求重试规则。
...
flowchart TD
    ...
    H --> D
    G -->|成功，源码为空| U["保存空源码结果及查询时间<br/>可能尚未开源<br/>等待手动补采或指定活动触发更新"]
    G -->|请求失败| I["记录实际错误"]
    I --> K{"已重试次数小于 3？"}
    K -->|是| L["重试次数加 1"]
    L --> E
    ...
    ACTIVITY["按链共享事件监控实时接收并补查<br/>具有续期资格的 Transfer<br/>或部署者 / 当前 owner 主动交易"] --> UPDATE["发出源码补查触发"]
    UPDATE --> ELIGIBLE{"项目仍研究中且尚无非空源码？"}
    ELIGIBLE -->|是| B
    ELIGIBLE -->|否| SKIP["不重复下载源码<br/>活动自身的其他业务动作照常执行"]
...
| 请求成功但源码为空 | 保存成功空响应、链、地址与查询时间，表示可能尚未开源；支持手动补采。研究期间由具有续期资格的新目标 Token `Transfer`，或部署者／当前 owner 发出的成功顶层交易触发更新后再次查询，不触发请求失败重试；`Approval` 不触发 |
...
图中的手动补采只面向研究中且尚无非空源码的项目；项目停止后不提供项目专属手动补采或刷新入口。所有研究中项目持续覆盖目标 Token 的 `Approval`、`Transfer` 以及部署者／当前 owner 主动交易。去重后，只有活动区块时间不晚于处理前当前期限的活动才有资格执行其动作：`Approval` 仅续期，永远不进入本源码补查流程；`Transfer` 续期、触发本流程并可按链配置刷新；部署者／当前 owner 发出的成功顶层交易续期并触发本流程，但不触发其他资料刷新。晚于期限的活动只保留证据。已有非空源码继续复用，不因每个活动重复下载或自动重新生成 AI 报告。活动检测不修改关联钱包固定的部署前 `0..B-1` 窗口：普通与内部交易分别倒序取第一页最多 300 条原始记录。新增 [内部 ETH 转账识别](token-wallet-internal-transfers-flow.md)属于历史钱包研究，不构成本流程的源码补查触发。

该补采方式由手动操作、具有续期资格的 `Transfer` 或监控地址主动交易触发，不设置定时无限轮询。每次真正发起查询仍按成功非空、成功为空、请求失败分别处理；同一项目已经执行或处于冷却中的源码请求需要合并，具体协调与调度方式后续确定。
...
取得或复用非空源码后，进入 [AI 静态质检流程](token-contract-review-flow.md)：先查该源码已有的完成报告，命中即复用；没有报告才调用 AI 静态分析，必查是否可无限增发、是否可修改余额、是否可限制转出、是否可绕过持有人授权转账、是否在放弃 owner 后仍保留控制权五项事实，逐项保存权限、条件、影响及代码依据，同时保留代理与外部未知依赖识别。管理员可针对某份共享源码手动重新检测。AI 只生成事实，不评级、不筛选、不排除项目。空源码及请求失败分支不生成成功报告，也不伪造否定事实。
...
失败重试间隔、通知提交失败时的处理、活动触发请求合并、同一哈希的并发请求协调及数据结构见关联目标技术设计，尚未实现。空源码已确认采用研究中管理员手动补采，以及具有续期资格的新 `Transfer` 或部署者／当前 owner 主动交易触发，不另设定时轮询；`Approval` 明确不触发源码补查。通知组件自身的投递和重试机制沿用现有设计。源码缺失只作为事实，不形成过滤条件。该有限失败重试上限不替代 [区块失败的无限重试规则](token-block-scan-flow.md)。
