...
失败重试间隔、通知提交失败时的处理、活动触发请求合并、同一哈希的并发请求协调及数据结构仍待技术设计。空源码已确认采用研究中管理员手动补采，以及具有续期资格的新 `Transfer` 或部署者／当前 owner 主动交易触发，不另设定时轮询；`Approval` 明确不触发源码补查。通知组件自身的投递和重试机制沿用现有设计。源码缺失只作为事实，不形成过滤条件。该有限失败重试上限不替代 [区块失败的无限重试规则](token-block-scan-flow.md)。
