...
> 关联目标技术设计：[对应子系统方案](../../design/token-intelligence/research-task-runtime.md)（分节已确认，书面待审阅；尚未实现）。

> 需求状态：讨论中
>
> 细分状态：持币数据来源与采集范围已明确，异常处理、刷新和读取模型已形成目标设计，尚未实现。首版仅研究 Ethereum Mainnet；持币资料改由 Ave 提供，不再设计 Etherscan PRO Key 或完整分页持币名单。

## 入口与目的

按项目所属链和目标代币合约地址，从 Ave 获取两类当前持币资料：持币地址总数，以及 Ave 排名中的前 100 个持币地址。前 100 名逐项展示具体地址、持有数量、持有占比和 Ave 提供的地址标签。

持币资料依赖链和目标代币地址，不依赖合约已开源，也不等待 AI 源码分析。本板块只采集、整理、保存和展示资料，不增加持仓集中度评分、筛选阈值或项目排除规则。源码 AI 同样只生成事实报告和依据，见 [AI 静态质检流程](token-contract-review-flow.md)。
...
Ave 持币资料可被管理员配置为具有续期资格的目标 Token `Transfer` 所触发的刷新组，并属于 `Transfer` 的默认刷新内容。`Approval` 和部署者／当前 owner 主动交易不得触发持币资料刷新。项目专属结果只展示最近一次成功事实；失败时保留旧值并单独记录最近失败时间和原因。项目因首次已识别 Swap、不活跃超时或管理员手动操作停止时，未开始任务取消，已开始任务按原有限重试规则收尾，之后冻结持币快照且不再手动刷新。具体衔接见 [项目事实研究流程](token-research-flow.md)。
...
Ave token-id 的链映射、认证配置复用、两项请求的协调、错误、重试、刷新、标签和存储见研究运行时及查询管理目标设计。还需在接入时核对新部署研究中 Token 是否能及时出现在 Ave 两个接口中；接口暂未收录时记录“提供方暂无数据”，不回退到 Etherscan 补齐。

本轮只确定业务数据来源、Top 100 范围、目标字段和研究边界，不固定内部任务、数据库或 API 契约。需求文档仍为 `讨论中`；后续任务按 [Superpowers 工作流](../../developer-guide/superpowers-development.md)收敛未决问题并开展方案设计。
