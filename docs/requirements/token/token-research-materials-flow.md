...
flowchart TD
    ...
    subgraph activity["项目活动更新与研究截止"]
        ACT["共享事件监控实时接收并补查<br/>目标 Token 的 Approval 或 Transfer"]
        UPDATE["产生项目更新事件"]
        NEED{"项目仍研究中且尚无非空源码？"}
        MANUAL["管理员手动补采源码"]
...
| 项目活动与研究截止 | 按链共享事件监控通过实时订阅与历史补查接收两类日志：目标 Token 新 `Approval`／`Transfer` 只为研究中且尚无非空源码的项目触发更新；可信协议交易场所的实际 Swap 覆盖全部研究项目。已有非空源码继续复用，不定时无限轮询。Swap 识别不限定 V2／V3／V4，可补充匹配所需池身份与 token 映射，首个可信 Swap 立即结束研究并停止自动补采，同时补齐更早范围以确定链上首笔，未交接项目放弃本轮参与。详见 [项目事件监控](token-project-activity-flow.md) 与 [Swap 事件识别](token-swap-events-research.md)。 |
...
尚无源码时支持管理员手动补采。研究期间，按链共享事件监控通过实时订阅和从部署区块开始的历史补查接收目标 Token 新 `Approval` 或 `Transfer` 事件，去重后发出更新事件；项目处理时仍在研究且仍无非空源码才再次获取，有源码则继续复用并退出活动动态目标。补采由手动操作或新活动触发，不定时无限轮询，也不因活动重复下载已有非空源码或自动重做 AI 报告。事件聚合和同项目在途请求协调后续细化；研究结束后停止活动驱动的自动补采。这些目标 Token 新活动与各钱包部署前 300 笔历史是两类资料，不改变历史采样边界，也不将授权或普通转账直接视为 Swap。
...
持币地址采集无需等待源码、AI 报告或 owner；直接保存 Ave 返回的持有数量、持有占比和提供方标签，已有角色匹配在对应依据可用时补充。标签缺失时不推测，Top100 实际少于 100 条时保留实际条数，不自动判为失败。当前只展示持币信息，不增加集中度阈值或筛选规则，也不扩大既定 L1–L5 范围。首次真实 Swap 截止不等待持币资料完成；后续程序按管理员规则提前排除时同样不等待。已启动采集如何收尾及保存结果继续细化。
...
- owner 缺失的基本处理、源码手动补采，以及按链共享监控通过实时订阅与历史补查接收研究中目标 Token 新 `Approval`／`Transfer` 并触发更新已明确；其他采集启动与更新、活动聚合、在途请求协调和资料矛盾处理继续设计。成功空源码、请求失败与其他资料缺失分别保存，当前都不直接形成筛选不通过结论。
- 关联钱包后台自动采集 L1–L5 并展示五层实际结果；折叠只影响显示。当前不据钱包资料筛选过滤；前置 Token 识别沿用既定逻辑，后续程序对源码报告的风险规则另行设计。
- 本阶段仅为真实 Swap 匹配保留必要池身份与 token 映射，不扩展为底池深度、储备、LP 或流动性研究；Ave 仅采集持币地址总数与 Top 100 分布，不采集市场、池或风险标签，也不引入系统自有的合约代码黑名单和钱包黑名单。模拟结果暂不作为必采项，按需采集方式与用途仍待设计。
