...
连线只表示后台研究任务的资料依赖，不要求本板块与静态质检、owner、税费接收钱包或税率分析合并为一次 AI 调用，也不要求互相等待。该块识别结果、项目资料及研究任务保存后即可推进区块，不等待本项完成。源码 AI 当前只生成事实报告和依据，不评级或自动排除；后续由程序按管理员维护的标准生成风险等级，判高风险时才按对应规则排除项目。

项目首次真实 Swap 为研究截止，识别不限定 V2、V3 或 V4；不等待链接提取完成。首次 Swap 时尚未交接的项目结束研究并放弃本轮参与；结束研究不代表资料齐备、高风险或可以买入。管理员手动重检共享报告不自动恢复项目，具体衔接见 [项目研究与筛选流程](token-research-flow.md)。
