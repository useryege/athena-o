...
> 需求状态：讨论中
>
> 细分状态：目标设计草案，尚未实现。本图记录已确认的 AI 源码链接识别、同源码结果复用及研究资料展示范围。

首版仅研究 Ethereum Mainnet，源码查询使用免费 Etherscan Key；后续扩展其他链时现有免费 Key 全部替换为付费 Key。链接提取仍只处理已经取得的源码。
...
连线只表示后台研究任务的资料依赖，不要求本板块与静态质检、owner、税费接收钱包或税率分析合并为一次 AI 调用，也不要求互相等待。该块识别结果、项目资料及研究任务保存后即可推进区块，不等待本项完成。源码 AI 当前只生成事实报告和依据，不评级、不筛选或自动排除。

公开链接是按 `code_hash` 共享的静态产物。项目因首次已识别 Swap、不活跃超时或管理员手动操作停止时，未开始任务取消，已开始任务按原有限重试规则完成；报告是否完成不延后停止。其他研究中项目或管理员重检产生新版时，已停止项目也展示最新共享有效版本，但不恢复研究或触发项目专属采集。具体衔接见 [项目事实研究流程](token-research-flow.md)。
...
提取结构、证据、归属冲突、技术重试及共享更新见[源码事实目标设计](../../design/token-intelligence/source-facts-and-read-plans.md)。首版提取实际 HTTPS 链接，其他协议扩展留到后续；不增加网页采集、固定链接数上限或研究完成门槛，不涉及 UI 实现。
