...
- 后台自动采集 L1–L5 钱包资料并展示五层结果；Swap 到来时按统一任务收尾规则取消未开始任务，让已开始任务在有限重试后完成，不等待五层齐备。
- 按链共享事件监控为所有研究中项目接收 `Approval` / `Transfer`，用于观察续期并按管理员配置触发资料刷新；项目无非空源码时必定触发或合并源码查询。授权或转账不等于成交。
...
共享监控维护两类动态目标：Approval/Transfer 覆盖所有研究中项目的 Token 地址；Swap 按首版七协议的可信 Pair、Pool、PoolManager 等发出方与事件主题接收，再根据池或 `poolId` 的实际币种映射匹配所有研究中项目。两类目标共享连续性原则，但事件解析和业务动作不能混用。
