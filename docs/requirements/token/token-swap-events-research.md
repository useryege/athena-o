...
> 需求状态：讨论中
>
> 细分状态：2026-09-08 调研记录与目标设计建议，尚未实现。首版范围为 Ethereum Mainnet（`chain_id = 1`）。本文的事件清单、接入顺序及技术方案供后续选择，不代表已确认全部接入或已经覆盖所有协议。

## 业务目的与已确认边界

系统研究尚未开启交易的 Token 项目。一旦识别到链上首次涉及该项目的 Swap，即结束该项目研究，不等待资料全部采集完成；尚未交接的项目同时放弃本轮参与。已交接项目的监控与交易执行规则不在本文确定。

这里需要识别不同协议中的实际交换事件，不限定事件名称必须是 `Swap`，也不限定 Uniswap V2、固定报价资产或固定 Router。仅出现池创建、添加流动性、普通转账或授权，不直接等于已发生 Swap。

与本次调研相关的其他决定：

- AI 只生成源码静态质检报告，不输出风险评级；报告明确描述无限增发、修改余额、限制转出等能力，未来由程序按管理员维护的标准生成评级。Swap 截止不依赖 AI 报告或评级完成。
- 后台自动采集 L1–L5 钱包资料，后续展示五层结果；首次 Swap 到来时不因五层资料尚未齐备而延长研究窗口。
- 区块扫描中检测项目合约发出的 `Approval` / `Transfer` 活动，用于触发无源码项目更新源码查询。这是源码补采触发，与首次 Swap 结束研究是两条不同规则，不能把授权或转账直接当作成交。
...
建议先建立 Uniswap V2、V3、V4 的识别能力，同时评估可复用部分事件解析的 Sushi 与 PancakeSwap；随后按实际项目样本补齐 Curve、Fluid、Maverick、Balancer、DODO 等事件族。接入顺序仍需选择，不能因后面的协议暂未实现便判断项目尚未交易。
...
仅订阅项目 Token 地址的日志会漏掉以上大多数 Swap，因为日志通常由池、Vault 或 PoolManager 发出。建议维护最低限度的交易场所身份与币种映射，为事件匹配服务；不因此扩展到流动性、深度、价格、底池质量或买入路径研究。
...
任意合约都可以发出同名甚至相同 topic0 的事件。因此建议组合校验链、可信发出方或池归属、协议版本 ABI、日志布局和币种映射，再以目标 Token 的合约地址匹配项目，不能凭符号、名称、调用函数名或 topic0 单独结束研究。

池创建、初始化和币种映射是识别基础，不是截止事件。多资产池只匹配本次实际交换的币种，不因目标 Token 出现在池的其他币种位置就视为它已经成交。不同协议的 native ETH / WETH 表达分别按协议解释。

## 订阅与历史补查建议

Geth 官方说明，订阅只发送当前事件，不补发历史；连接关闭时该连接上的订阅被移除。因此建议使用实时日志订阅降低发现延迟，并使用 `eth_getLogs` 补足历史、断线和登记时差。这里不改变项目发现扫描已有的逐块轮询规则，两者是不同职责。[Geth 日志订阅](https://geth.ethereum.org/docs/interacting-with-geth/rpc/pubsub)

| 符号 | 含义 |
| --- | --- |
| `B` | 当前项目部署区块，首次 Swap 补查包含该区块 |
| `H` | 本次补查确定的终点高度，不使用查询期间不断移动的隐含终点 |
| 日志覆盖游标 | 对选定协议和过滤范围已经完整取得并持久保存的连续区块进度，不等于最后收到一条日志的高度 |

建议流程如下；具体进程、存储结构与调度尚未确定。
...
flowchart TD
    ...
    evidence --> endResearch["结束项目研究，保留已有资料<br/>尚未交接的项目放弃本轮参与"]
...
2. 项目首次登记时补查 `B..H`，两端包含。该范围不同于钱包历史的 `0..B-1`；部署和首次 Swap 可以发生在同一区块，不能从 `B+1` 才开始。
...
6. 若某条可信事件已证明项目成交，可据此停止继续研究；要把它标为链上最早事件，仍须补齐此前尚未覆盖的范围。记录事件发生时间和系统发现时间，不能把迟发现包装为实时发现。
...
未发现匹配事件，只说明在已经接入的协议、实际完成的区块范围和已解析资料中尚未发现；不能直接声称该项目在 Ethereum 全部交易场所都没有交易。对于协议未接入、未知池、版本不匹配、映射缺失或技术请求失败，应保留具体原因，不伪造“确认未开盘”的结论。

可信 Swap 证明发生了协议定义的交换路径，不自动证明任意钱包现在都能正常买卖。例如 V2 存在 flash swap，V4 支持 hooks。零实际交换、纯同币借还等边界如何纳入本业务的首笔交易口径，需要在事件适配规则中明确；本次不擅自增加成交金额、钱包身份或买卖方向阈值。[V2 Pair 实现](https://raw.githubusercontent.com/Uniswap/v2-core/master/contracts/UniswapV2Pair.sol)、[V4 PoolManager 实现](https://raw.githubusercontent.com/Uniswap/v4-core/main/src/PoolManager.sol)

实施前尚需选择或细化：

- 首批协议与版本清单，以及各 Ethereum 部署对应的 ABI；当前源码分支可能继续变化，不能只按协议品牌选择解码器。
- 实时日志接收、历史补查、协议解析、项目关联和截止写入的具体职责与持久化边界。
- 首次 Swap 截止与正在执行的项目采集任务如何协作，哪些已发出请求完成后仍保存结果；不因实现细节延长研究窗口。
- RFQ、订单成交和结算事件是否进入同一截止规则，及多路由、多池、重复依据的合并方式。
- 最低成交语义、解析失败处理和覆盖状态表达；不把所有名为 Swap 的日志直接当成业务成交。

本文只支撑首次交易识别和研究截止，不负责完整的钱包买卖归因、交易路由构建、下单或卖出策略。返回 [Token 目标设计](token.md) 或 [研究资料索引](README.md)。
