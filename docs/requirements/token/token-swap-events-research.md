...
> 需求状态：讨论中
>
> 细分状态：2026-09-08 调研记录，尚未实现。首版范围为 Ethereum Mainnet（`chain_id = 1`）。按链共享事件监控、实时订阅与历史补查的连续性规则已经确认；事件清单、首批协议及接入顺序仍供后续选择，不代表已经覆盖所有协议。
...
- 按链共享事件监控通过实时订阅与历史补查接收项目合约发出的 `Approval` / `Transfer` 活动，只为仍在研究且尚无非空源码的项目触发源码更新。这与首次 Swap 结束研究是两条不同规则，不能把授权或转账直接当作成交。
...
仅订阅项目 Token 地址的日志会漏掉以上大多数 Swap，因为日志通常由池、Vault 或 PoolManager 发出。必须维护最低限度的交易场所身份与币种映射，为事件匹配服务；不因此扩展到流动性、深度、价格、底池质量或买入路径研究。
...
共享监控维护两类动态目标：Approval/Transfer 按仍在研究且尚无非空源码的 Token 地址过滤；Swap 按已接入协议的可信 Pair、Pool、Vault、PoolManager 等发出方与事件主题接收，再根据池或 `poolId` 的实际币种映射匹配所有仍在研究的项目。两类目标共享连续性原则，但事件解析和业务动作不能混用。
