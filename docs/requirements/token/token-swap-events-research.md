...
以下边界已在关联目标技术设计中展开；精确部署 ABI/起块在实施配置时核验，未来协议仍不在本轮范围：
...
- 正常追平时，已登记项目的 Swap 停止已确认采用 [P95 ≤ 15 秒目标](token.md#已明确的正常运行时效目标尚未验证)；一小时以内断线在上游恢复后 5 分钟内追平，更长故障及大段历史补查展示进度和预计耗时，完整范围见[活动刷新、恢复与停止](token.md#活动刷新恢复与停止)。
