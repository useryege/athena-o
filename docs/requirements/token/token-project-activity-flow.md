...
| --- | --- | --- | --- |
| `Approval` / `Transfer` | 本链所有研究中项目 | 目标 Token 合约 | 保存活动摘要；期限内（含期限时点）活动重置观察期限并按管理员配置刷新资料；无非空源码时强制触发源码查询 |
...
flowchart TD
    ...
    dedupeActivity --> activityEligible{"事件区块时间是否不晚于当前期限？"}
    activityEligible -->|是| renew["以事件区块时间重置观察期限<br/>续期不受冷却限制"]
    activityEligible -->|否| late["记录迟到依据，不续期或刷新<br/>等待覆盖完成后按超时停止"]
    renew --> source{"是否仍无非空源码？"}
...
活动是否有续期资格按链上时间判断：事件区块时间早于或等于处理前的当前期限时，把期限重置为“事件区块时间 + 处理该事件时的当前链配置时长”，续期次数不设上限；事件区块时间晚于当前期限时不续期，即使因覆盖滞后尚未写入超时状态也不能延长。相同日志只处理一次，不得重复续期；已经因任何原因停止后，迟到活动不能重开项目。
...
1. 每个发生在当前期限内（含期限时点）的有效活动都重置观察期限，且不受刷新冷却限制；
2. 项目仍无非空源码时，每个具有续期资格的活动都具备触发源码重新查询的资格，并与正在执行或冷却中的同类请求合并。
...
owner 刷新成功后替换当前 owner；税费接收地址刷新成功后按税费用途替换当前地址集合。新出现的当前地址自动成为 L1，并执行该地址的固定 `0..B-1` 历史、五项资产余额和 L2–L5 研究。被移出的旧地址失去相应当前角色；如果它还具有部署者、初始接收方或其他税费用途等角色，仍保留节点、其他标签及已有事实。普通活动不重抓所有既有钱包的固定历史窗口，也不整体重建资金来源图。
