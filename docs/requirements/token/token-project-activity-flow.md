...
每条链使用一套共享事件监控能力，动态覆盖本链所有研究中项目，不为每个项目分别建立独立订阅。首版只运行 ETH；未来接入 BSC 时复用同一套业务代码和构建产物，按 BSC 配置启动独立实例。具体进程与组件边界见关联目标技术设计。
...
部署前 [内部 ETH 转账识别](token-wallet-internal-transfers-flow.md)属于 L1–L5 钱包历史：普通与内部交易均固定 `0..B-1`，分别倒序取第一页最多 300 条原始记录。该扩展不新增本表监控目标或触发动作；内部 ETH 入账不因被纳入历史研究而续期、补查源码、刷新资料或停止项目。
...
第一版不要求为业务展示保存研究期的全部活动明细。`Approval`、`Transfer` 和监控地址主动交易分别保存首次证据、最近证据及累计去重数量，并保存当前观察期限。日志证据至少能追溯链、区块、交易哈希、交易序号、`logIndex`、发出合约、事件主题和区块时间；主动交易证据至少能追溯链、区块、交易哈希、交易序号、`from`、命中地址与角色、成功状态和区块时间。原始数据持久化边界见关联目标技术设计。
...
冷却从本轮首次实际派发开始，技术重试不重置；同组最多一轮执行和一个合并的待执行后继。没有新的合法触发时，冷却结束不产生定时刷新。任务仅领取或等待配额时不算已经开始；取得配额并登记执行后，才按在途任务处理停止与有限重试。
...
owner 刷新成功后替换当前 owner，并按前述生命周期切换交易监控目标；税费接收地址刷新成功后按税费用途替换当前地址集合。新出现的当前地址自动成为 L1，并执行该地址固定 `0..B-1` 的普通与内部历史（分别倒序第一页最多 300 条原始记录）、六项资产余额与估值以及 L2–L5 研究。被移出的旧地址失去相应当前角色；如果它还具有部署者、初始接收方或其他税费用途等角色，仍保留节点、其他标签及已有事实。普通活动不重抓所有既有钱包的固定历史窗口，也不整体重建资金来源图。
...
多个停止条件竞争时，主停止原因保留首次成功写入值。后来补到的证据另存，不改写主停止原因；Swap 缺口仍按当前覆盖版本补齐，并可更新更早的已识别 Swap 证据。

任何停止原因发生后：

- 排队但未开始的研究与刷新任务直接取消；
- 已经开始的任务继续原有有限重试策略，直至成功或重试耗尽，并保存最终结果或失败；
- 已经开始但正在等待重试的任务仍属于在途收尾，不能作为未开始任务取消；
...
6. 项目登记前到达但尚未关联的数据不能永久丢弃；可先保存后关联，也可通过登记后的历史补查取得，具体持久化边界见关联目标技术设计。
...
当前不考虑区块替换或链重组后的状态回滚。正常追平时，已登记项目的活动续期和 Swap 停止分别采用 [P95 ≤ 15 秒目标](token.md#已明确的正常运行时效目标尚未验证)，从配置节点首次可查询到事件所在新区块起计至状态可靠落库。其他已确认目标为：刷新前置资料就绪且冷却结束后首轮 P95 ≤ 1 分钟，一小时以内断线在上游恢复后 5 分钟内追平，全部超时条件满足后 P95 ≤ 15 秒停止，人工停止 API P95 ≤ 2 秒；完整边界见[活动刷新、恢复与停止](token.md#活动刷新恢复与停止)。15 秒目标不替代超时所需的完整覆盖门槛。组件、数据结构、配置入口和调度细节在本轮关联目标技术设计中展开，尚未实现。
