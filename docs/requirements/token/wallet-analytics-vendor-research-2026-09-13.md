...
**用户已选择 Nansen API，并确定将其接入 ATHENA；不再设计和实现本地钱包战绩计算引擎。** 当前有效需求见[钱包交易与战绩](wallet-trading-performance.md)。下文各家产品比较保留为选型证据，不表示同时接入或自动切换供应商。

初期约 3–5 个用户、查询频率低，且本地开发也需要免费额度。使用方向为 **Nansen Free API 起步，额度不足时按需购买 $10／10,000 credits**，无须为 API 订阅网页 Pro。供应商与接入方向已确认，具体接入细则继续维护；尚未注册、充值、调用真实接口或实现集成。费用测算见下文。
...
Nansen 接入仍有字段与覆盖含义需要验证：`win_rate` 的单位和分母未明确，摘要可能缓存约一小时；成本法、转账处理及历史范围不能直接认定与原方案一致。最近 300／1,000 笔窗口、指定基础币成交段和最新价重算已退出本地实现范围，不为复现它们另建引擎。[接口定义](https://docs.nansen.ai/api/profiler/address-pnl-and-trade-performance) 官方再分发指南将钱包 PnL／摘要列为允许，交易明细需署名，同时仍有一般使用条款和竞争性使用限制；这不是对所有数据及所有用途的授权。[再分发指南](https://docs.nansen.ai/guides/redistribution-guide)

供应商及不自研计算的方向已经用户确认；费用测算仍是使用量假设，尚未采购、注册、实测或完成接入。

## 后续接入工作

1. **确认首版展示内容与查询范围**：围绕 Nansen 的钱包总览、逐币表现和交易明细，确定时间窗口、分页、筛选与缺失值展示；不重新选择供应商或重开本地算法设计。
2. **核对真实接口数据**：用同一 Ethereum 钱包对照官方响应与页面解释，确认胜率单位和分母、转账含义、费用及更新时效，不把未知字段补成零。
...
**用户确认的当前安排（2026-09-13）：选择 Nansen API 接入 ATHENA，停止本地钱包战绩计算方案的设计与实现，继续维护接入需求。** 选型已完成；接入技术设计、真实接口验证和实现尚未完成。原本地规则及未确认的 AA 成本建议已归档，不再作为当前接入要求。
