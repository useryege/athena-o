...
> 签名能力补充（2026-09-15）：[专用签名可行性调研](signing-feasibility-2026-09-15.md)已完成官方 SDK 与现有 Go 加密库的离线对照，26 组载荷和 146 组字段改动检查通过。结论为签名算法可实现；本文初轮未执行签名的记录保持其原时点含义，真实账户认证、链上授权、交易和领取仍待验证。补充调研也发现 SDK 的 V2／V3 合约路径及默认授权集合差异，实施须按本期范围核准。
