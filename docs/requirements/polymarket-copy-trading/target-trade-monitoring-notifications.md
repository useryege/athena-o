...
整份需求保持 `讨论中`。核心目标、用户主动暂停或取消时已排队通知继续完成且撤权或解绑仍停止发送，以及 Telegram 投递结果不确定时优先避免重复通知，已由用户明确。按单条成交记录独立触发、订阅生效时间的精确定义和时效口径仍在完善；文中的建议不代表用户已经确认整份需求，不能作为后端技术设计输入。
...
| 活动粒度 | 建议按每条新识别的公开成交记录独立触发，不等待同一交易的其他分片；同一市场或交易的不同记录可以分别提醒。可明确识别为同一已处理记录的重复读取不新增提醒，具体业务边界待确认。 |
| 历史边界 | 首次订阅不回放历史；系统故障恢复后自动补查并标记延迟；用户暂停、撤权或取消期间发生的活动均不补发。建议以明确展示的订阅生效时刻和成交发生时间划界，不固定增加 5 分钟等待；精确边界待确认。 |
...
| 成交活动 | 建议以一条新识别的公开成交记录作为最小通知事件，不要求收齐相同交易哈希、Market/Asset、Outcome 和方向的其他分片；一条记录不一定对应目标的一整张订单。此粒度待确认。 |
| 初始基线 | 创建或手动恢复订阅时用于建立可信新旧边界的基准，本身不产生用户通知；生效时间的精确定义见待确认的订阅生效边界建议。 |
...
用户已明确市场发现是核心目标，并提出改为单次独立触发、允许重复提醒。建议据此采用以下目标行为，等待确认具体含义：
...
- 保留用户级突发摘要保护。为满足市场发现目标，建议摘要直接列出涉及的目标、市场、Outcome、方向及市场链接，不能仅列目标活动数量；详细成交仍在站内逐条保留。

上述“多条新成交可以分别提醒”与“同一条 Telegram 消息在结果未知时不自动重发”是不同规则，两者不冲突。

## 订阅生效边界建议（待确认）

首次订阅和手动恢复都应向用户展示明确的生效时刻。建议以实际成交发生时间是否处于有效订阅区间决定资格，并在形成活动前遵守当前权限和订阅状态；首次发现时间不能替代成交时间。建立可信边界的方法留给技术设计。

例如，若订阅于 12:00:00 生效，11:59:59 成交但 12:01 才发现的交易不通知；12:00:01 成交、12:01 才发现的交易，在订阅持续有效时仍应识别。建议把开始时刻纳入区间，暂停、取消或撤权的生效时刻作为区间结束，不纳入结束时刻及之后的成交；具体同秒边界的可实现性在技术设计阶段验证。

不建议固定设置 5 分钟启动等待。12:00 订阅、仅识别 12:05 起发生的成交，会主动放弃前 5 分钟的市场信息；同时仍须按成交时间排除 12:05 前发生、12:05 后才被发现的交易。若用户确实需要该等待，应明确显示 12:05 才开始生效，并说明等待期间的成交不补发，不能把它混同于每笔通知延迟 5 分钟。

## 时效口径建议（待确认）

时效口径定义一段延迟从什么事件开始、到什么事件结束。用户无需决定采集或发送的技术细节；下面补齐目标行为和衡量边界，不改变现有普通通知的数值目标：
...
| 首条待汇总记录形成 → Telegram 明确成功接收对应摘要 | 建议统一 60 秒为正常情况下的成功提交上限；在 60 秒内只生成摘要内容不算完成发送目标。摘要仍至多每 60 秒一条。 |
| 故障补查、Telegram 异常或投递结果未知 | 明确显示延迟、失败或结果未知；与正常时效样本分别统计，不能用排除异常来隐藏异常次数、持续时间或未完成投递。仅按已确认的避免重复原则对确定未成功的暂时失败有限重试。 |
...
以下规则保留已确认决定，并将仍需确认的调整标为“建议”。整份需求仍需最终确认：
...
8. 创建订阅时先建立可信的新旧边界，成功后该订阅才进入正常监控；首次订阅不回放历史。建议展示明确生效时刻，并按“订阅生效边界建议”以成交发生时间判断资格，不因晚发现而把历史交易当成新交易。
9. 订阅持续有效期间发生系统或外部依赖故障时进入异常状态并继续占用配额；恢复后由系统自动补查故障窗口，完成补查后继续原订阅，不要求用户手动恢复。
10. 用户主动暂停后停止新活动的识别和通知生成，暂停期间发生的活动不补发；暂停前已经排队的普通通知及待汇总活动继续按原规则完成，不因暂停清除。产品在操作时和状态中明确提示“已排队通知仍会继续发送，可能稍后收到”。暂停状态继续占用配额并保留目标唯一性。用户恢复时重新建立基线，只识别恢复成功后的新活动；旧队列的完成不属于暂停期间活动补发。
...
13. 建议对能明确识别为同一已处理公开成交记录的重复返回、多次扫描和跨服务重启保持站内活动唯一；同一市场或交易哈希的不同新成交记录可以分别提醒。Telegram 投递遵循第 23 条的避免重复原则，不以自动重发消除未知结果。
14. 不同订阅即使指向同一目标，也必须分别按订阅所有者的权限和接收状态决定是否通知。
15. 建议取消跨成交分片的归并等待：每条新识别的公开成交记录独立触发，不因市场、方向、时间戳或交易哈希相同而合并；迟到的新分片形成新活动，保留原始事实，不修正为目标整张订单的总量或完整交易意图。
...
26. 非突发摘要覆盖的活动，从站内活动记录形成到 Telegram 成功提交发送，95% 不超过 5 秒；Telegram 服务自身在成功接收后的最终送达时间不属于 ATHENA 可保证范围。突发摘要不适用该 5 秒指标；建议将其 60 秒统一为正常情况下从首条待汇总记录形成到 Telegram 明确成功接收摘要的时限，故障处理见“时效口径建议”。
...
36. 突发摘要模式下，后续活动不再逐条发送 Telegram，而是至多每 60 秒发送一条摘要；首条等待汇总的活动应在其站内记录形成后 60 秒内得到摘要提醒，时限终点按“时效口径建议”待确认。摘要包含覆盖时间范围、活动总数、各目标账户的活动数量和站内完整记录链接；为满足核心目标，建议同时直接列出涉及的目标、市场、Outcome、方向及市场链接。
...
6. ATHENA 判断成交发生时间是否位于有效订阅区间、是否属于通知范围，以及该订阅是否已经处理该公开成交记录；精确区间口径仍待确认。
...
flowchart TD
    ...
    subgraph MONITOR["二、持续监控与活动识别"]
        ...
        K --> L
        L -->|否| G
        L -->|是| M[建议每条新公开成交记录独立触发<br/>不等待其他成交分片]
        M --> N{该订阅是否已经处理？}
        N -->|是| G
    ...
    subgraph NOTICE["三、用户通知"]
        ...
        S --> T[摘要展示时间范围与数量<br/>建议直接列目标、市场与市场链接]
        R --> U{Telegram 提交结果？}
        T --> U
        U -->|成功| V[记录投递成功状态]
        U -->|确定未成功且可重试| W[有限重试]
...
| 目标订阅 | 监控中 | 用户主动暂停 | 已暂停 | 停止新增活动通知；已排队通知继续完成并提示可能延迟。暂停期间发生的活动不补发，继续占用配额并保留目标唯一性。 |
...
| 目标订阅 | 任意非终态 | 当前具有产品权限的用户取消 | 已取消 | 停止新监控和新通知，已排队通知继续完成并提示可能延迟；立即释放配额与目标唯一性，不能恢复原订阅。重新获权用户可以直接取消权限停用订阅；已经因撤权终止的旧队列不因取消而重新发送。 |
...
- **外部数据重复、乱序或同秒多笔**：建议对可明确识别为重复的公开记录不新增提醒，不因同一市场或交易哈希而合并不同记录；有效监控期间晚到的新记录依时间边界规则判断资格。
- **同一链上交易包含多个成交分片**：建议每个新识别分片独立形成活动和提醒，不等待完整分片、不宣称对应整笔订单总量；同一市场可以多次提醒。
...
- 建议只展示该条公开成交记录的数量、金额与价格，不展示无法由单条记录证明的整笔订单总量、加权均价或价格范围；
...
突发摘要展示其覆盖时间范围、活动总数、各目标账户的活动数量及站内完整记录链接，并明确说明详细成交仍逐条保存在 `Activity Alerts`。为满足市场发现目标，建议同时直接展示涉及的目标、市场、Outcome、方向及市场链接，使用户从摘要本身也能知道目标在交易哪些市场。摘要覆盖的活动不再逐条补发 Telegram。
...
12. **可识别的重复记录不重新触发（建议）**：同一已处理公开成交记录在重复观察和进程重启后不形成新站内活动；投递重试不新建活动。不同新成交记录即使属于同一市场或交易哈希，也可以分别提醒。
13. **新成交分片独立触发（建议）**：同一交易的一个分片已形成活动并通知后，又识别到另一个新分片；后者独立形成新活动，正常流量下独立提醒。两条记录均突出目标、市场、Outcome、方向和市场链接，各自展示源端事实，不等待完整交易、不拼成整笔订单汇总，也不把多次提醒表述为必然发生了多张独立订单。
14. **用户暂停与恢复**：暂停期间发生的活动不通知，暂停前已排队通知继续完成，用户可见“已排队通知仍会继续发送，可能稍后收到”的提示。暂停订阅仍占用配额且阻止重复订阅同一目标；恢复并建立新基线后只识别新的活动。
...
19. **站内历史完整**：无论 Telegram 是否绑定或投递成功，每个成交活动都形成且只形成一条可持久查看的站内记录，不自动过期或删除。
...
30. **Telegram 突发摘要**：同一用户在任意连续 60 秒内收到 12 个成交活动；前 10 个逐条 Telegram 通知，第 11、12 个由摘要覆盖。建议在正常情况下从首条等待汇总的记录形成起 60 秒内取得摘要成功提交结果；摘要除覆盖范围、总数、各目标数量和站内链接外，还直接展示涉及的市场和市场链接，帮助用户判断目标正在交易哪些市场。两项活动以后不再逐条补发，异常结果遵守避免重复的投递规则。
...
- 首次订阅不回放历史；系统故障恢复后自动补查。用户暂停期间发生的活动不通知且恢复后不补发，暂停与异常订阅继续占用配额；取消为不可恢复终态并释放配额，重新订阅同一目标时建立新基线。主动暂停或取消前已经排队的通知继续完成并提示可能延迟，不等同于补发停用期间的新成交。
- 用户提出单次独立触发、允许重复提醒，并要求结合市场发现目标判断。文档已改为逐条成交记录触发的建议；同一市场可多次提醒、可识别的同一记录不重复触发，以及摘要直接展示市场的具体边界仍待确认。
- 每个成交活动均形成不自动过期的独立站内记录。用户当时已绑定 Telegram 时，正常流量逐条私聊；同一用户任意连续 60 秒超过 10 条后，第 11 条起至多每 60 秒形成一条用户级摘要，首条待汇总活动等待不超过 60 秒，摘要覆盖活动不再逐条补发且不同用户互不影响。未绑定不阻止订阅，绑定、解绑或重新绑定均不补发此前消息；投递结果不影响站内历史；第一阶段不提供逐订阅渠道配置。活动粒度与摘要市场信息的调整见上文建议。
...
1. **独立触发的精确定义**：建议每条新识别的公开成交记录独立触发，允许同一市场或交易哈希的不同记录分别提醒；可明确识别为同一已处理记录的重复读取不新增提醒。保留现有突发保护，并让摘要直接展示涉及的目标、市场、Outcome、方向及市场链接。用户已提出简化方向，以上完整口径尚需确认。
2. **订阅生效时间**：建议首次订阅或手动恢复建立可信边界后明确展示生效时刻，按成交发生时间是否位于有效区间判定，不设置固定 5 分钟等待。若采用用户提出的等待方案，需要接受等待期间活动不补发，并仍明确成交时间边界。
3. **暂停或撤权时尚未形成记录的故障补查**：已经排队与仍未识别、未形成活动的故障窗口不是同一情况。用户本次决定只明确了已排队通知；有效监控期间的欠缺活动在之后暂停或撤权时如何保留和收尾，仍需补齐。
4. **时效口径**：建议采用“时效口径建议”统一普通通知与摘要的衡量终点和异常范围；公开可查询起点的可观测性需要技术设计验证，不能用发现时间替代来掩盖发现延迟。

整份需求仍需用户明确确认后才能将状态更新为 `已确认`。本次仅维护需求，不创建技术设计或修改实现。
