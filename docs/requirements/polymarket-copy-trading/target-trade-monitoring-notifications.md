...
> 需求状态：已确认
>
> 关联技术设计：[Activity Alerts 后端技术设计](../../design/trading/trader-sync-activity-alerts.md)（已实现）

> UI 补充：页面与交互及[完整 UI spec](../../superpowers/specs/2026-09-10-trader-sync-activity-alerts-ui-design.md)已整体确认，[长期 UI 设计](../../design/web-ui/trader-sync-activity-alerts.md)已实现并同步。[验收记录](../../testing/trader-sync-activity-alerts-acceptance.md)说明本地与浏览器证据及外部验证限制。
...
第一阶段需求此前已正式确认，首期 10 人规模、低频目标定位、逐条活动、完成基线立即生效、不补历史及权限边界继续有效。本轮技术核实引出的业务调整已逐项确认，按事实将需求状态记为 `已确认`。用户已逐项确认：以链上结算时间判断订阅边界并明确展示“结算时间”；摘要 60 秒内开始提交，成功回执耗时单独记录；每 60 秒至多开始一批摘要，超长分多条完整展示，60 秒时限针对该批首条提交，整批完成耗时单列。三项决定已全部写入本文。2026-09-10 本轮另已确认：保留前 10 条逐条 Telegram 提醒，同用户集中成交允许按限速排队；平稳流量仍以成功回执 P95 ≤ 5 秒为目标，集中成交单列排队时延并保留在总体统计中，不承诺这类突发也达到 5 秒。另已确认发送许可边界：撤权/解绑提交后禁止新发送许可，未获许可队列终止；此前已进入发送中的单条请求可完成或形成未知结果，不等待网络回执才完成撤权。[完整设计 spec](../../superpowers/specs/2026-09-10-trader-sync-activity-alerts-design.md)已获用户整体确认；21项[前后端实现计划](../../superpowers/plans/2026-09-10-trader-sync-activity-alerts.md)已落实到当前代码与长期文档。验收覆盖隔离数据库、真实本地组件链、受控浏览器与一次明确授权的公网 Telegram 发送；100 个真正活跃实网目标、公开时刻 P95/P99、供应商静默漏推和长期稳定仍需外部环境验证。
...
后续[交易板块需求](copy-trading.md)于 2026-09-14 完成首版业务规则的逐项确认，采用事前绑定钱包、用户确认启用、成交通知与 ATHENA 内手动市价下单；金额及是否交易由用户当次决定，初版不自动跟单。已进入[交易总体设计讨论](../../design/trading/polymarket-manual-trading.md)，尚未开始交易代码实现。本文的第一阶段继续只提供授权、订阅、可靠监控和用户通知，不触发任何交易操作。
...
本提醒已纳入独立添加页，在确认创建前显示，不增加额外勾选；具体界面见本文“界面与交互决定”及关联 UI spec。
...
本节补充既有 45 条业务规则的可见交互，不改变后端监控、通知、配额或隔离边界。完整书面呈现见 [UI spec](../../superpowers/specs/2026-09-10-trader-sync-activity-alerts-ui-design.md)，用户已整体确认该文档；成员六个路由、管理员列表/详情、Notifications 往返和 Service Status 集成均已实现并完成受控浏览器验收。

1. 桌面优先、手机完整可用；沿用 ATHENA 既有英文界面、深浅主题和 UTC+8 明示时间。交互线框用于确认流程，不等同业务联调。
2. 主页活动主区与当前目标状态同屏，手机折叠目标栏。默认活动包含 owner 所有历史；侧栏按具体订阅筛选，同钱包重订阅不合并旧 ID。目标/结算时间筛选不改变监控资格。
3. 独立添加页先解析身份，在同页展示完整钱包、统计、六区间 P/L（默认 1Y）及备注，再明确确认创建。身份失败禁止创建，辅助不可用不阻止；成功返回原主页并定位准备监控的新目标。
4. 主页侧栏只负责筛选和状态；完整订阅列表与独立详情管理备注、暂停、恢复、取消和中断记录。Current/Cancelled 分开，已取消默认隐藏；暂停/恢复直接执行，取消确认一次并说明旧队列继续、历史保留及不可恢复。
5. 活动采用独立详情页，与 Telegram 深链接共用。成交事实优先、通知结果单列、原始证据展开；Combo 是一笔活动，NO 不得逐腿取反，金额/币种/费用按来源精确展示。
6. 摘要有独立批次页，可查看全部分条和关联活动。一个活动跨多部分时全部可读；各结果分别保留，全部 sent 才显示整批成功。未知不提供自动重发。
7. 可见会员页面每 5 秒刷新监控和通知状态；新活动先提示，点击后载入，保持未点击时的阅读、复制、焦点和位置。隐藏页停止轮询，后台继续监控。分页用游标，不虚构总数；状态更新不使历史活动移位。
8. 复用 Notifications 绑定入口；从添加页往返保留同 owner 会话内输入和备注，过期确认仍需重新解析。未绑定或不可达不阻止创建，当前绑定不能追溯修改历史资格；换账号/退出/撤权清除草稿。
9. 管理员用独立订阅概要列表和详情，运行健康并入 Service Status，互相链接。只展示用户/钱包/生命周期/健康和数量，不提供备注、完整活动、逐条消息/投递或订阅操作。关联投递数量不等同 attempt 数，也不跨订阅简单相加。
10. 首次加载、空态、筛选无结果、刷新失败、权限失效分别表达；失败保留旧数据及更新时间，获知撤权立即清空并忽略迟到响应。覆盖手机、键盘、焦点、长内容、精确复制和深浅主题，不仅验证正常桌面画面。

界面验收补充编号 UI-1 至 UI-6：

| 编号 | 必须验证 |
| --- | --- |
| UI-1 | 桌面和手机完整走通添加、确认/过期、备注、六态管理、取消后新 ID，返回保留上下文。 |
| UI-2 | 独立活动/摘要与 Telegram 登录后深链接、部署子路径一致；资料缺失、Combo、精度及所有部分结果可读。 |
| UI-3 | 5 秒可见刷新、隐藏停止、提示点击、初始空列表及历史页发现新活动；固定阅读位置、失败保留及游标隔离。 |
| UI-4 | 绑定往返草稿、过期不延长、未绑定历史不补发、权限清理及迟到响应不可恢复正文。 |
| UI-5 | 管理员独立概要、无私有明细和写操作，运行数据单位/时间窗口及关联计数解释准确。 |
| UI-6 | 1440/1280、900 附近、390；深浅主题、全键盘/触屏、焦点与非颜色状态、长钱包/金额/Combo、曲线文字替代。 |
...
- 首期 10 人规模及既有需求此前已正式确认，用户已授权开展后端技术设计。本轮全部业务调整和技术设计各节已逐项确认，完整书面技术 spec 也已获整体确认；本次未修改后端源码。Copy Trading 的具体需求继续另行讨论。

## 待确认问题

既有业务条目已确认，包括首期按 10 人设计的规模、低频产品定位、UI 使用提醒与不自动限制高频目标的边界，以及独立触发与市场摘要、完成基线后立即生效、当前不做历史补查、恢复后继续实时监控并保留中断说明。

本轮发现的三项业务边界均已逐项确认并写回：链上结算时间、摘要 60 秒内开始提交、每 60 秒至多一批且超长分多条完整展示。本轮又已确认同用户集中成交的普通通知排队边界，详见时效专节。撤权/解绑与已进入发送中的请求划界也已确认，详见“发送许可与撤权边界”；其余已有业务决定继续有效。当前没有尚待用户选择的业务取舍；原后端书面规格已整体确认。后续 UI 页面与交互及完整书面 UI spec 也已整体确认；状态不额外增加审批流程。

数据源、金额与钱包、Profile URL 和收益口径的事实见[契约核验](source-contract-verification.md)；容量及节点边界见[采集核验](collector-contract-verification.md)。金额按实际来源币种准确显示是对既有“仅陈述来源事实”规则的对齐；没有加入币种转换或交易执行。后端、UI 与运行时组合已经实现；[验收记录](../../testing/trader-sync-activity-alerts-acceptance.md)把受控本地证据、一次授权公网发送和仍缺失的实网时效/完整性证明分开陈述，不要求用户选择事实真假。
