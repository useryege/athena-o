...
本文定义产品第一阶段的目标行为。产品总名同时覆盖本阶段的活动订阅与通知，以及未来独立讨论的 Copy Trading 板块。各项关键决定已经逐项确认并写入本文，当前等待用户对整份需求作最终确认；确认前不能作为后端技术设计输入。
...
每个订阅必须有唯一 ATHENA 用户所有者。管理员只能读取已确认的运行概要；用户自定义订阅名称或备注、完整活动内容、Telegram 消息正文和逐条投递历史不属于管理员只读范围。普通用户不得查看、修改或接收其他用户的订阅数据。
...
3. 管理员只读范围包括：用户身份、订阅目标及完整 Proxy Wallet、订阅状态及生命周期时间、最近成功监控时间、异常摘要、待补查状态、活动数量以及 Telegram 投递成功/失败汇总。第一阶段管理员产品界面和业务接口不提供用户自定义订阅名称或备注、完整活动内容、Telegram 消息正文或逐条投递历史，也不得提供创建、修改、暂停、恢复、取消、重发或删除用户订阅及通知的入口。
...
- **管理员请求受限明细**：第一阶段不向管理员返回用户自定义订阅名称或备注、完整活动内容、Telegram 消息正文或逐条投递历史；管理员仍可通过运行状态、异常摘要和数量汇总判断订阅是否正常。
...
第一阶段不向管理员显示用户自定义订阅名称或备注、完整站内活动内容、Telegram 消息正文或逐条投递历史。管理员也不能重发、删除通知或执行任何订阅状态操作。未来如果需要为客服或审计增加受控明细访问，必须另行讨论和确认权限需求。
...
6. **管理员最小只读范围**：管理员能看到用户身份、目标与完整 Proxy Wallet、订阅状态和生命周期时间、监控健康与待补查状态、活动数量及 Telegram 成功/失败汇总；不能看到用户自定义订阅名称或备注、完整活动内容、Telegram 消息正文或逐条投递历史，也不能创建、修改、暂停、恢复、取消订阅或重发、删除通知。
...
- 管理员只读查看用户身份、目标与完整 Proxy Wallet、订阅状态和生命周期时间、监控健康与待补查状态、活动数量及 Telegram 成功/失败汇总；第一阶段不查看用户自定义订阅名称或备注、完整活动内容、Telegram 消息正文或逐条投递历史，也不能操作订阅、重发或删除通知。
