# Polymarket 目标账户订阅与活动通知需求（产品名称待确认）
...
- 当前建议不把挂单、改单、撤单、拆分、合并、赎回、奖励、充值和提现纳入通知范围；最终活动范围仍待确认。
...
| 角色 | 已明确或建议的职责与边界 |
| --- | --- |
| ATHENA 管理员 | 已明确：开通或关闭用户使用本产品的权限。管理员是否能查看或干预用户订阅仍待确认。 |
...
每个订阅必须有唯一 ATHENA 用户所有者。除非后续明确授权管理员运营能力，否则一个普通用户不得查看、修改或接收其他用户的订阅数据。
...
| 规范目标身份 | 用于唯一识别目标且不随显示名称变化的身份；当前建议使用 Polymarket Profile Address / Proxy Wallet。 |
| 目标订阅 | 由一个 ATHENA 用户拥有、指向一个规范目标身份的持续活动关注关系。 |
| 目标活动 | Polymarket 对目标公开的一项操作事实。第一阶段具体纳入哪些活动类型仍待确认。 |
...
3. 每个目标必须解析为一个规范目标身份。显示名称、头像和简介只用于展示，变化后不得导致目标被当成新身份或历史活动被重新通知。
4. 当前建议第一阶段只以公开的 `TRADE` 触发通知，同时覆盖 `BUY` 和 `SELL`、Maker 与 Taker；其他目标活动暂不通知。
5. 创建订阅时先建立初始基线，基线之前及基线内的历史活动不通知；基线成功后该订阅才进入正常监控。
6. 订阅持续有效期间发生短时故障，恢复后必须补查故障窗口。用户主动暂停或取消订阅期间的活动默认不补查；恢复或重新订阅时建立新边界。
7. 当前建议不设置最小金额、市场类别、Outcome 或买卖方向过滤，避免用户误以为订阅覆盖全部活动但实际被静默过滤。
8. 同一逻辑活动即使被重复返回、多次扫描、通知重试或跨服务重启，也最多向同一订阅发送一条业务通知。
9. 不同订阅即使指向同一目标，也必须分别按订阅所有者的权限和接收状态决定是否通知。
10. 多笔不同活动不得仅因秒级时间戳或交易哈希相同被错误合并。若采用汇总通知，只有市场/Asset、Outcome 和方向等边界也一致的成交分片才能进入同一消息。
11. 通知只能陈述外部数据支持的事实；字段缺失、冲突或无法核验时不得推测目标意图、实际仓位或盈亏。
12. 识别、保存和通知目标活动的任何步骤都不得调用交易执行能力，也不得产生钱包、余额、订单或仓位副作用。
13. 通知渠道暂时失败时，已识别活动不得被重新当成新活动；通知应最终送达订阅用户，或形成该用户与获授权管理员可见的明确失败结果。
14. 目标没有新活动是正常状态，不产生心跳式活动消息；用户应能通过独立订阅状态区分无活动和监控失效。
15. 管理员撤销产品权限后必须立即阻止用户继续操作产品；现有订阅是暂停、终止还是保留待恢复仍待确认。
...
- **重复订阅同一目标**：同一用户是否允许创建多个有效订阅仍待确认；不同用户可以独立订阅。
...
- 目标标识：建议接受 Polymarket Profile URL 或合法 0x 地址，最终让用户确认解析出的 Proxy Wallet。
...
6. **活动事实正确**：通知中的目标、活动类型、方向、市场、Outcome、价格、数量和时间与公开事实一致。
7. **重复处理不重复通知**：同一逻辑活动在重复观察、通知重试和进程重启后，对同一订阅仍只有一条业务通知。
8. **多分片汇总不丢事实**：若采用建议粒度，一组符合汇总边界的成交分片只产生一条摘要通知，分片数、总量、金额、加权均价和价格范围正确。
9. **故障后连续**：订阅保持有效时发生短时故障，恢复后补回窗口内的新活动，不回放初始基线前历史。
10. **权限撤销立即生效**：管理员关闭用户权限后，用户不能继续操作产品或收到不符合最终撤权规则的新通知。
11. **投递失败可诊断**：活动已识别但通知无法送达时，订阅用户或获授权管理员能看到明确状态，不产生第二个逻辑活动。
12. **绝无交易副作用**：所有第一阶段场景均不创建订单、签名，也不改变任何钱包、余额或 Polymarket 仓位。
...
| --- | --- | --- | --- |
| Q1 | 产品正式名称是什么？ | 推荐 `Trader Sync`；产品下设置第一阶段 `Activity Alerts` 与未来 `Copy Trading` 两个功能板块。 | 需求域、权限名称、导航和后续产品语言。 |
| Q2 | 产品权限是简单开通/关闭，还是区分只读与可管理？ | 第一期使用简单开通/关闭；获授权用户只能管理自己的订阅。 | 管理员配置与用户授权语义。 |
| Q3 | 每个用户最多可订阅多少目标？ | 第一期上限 20 个；同一用户对同一规范目标最多一个有效订阅。 | 产品容量、错误语义和资源公平性。 |
| Q4 | 接受哪些目标标识？ | 接受 Polymarket Profile URL 或合法 0x 地址，解析后必须展示并由用户确认唯一 Proxy Wallet；不凭显示名称直接创建。 | 身份唯一性和误订阅风险。 |
| Q5 | 哪些目标活动触发一期通知？ | 只包含公开 `TRADE`，覆盖 `BUY`、`SELL`、Maker 与 Taker，默认不过滤金额或市场。 | 监控范围和消息数量。 |
| Q6 | 首次订阅、故障恢复和主动暂停如何处理历史？ | 首次不回放；运行故障必须补查；主动暂停或取消后不补相应期间活动。 | 新旧边界和连续性。 |
| Q7 | 一条交易通知对应什么粒度？ | 选择 B：相同交易哈希、市场/Asset、Outcome 和方向的分片保留明细并汇总为一条通知。 | 去重、汇总和消息数量。 |
| Q8 | 通过什么渠道通知订阅用户？ | 使用用户已绑定的 Telegram 私聊；未绑定或不可达时的订阅与补发行为需继续讨论。 | 用户体验、通知可靠性和隐私。 |
| Q9 | 正常监控的时效目标是什么？ | 交易可从公开数据源查询后 15 秒内进入用户通知流程，并记录交易时间到发现、送达的实测延迟。 | 监控频率、容量和可感知时效。 |
| Q10 | 管理员关闭产品权限后如何处理已有订阅？ | 立即暂停全部订阅并停止新通知；保留订阅，重新开通权限后由用户手动恢复并建立新基线。 | 撤权安全、数据保留和恢复行为。 |
| Q11 | 谁能看到监控异常和投递失败？ | 受影响的订阅用户看到自己的状态；管理员只看运营所需的安全摘要，不默认查看用户全部活动明细。 | 隐私、支持和运维能力。 |
| Q12 | 突发大量活动如何通知？ | 按 Q7 的逻辑活动逐条排队且不丢弃；是否增加批量摘要以后再确认。 | 消息噪声和通知延迟。 |
