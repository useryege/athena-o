...
各项业务决定已逐项确认，包括人工选择低频目标的产品定位、UI 使用提醒与性能保障范围、按单条新成交记录独立触发并保留市场摘要、完成基线后立即生效，以及暂停、取消或撤权时停止旧补查并保留缺口说明。当前没有未解决的业务待确认项；整份需求仍保持 `讨论中`，等待用户最终确认后才能作为后端技术设计输入。
...
| 历史边界 | 首次订阅和手动恢复在完成基线后立即生效，展示明确生效时刻，按成交发生时间判断，不额外等待 5 分钟。未被暂停、取消或撤权的订阅在故障恢复后自动补查；暂停、取消或撤权时停止尚未形成活动记录的旧补查并保留缺口，之后恢复只识别新生效时刻起的成交，停用期间不补发。 |
...
| 站内通知 | 每个成交活动都立即形成一条独立、持久且不自动过期的 `Activity Alerts` 记录，无论 Telegram 是否绑定或投递成功。 |
| Telegram 通知 | 活动形成时已绑定 Telegram 才私聊通知；绑定、解绑和重新绑定都不补发历史。仅对确定未成功的暂时失败有限重试；结果不确定时停止自动重发并显示“投递结果未知”，优先避免重复通知。第一阶段没有逐订阅渠道配置。 |
| 突发摘要 | 同一用户任意连续 60 秒内前 10 条逐条 Telegram 通知，第 11 条起进入用户级摘要；摘要直接展示涉及的目标、市场、Outcome、方向及市场链接。摘要至多每 60 秒一条，正常情况下从首条待汇总站内记录形成到 Telegram 确认接收摘要不超过 60 秒，不同用户互不影响。 |
| 时效目标 | 面向人工选择的低频目标及其正常负载、偶发短时突发：公开可查询到站内记录形成 P95 不超过 15 秒、P99 不超过 30 秒；非突发 Telegram 提交 P95 不超过 5 秒；摘要从首条待汇总记录形成到 Telegram 确认接收不超过 60 秒。高频场景不纳入性能保障；外部延迟、故障补查及 Telegram 异常单独标识和统计，已识别记录不因频率高而丢弃。 |
...
性能保障面向低频目标及其正常负载、偶发成交分片或短时集中成交，具体承载范围由技术设计明确。持续高频交易监控和高频跟单不属于产品的性能支持目标。用户仍可按已有权限、目标解析和配额规则订阅目标，系统继续按既有活动识别、逐条保留和突发摘要规则处理；高频场景不承诺达到正常性能目标，但不能因此静默丢弃已识别活动或把异常表现为没有成交。无法可靠监控或补查时，继续如实显示异常、延迟和可能缺失的范围。
...
| 故障补查 | 订阅未被暂停、取消或撤权，但因系统或外部依赖故障暂时无法观察时，在依赖恢复后补回当前有效区间内的新活动；暂停、取消或撤权会终止尚未形成活动记录的旧补查。 |
| 已终止补查缺口 | 因暂停、取消或撤权而停止的旧故障窗口中，尚未完整识别或形成活动记录的范围；保留范围、终止原因和时间，不再列为待补查，也不在恢复订阅后补成站内活动或通知。 |
...
| 已排队通知 | 已形成站内活动、已取得当时 Telegram 通知资格，并已进入普通发送或待汇总范围的提醒；包括等待组成摘要的活动。尚未识别或尚未形成活动记录的故障补查不属于已排队通知。 |
| 投递结果未知 | 通知可能已被 Telegram 接收，但 ATHENA 无法可靠确认结果；优先避免重复通知，停止自动重发并保留用户可见的未知终态，不计为确定成功或确定失败。 |
| 突发摘要 | 同一用户在任意连续 60 秒内收到超过 10 个新成交活动时，为避免 Telegram 消息洪泛而对第 11 个及后续活动形成的用户级汇总通知；不替代或合并站内活动记录。 |
...
首次订阅和手动恢复在完成基线后立即生效，不额外等待 5 分钟，并向用户展示明确的生效时刻。以实际成交发生时间是否处于有效订阅区间决定资格，并在形成活动前遵守当前权限和订阅状态；首次发现时间不能替代成交时间。建立可信边界的方法留给技术设计。

例如，若订阅于 12:00:00 生效，11:59:59 成交但 12:01 才发现的交易不通知；12:00:01 成交、12:01 才发现的交易，在期间未暂停、取消或撤权时仍应识别。开始时刻纳入区间，暂停、取消或撤权的生效时刻作为区间结束，不纳入结束时刻及之后的成交；具体同秒边界的可实现性在技术设计阶段验证。

## 故障补查与订阅停用（已确认）

订阅未被暂停、取消或撤权时，系统或外部依赖故障恢复后自动补查当前有效区间，补回记录标记为延迟活动。用户主动暂停、取消或管理员撤权时，停止尚未形成站内活动记录的旧故障补查，保留已终止补查缺口的范围、原因和时间；之后依赖恢复、重新获权、恢复订阅或重新订阅，都不补这些旧成交的站内活动或 Telegram 通知。恢复订阅完成新基线后，只识别新生效时刻起的成交。

例如，系统 12:00 故障，目标 12:02 成交，用户 12:03 暂停，系统 12:04 恢复。12:02 的成交若尚未形成站内记录，则不再补查通知；旧故障窗口保留缺口说明。用户以后恢复订阅时建立新基线，历史缺口仍可见，不把旧窗口标成已完整补查。

已经形成的站内记录继续保留。已经排队的通知仍按已确认规则处理：主动暂停或取消时继续完成；撤权、解绑或重新绑定时停止受影响旧投递。旧补查终止不删除已经形成的活动，也不把已经排队的消息重新生成一遍。

## 时效口径（已确认）

时效口径定义一段延迟从什么事件开始、到什么事件结束。用户已确认摘要的 60 秒以 Telegram 确认接收为终点，外部故障单独处理和统计；正常监控及普通通知的数值目标继续适用。采集时刻的可观测性和指标计算方法留给技术设计：

以下正常性能目标适用于“低频目标定位与使用提醒”定义的低频使用场景及正常支持负载，包括偶发短时突发；持续高频场景不纳入性能保障。该范围不改变已识别记录的保留、突发摘要、异常可见性或用户隔离规则。

| 时间段 | 已确认口径 |
| --- | --- |
| 成交发生 → 数据公开可查询 | 外部公开延迟，单独呈现可核验的证据，不纳入 ATHENA 的 15/30 秒正常监控目标。 |
...
| 站内记录形成 → Telegram 明确成功接收普通通知 | 保留正常情况下 P95 ≤ 5 秒目标。成功接收指 Telegram 确认接收发送请求，不代表用户设备已展示、用户已阅读或已经下单。 |
| 首条待汇总记录形成 → Telegram 明确成功接收对应摘要 | 正常情况下不超过 60 秒；在 60 秒内只生成摘要内容不算完成发送目标，也不要求用户已经阅读。摘要仍至多每 60 秒一条。 |
| 故障补查、Telegram 异常或投递结果未知 | 外部故障时不强行承诺 60 秒，明确显示延迟、失败或结果未知；与正常时效样本分别统计，不能用排除异常来隐藏异常次数、持续时间或未完成投递。仅按已确认的避免重复原则对确定未成功的暂时失败有限重试，结果未知时停止自动重发。 |

例如，交易于 12:00:00 发生，12:00:08 公开，12:00:12 被 ATHENA 发现，12:00:13 形成站内记录，12:00:16 得到 Telegram 成功回执：交易至提交共 16 秒，ATHENA 的公开至记录耗时为 5 秒，普通通知提交耗时为 3 秒。本例中的公开时刻仅用于解释，实际无法可靠观测时必须标注该分段未知，不能伪造精确外部延迟。

## 业务规则与不变量

以下规则已逐项确认；整份需求仍需最终确认：

1. 只有当前具有产品权限的用户才能创建、查看、修改、暂停、恢复或取消自己的目标订阅。
2. 用户只能读取自己的订阅、相关目标活动和通知状态；相同目标被多个用户订阅时，各用户仍拥有独立订阅关系。
3. 管理员只读范围包括：用户身份、订阅目标及完整 Proxy Wallet、订阅状态及生命周期时间、最近成功监控时间、异常摘要、待补查状态、活动数量以及 Telegram 投递成功、确定失败和结果未知数量汇总。第一阶段管理员产品界面和业务接口不提供目标备注、完整活动内容、Telegram 消息正文或逐条投递历史，也不得提供创建、修改、暂停、恢复、取消、重发或删除用户订阅及通知的入口。
...
8. 创建订阅时先建立可信的新旧边界，基线完成后立即生效并进入监控中，不额外等待 5 分钟；首次订阅不回放历史。展示明确生效时刻，按“订阅生效边界”以成交发生时间判断资格，不因晚发现而把历史交易当成新交易。
9. 订阅在监控中发生系统或外部依赖故障时进入异常状态并继续占用配额；若期间未暂停、取消或撤权，依赖恢复后由系统自动补查当前有效区间的故障窗口，完成补查后继续原订阅，不要求用户手动恢复。发生暂停、取消或撤权时，按第 44 条终止尚未形成记录的旧补查。
10. 用户可暂停监控中或异常的订阅。主动暂停后停止新活动的识别和通知生成，并终止尚未形成记录的旧故障补查；暂停期间发生的活动不补发。暂停前已经排队的普通通知及待汇总活动继续按原规则完成，不因暂停清除。产品在操作时和状态中明确提示“已排队通知仍会继续发送，可能稍后收到”。暂停状态继续占用配额并保留目标唯一性。用户恢复时重新建立基线，只识别新生效时刻起的活动，不恢复已终止的旧补查；旧队列的完成不属于暂停期间活动补发。
...
19. 每个成交活动都必须在 `Activity Alerts` 中形成持久站内记录，作为订阅用户的完整活动历史；第一阶段不自动过期或删除这些记录。
...
26. 在低频目标使用场景及正常支持负载下，非突发摘要覆盖的活动，从站内活动记录形成到 Telegram 成功提交发送，95% 不超过 5 秒；Telegram 服务自身在成功接收后的最终送达时间不属于 ATHENA 可保证范围。偶发短时突发形成的摘要不适用该 5 秒指标，而是从首条待汇总站内记录形成到 Telegram 明确确认接收摘要不超过 60 秒；只生成摘要内容不算完成，也不要求用户已经阅读。高频场景不纳入性能保障，外部故障单独处理和统计，见“时效口径”。
27. 每条站内活动记录保存交易发生时间、ATHENA 发现时间、站内记录形成时间；存在 Telegram 投递时还保存其成功、确定失败、结果未知或终止时间及原因。
...
33. 重新获权用户必须手动恢复每个订阅；恢复时建立新基线并立即生效，只通知新生效时刻起的交易，不补发撤权期间的活动，也不恢复已终止的旧故障补查；历史缺口继续可见。
...
36. 突发摘要模式下，后续活动不再逐条发送 Telegram，而是至多每 60 秒发送一条摘要；在低频目标使用场景及正常支持负载下，从首条待汇总站内记录形成起，60 秒内应取得 Telegram 确认接收对应摘要的成功回执。外部故障按“时效口径”单独处理和统计，持续高频场景不纳入性能保障。摘要包含覆盖时间范围、活动总数、各目标账户的活动数量和站内完整记录链接，并直接列出涉及的目标、市场、Outcome、方向及市场链接。
37. 已进入某条突发摘要的活动，无论摘要最终成功、确定失败或结果未知，都不得再改为逐条 Telegram 补发；仅对确定未成功提交的暂时失败有限重试同一摘要，结果未知时停止自动重发。各站内活动显示对应摘要的结果，活动记录不受影响。
...
44. 用户暂停、取消或管理员撤权时，停止该订阅尚未形成活动记录的旧故障补查，保留缺口范围、终止原因和时间，不再把该缺口列为待补查，也不在恢复订阅后补成站内记录或 Telegram 通知。恢复完成新基线后只识别新生效时刻起的成交；已经形成的历史活动继续保留，旧排队通知遵循第 43 条。当前新区间可以恢复为监控中，但历史缺口必须继续可见，不能被标为已完整补查。
...
6. ATHENA 判断成交发生时间是否位于当前有效订阅区间、当前是否仍有权限且允许监控、是否属于通知范围，以及该订阅是否已经处理该公开成交记录。
...
flowchart TD
    ...
    subgraph MONITOR["二、持续监控与活动识别"]
        ...
        J --> CHECK{旧监控区间是否仍未被<br/>暂停、取消或撤权？}
        CHECK -->|是| K[依赖恢复后自动补查<br/>补回活动标记为延迟]
        CHECK -->|否| GAP[终止未形成记录的旧补查<br/>保留历史缺口说明]
    ...
    subgraph NOTICE["三、用户通知"]
        ...
        Q -->|否| S[加入用户级突发摘要<br/>低频支持负载下首条记录形成后<br/>60 秒内 Telegram 确认接收]
...
| 目标订阅 | 监控中或异常 | 用户主动暂停 | 已暂停 | 停止新增活动通知和未形成记录的旧补查，保留缺口；已排队通知继续完成并提示可能延迟。暂停期间发生的活动不补发，继续占用配额并保留目标唯一性。 |
| 目标订阅 | 监控中 | 暂时无法可靠观察且超过异常阈值 | 异常 | 告知订阅用户监控不可靠，保留待补查范围并继续占用配额。 |
| 目标订阅 | 异常 | 期间未暂停、取消或撤权，依赖恢复且故障窗口补查完成 | 监控中 | 系统自动补查当前区间故障活动后继续原订阅，无需用户操作。 |
| 目标订阅 | 已暂停 | 用户恢复 | 待建立基线 | 建立新边界；不通知暂停期间活动，也不恢复已终止的旧补查，历史缺口继续可见。 |
| 目标订阅 | 任意非终态 | 管理员撤销用户产品权限 | 权限停用 | 立即停止监控、旧补查和待投递通知，保留缺口、订阅与既有历史，继续占用配额。 |
| 目标订阅 | 权限停用 | 用户重新获权后手动恢复 | 待建立基线 | 建立新边界；不通知撤权期间活动，也不恢复已终止的旧补查，历史缺口继续可见。 |
| 目标订阅 | 任意非终态 | 当前具有产品权限的用户取消 | 已取消 | 停止新监控、新通知和未形成记录的旧补查，保留缺口；已排队通知继续完成并提示可能延迟。立即释放配额与目标唯一性，不能恢复原订阅。重新获权用户可以直接取消权限停用订阅；已经因撤权终止的旧队列不因取消而重新发送。 |

未成功建立初始基线的订阅不得表现为“监控中”。在未被暂停、取消或撤权的连续监控区间内，异常订阅不得通过丢弃故障窗口自动恢复为正常。因暂停、取消或撤权而终止的旧补查必须保留缺口说明；之后恢复或重新订阅可在完成新基线后进入监控中，但不能把旧缺口标为已经补齐。
...
- **外部依赖暂时失败或限流**：订阅进入异常状态；期间未暂停、取消或撤权时，依赖恢复后系统自动补查，不把技术失败伪装成目标零活动，也不要求用户手动恢复。
- **故障中暂停、取消或撤权**：立即停止未形成活动记录的旧补查，保留缺口范围和终止原因；依赖恢复后不再补旧记录。恢复订阅完成新基线后仅处理新生效时刻起的成交。已经形成的记录和旧排队通知分别按保留及投递规则处理。
...
- **短时突发大量活动**：站内仍逐条即时形成完整记录；同一用户任意连续 60 秒内前 10 条 Telegram 逐条发送，第 11 条起进入用户级摘要模式，至多每 60 秒发送一条摘要。在低频目标及正常支持负载下，从首条待汇总站内记录形成到 Telegram 确认接收摘要不超过 60 秒；外部故障按既有投递规则处理、展示异常并单独统计。活动速率恢复后退出摘要模式；不同用户互不影响。该摘要规则不构成持续高频监控的性能承诺，也不是交易者频率准入标准。
...
- 存在 Telegram 投递时的成功、确定失败、结果未知或终止时间及原因；故障补查产生的记录明确标识为延迟活动；
- Polymarket 页面链接；
- 交易哈希或其他可用于人工核验的来源标识；
- 明确的“仅通知、未执行交易”语义。
...
突发摘要展示其覆盖时间范围、活动总数、各目标账户的活动数量及站内完整记录链接，并明确说明详细成交仍逐条保存在 `Activity Alerts`。摘要同时直接展示涉及的目标、市场、Outcome、方向及市场链接，使用户从摘要本身也能知道目标在交易哪些市场。摘要覆盖的活动不再逐条补发 Telegram。

站内记录保存每个成交活动的完整历史和 Telegram 投递状态。第一阶段不提供逐订阅通知渠道设置；Telegram 是否发送完全取决于活动记录形成时该用户是否已绑定，并可能按用户级突发规则表现为逐条私聊或摘要私聊。

### 订阅状态输出

- 当前订阅是否待建立基线、监控中、已暂停、异常、权限停用或已取消；
- 最近一次成功观察时间和最近一笔已处理活动时间；
- 是否存在仍待补查的窗口或待投递通知；已经终止补查的历史缺口单独显示范围、原因和终止时间，不再表现为待补查；
...
- 最近成功监控时间、异常摘要和是否存在待补查窗口；已经终止的历史缺口仅显示范围、原因和终止时间，不提供对应成交明细；
...
3. **首次订阅立即生效且不回放历史**：基线完成后立即生效并展示明确时刻，不额外等待 5 分钟。若于 12:00:00 生效，11:59:59 成交但 12:01 才发现的交易不通知；12:00:01 成交、12:01 才发现的交易，在期间未暂停、取消或撤权时独立形成活动和提醒。
4. **相同目标可被独立订阅**：两个获授权用户订阅同一目标；新活动分别通知两位用户，双方不能看到对方的订阅或通知状态。
5. **订阅所有权隔离**：用户不能读取、暂停、恢复、取消或更改另一用户的订阅。
6. **管理员最小只读范围**：管理员能看到用户身份、目标与完整 Proxy Wallet、订阅状态和生命周期时间、监控健康与待补查状态、活动数量及 Telegram 成功、确定失败和结果未知数量汇总；不能看到目标备注、完整活动内容、Telegram 消息正文或逐条投递历史，也不能创建、修改、暂停、恢复、取消订阅或重发、删除通知。
...
14. **用户暂停与恢复**：监控中或异常的订阅均可暂停；暂停期间发生的活动不通知，暂停前已排队通知继续完成，用户可见“已排队通知仍会继续发送，可能稍后收到”的提示。暂停订阅仍占用配额且阻止重复订阅同一目标；恢复完成新基线后立即生效，只识别新生效时刻起的活动，不额外等待 5 分钟，也不恢复已终止的旧补查。
...
16. **故障后连续**：订阅发生短时故障，期间未被暂停、取消或撤权；依赖恢复后由系统自动补回当前监控区间内的新活动，不回放初始基线前历史，也不要求用户手动恢复。
...
19. **已识别站内历史完整**：无论 Telegram 是否绑定或投递成功，当前允许监控区间内每个符合条件且已识别的成交活动都形成且只形成一条可持久查看的站内记录，不自动过期或删除；因停用而终止旧补查的缺口单独保留说明，不宣称该窗口已完整识别。
...
24. **低频场景正常监控时效**：在人工选择低频目标的正常支持负载样本中，从交易可被公开查询到站内记录形成，至少 95% 不超过 15 秒、99% 不超过 30 秒；非突发摘要覆盖的活动从站内记录形成到 Telegram 成功提交发送，至少 95% 不超过 5 秒。持续高频目标不纳入性能保障，不能把低频正常场景的未达标结果归为高频而隐藏。
...
28. **重新获权后手动恢复**：用户逐个恢复订阅并建立新基线，完成后立即生效，只接收新生效时刻起的活动；不额外等待 5 分钟，不补发撤权期间交易，也不恢复已终止的旧补查，历史缺口仍可见。
29. **突发时站内不合并**：同一用户在 60 秒内收到超过 10 个成交活动；每个已识别活动仍形成独立完整的站内记录，记录数量和活动事实均不因 Telegram 摘要而减少。不能因为触发摘要就把目标自动判为高频并禁止订阅。
30. **Telegram 突发摘要**：低频目标偶发集中成交，使同一用户在任意连续 60 秒内收到 12 个成交活动；前 10 个逐条 Telegram 通知，第 11、12 个由摘要覆盖。在正常支持负载下，从首条待汇总站内记录形成起 60 秒内取得 Telegram 确认接收摘要的成功回执；仅生成摘要内容不算完成，也不要求用户已经阅读。摘要显示覆盖范围、总数、各目标数量、站内链接，并直接列出涉及的目标、市场、Outcome、方向及市场链接。两项活动以后不再逐条补发，异常结果遵守避免重复的投递规则。
...
35. **外部故障时效可见**：Telegram 故障导致摘要超过 60 秒时，展示延迟、确定失败或投递结果未知，按既有规则有限重试或停止自动重发；故障记录与正常时效样本分别统计，同时保留异常次数、持续时间和未完成投递信息，不把只生成摘要内容算作发送完成，也不隐藏异常结果。
36. **故障中暂停终止旧补查**：系统 12:00 故障、目标 12:02 成交、用户 12:03 暂停，12:04 依赖恢复。12:02 的成交若尚未形成站内记录，则不再补入站内或 Telegram；保留已终止缺口的范围、原因和时间。用户以后恢复并完成新基线后仅识别新生效时刻起的成交；旧缺口仍可见，已排队通知按原规则继续完成。
37. **撤权或取消终止旧补查**：在尚未完成故障补查时撤权或取消订阅，旧补查停止，历史缺口保留，已形成站内记录不删除。重新获权、恢复或重新订阅后均不补旧缺口。撤权停止未完成投递；主动取消允许此前已排队通知继续完成，且不为旧通知新建活动。
...
- 管理员只读查看用户身份、目标与完整 Proxy Wallet、订阅状态和生命周期时间、监控健康与待补查状态、活动数量及 Telegram 成功、确定失败和结果未知数量汇总；第一阶段不查看目标备注、完整活动内容、Telegram 消息正文或逐条投递历史，也不能操作订阅、重发或删除通知。
...
- 首次订阅和手动恢复在完成基线后立即生效，不额外等待 5 分钟，并展示明确生效时刻；按成交发生时间是否处于当前有效区间判断资格，生效前成交即使晚发现也不通知。
- 首次订阅不回放历史；未被暂停、取消或撤权的订阅在系统故障恢复后自动补查当前区间。暂停、取消或撤权时停止尚未形成记录的旧故障补查，保留已终止缺口，之后不再补入站内或通知。恢复或重新订阅仅识别新生效时刻起的成交；暂停与异常订阅继续占用名额，取消释放名额且不可恢复原订阅。
- 每条新识别的公开成交记录独立触发，同一市场或交易哈希的不同记录可以多次提醒；可明确识别为同一已处理记录的重复读取不新增提醒。保留原始事实，不等待完整成交分片，不把一条记录表述为整张订单。
- 每个成交活动均形成不自动过期的独立站内记录。用户当时已绑定 Telegram 时，正常流量逐条私聊；同一用户任意连续 60 秒超过 10 条后，第 11 条起至多每 60 秒形成一条用户级摘要，在低频目标的正常支持负载下，首条待汇总站内记录形成后 60 秒内取得 Telegram 确认接收的成功回执。摘要直接列出目标、市场、Outcome、方向及市场链接，覆盖活动不再逐条补发且不同用户互不影响。该阈值仅控制消息量，不判断目标是否低频。未绑定不阻止订阅，绑定、解绑或重新绑定均不补发此前消息；投递结果不影响站内历史；第一阶段不提供逐订阅渠道配置。
- Telegram 发送结果不确定时优先避免重复通知；据此仅对确定未成功的暂时失败有限重试，结果无法确认时停止自动重发并保留“投递结果未知”。普通通知与摘要都遵循这一原则。
- 用户已明确：仅主动暂停或取消订阅时，已经排队的通知继续完成，并提示可能存在延迟；管理员撤权或 Telegram 解绑仍停止发送。原有重新绑定后不补发历史的规则继续适用。
- 在低频目标及正常支持负载下，从交易可被公开查询到站内记录形成的 95% 不超过 15 秒、99% 不超过 30 秒；非突发摘要覆盖的活动从站内记录形成到 Telegram 成功提交发送的 95% 不超过 5 秒。偶发短时突发的摘要从首条待汇总站内记录形成到 Telegram 确认接收不超过 60 秒，仅生成内容不算完成，也不要求用户已经阅读。持续高频场景不纳入性能保障，已识别记录仍须保留。Polymarket 公开前的外部延迟单独记录，故障补查消息明确标识为延迟活动。
...
业务待确认问题：无。低频产品定位、UI 使用提醒与不自动限制高频目标的边界，以及独立触发与市场摘要、完成基线后立即生效、停止旧补查并保留缺口等条目均已确认。

整份需求仍需用户最终确认后才能将状态更新为 `已确认`。本次仅确认业务条目并维护需求，不创建技术设计或修改实现。
