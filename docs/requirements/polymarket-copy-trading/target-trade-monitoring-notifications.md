...
本文定义产品第一阶段的目标行为。核心目标是帮助订阅用户及时识别目标账户正在交易哪些市场；用户收到信息后，自行前往对应市场判断是否手动下单。产品总名同时覆盖本阶段的活动订阅与通知，以及未来独立讨论的 Copy Trading 板块；本阶段不提供交易执行入口。

各项业务决定已逐项确认，包括按单条新成交记录独立触发并保留市场摘要、完成基线后立即生效，以及暂停、取消或撤权时停止旧补查并保留缺口说明。当前没有未解决的业务待确认项；整份需求仍保持 `讨论中`，等待用户最终确认后才能作为后端技术设计输入。
...
| 时效目标 | 正常情况下，从活动公开可查询到站内记录形成：P95 不超过 15 秒、P99 不超过 30 秒；非突发 Telegram 提交：P95 不超过 5 秒；摘要按首条待汇总记录形成到 Telegram 确认接收计时，不超过 60 秒。外部公开延迟单独标识；故障补查及 Telegram 异常单独处理和统计，明确展示延迟、失败或结果未知。 |
...
26. 正常情况下，非突发摘要覆盖的活动，从站内活动记录形成到 Telegram 成功提交发送，95% 不超过 5 秒；Telegram 服务自身在成功接收后的最终送达时间不属于 ATHENA 可保证范围。突发摘要不适用该 5 秒指标，而是从首条待汇总站内记录形成到 Telegram 明确确认接收摘要不超过 60 秒；只生成摘要内容不算完成，也不要求用户已经阅读。外部故障单独处理和统计，见“时效口径”。
...
36. 突发摘要模式下，后续活动不再逐条发送 Telegram，而是至多每 60 秒发送一条摘要；正常情况下，从首条待汇总站内记录形成起，60 秒内应取得 Telegram 确认接收对应摘要的成功回执。外部故障按“时效口径”单独处理和统计。摘要包含覆盖时间范围、活动总数、各目标账户的活动数量和站内完整记录链接，并直接列出涉及的目标、市场、Outcome、方向及市场链接。
...
flowchart TD
    ...
    subgraph NOTICE["三、用户通知"]
        ...
        Q -->|是| R[逐条 Telegram 私聊<br/>正常提交 P95 不超过 5 秒]
        Q -->|否| S[加入用户级突发摘要<br/>正常情况下首条记录形成后<br/>60 秒内 Telegram 确认接收]
...
- **突发大量活动**：站内仍逐条即时形成完整记录；同一用户任意连续 60 秒内前 10 条 Telegram 逐条发送，第 11 条起进入用户级摘要模式，至多每 60 秒发送一条摘要。正常情况下，从首条待汇总站内记录形成到 Telegram 确认接收摘要不超过 60 秒；外部故障不强行承诺该时限，按既有投递规则处理、展示异常并单独统计。活动速率恢复后退出摘要模式；不同用户互不影响。
...
29. **突发时站内不合并**：同一用户在 60 秒内收到超过 10 个成交活动；每个活动仍立即形成独立完整的站内记录，记录数量和活动事实均不因 Telegram 摘要而减少。
30. **Telegram 突发摘要**：同一用户在任意连续 60 秒内收到 12 个成交活动；前 10 个逐条 Telegram 通知，第 11、12 个由摘要覆盖。正常情况下，从首条待汇总站内记录形成起 60 秒内取得 Telegram 确认接收摘要的成功回执；仅生成摘要内容不算完成，也不要求用户已经阅读。摘要显示覆盖范围、总数、各目标数量、站内链接，并直接列出涉及的目标、市场、Outcome、方向及市场链接，使用户能够从摘要识别目标正在交易哪些市场。两项活动以后不再逐条补发，异常结果遵守避免重复的投递规则。
...
- 每个成交活动均形成不自动过期的独立站内记录。用户当时已绑定 Telegram 时，正常流量逐条私聊；同一用户任意连续 60 秒超过 10 条后，第 11 条起至多每 60 秒形成一条用户级摘要，正常情况下首条待汇总站内记录形成后 60 秒内取得 Telegram 确认接收的成功回执。摘要直接列出目标、市场、Outcome、方向及市场链接，覆盖活动不再逐条补发且不同用户互不影响。未绑定不阻止订阅，绑定、解绑或重新绑定均不补发此前消息；投递结果不影响站内历史；第一阶段不提供逐订阅渠道配置。
...
- 正常监控时，从交易可被公开查询到站内记录形成的 95% 不超过 15 秒、99% 不超过 30 秒；正常情况下，非突发摘要覆盖的活动从站内记录形成到 Telegram 成功提交发送的 95% 不超过 5 秒。突发摘要从首条待汇总站内记录形成到 Telegram 确认接收不超过 60 秒，仅生成内容不算完成，也不要求用户已经阅读。Polymarket 公开前的外部延迟单独记录，故障补查消息明确标识为延迟活动。
...
业务待确认问题：无。独立触发与市场摘要、完成基线后立即生效、停止旧补查并保留缺口三项均已确认。
