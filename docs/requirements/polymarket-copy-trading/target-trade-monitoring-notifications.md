...
第一阶段需求此前已正式确认并进入技术设计，首期 10 人规模、低频目标定位、逐条活动、完成基线立即生效、不补历史及权限边界继续有效。技术核实发现时间口径与摘要边界需要调整，本次修订按仓库流程退回 `讨论中`。用户已单项确认以链上结算时间判断订阅边界，并明确展示“结算时间”；连续摘要时效和超长内容仍逐项确认，候选方案尚未替代对应规则。调整收齐后再确认整份修订需求；已有设计保持 `设计中`，没有实现授权。
...
- 在这一规模内，低频目标正常负载及偶发短时突发继续适用既有 15/30 秒监控、5 秒普通通知和 60 秒摘要目标。具体成交速率、外部配额与实际达标情况由技术设计核实，不能仅按人数宣称容量已经验证。
...
以下规则保留既有确认结论并纳入已明确选择的结算时间调整；摘要相关候选调整见“待确认问题”，整份修订需求尚待重新确认：
...
39. **首期规模覆盖**：10 名用户各自保持 10 个订阅监控，目标完全不重叠时覆盖 100 个不同目标；用户关闭网页后监控继续。低频正常负载及偶发短时突发适用既有时效目标；多人订阅同一目标时，各用户的活动、通知和摘要仍按所有者独立处理。
...
- 首期 10 人规模及既有需求此前已正式确认，用户已授权开展后端技术设计。本次结算时间调整已单项确认，摘要边界仍在讨论，整份修订需求尚待重新确认；尚未确认技术设计或授权修改后端源码。Copy Trading 的具体需求继续另行讨论。

## 待确认问题

既有业务条目已确认，包括首期按 10 人设计的规模、低频产品定位、UI 使用提醒与不自动限制高频目标的边界，以及独立触发与市场摘要、完成基线后立即生效、当前不做历史补查、恢复后继续实时监控并保留中断说明。

结算时间口径已由用户明确选择并写入本文，不再作为待决策事项。剩余业务问题逐项处理：

1. **连续摘要时效：**“摘要至多每 60 秒一条”和“首条待汇总活动形成后 60 秒内收到成功回执”存在正常网络耗时下的边界冲突。待选择保留频率并将 60 秒改为开始提交时限，或保留成功回执目标并允许更短摘要间隔。候选选择尚未替代现有摘要条款。
2. **超长摘要内容：**涉及市场较多时，一条 Telegram 文本无法保证容纳全部市场信息。待选择允许一个周期分多条发送完整摘要，或单条保留可容纳概览并链接站内完整列表；站内逐条保存要求继续有效。

具体依据见[设计中的待确认问题](../../design/trading/trader-sync-activity-alerts.md#待确认问题)。按仓库工作流，本次修订为 `讨论中`，单项选择不构成整份修订需求或技术设计确认。数据源完整覆盖、金额与钱包口径、Profile URL 精确解析、收益资料来源、容量与时效观测继续核实；核实事实不交由用户选择真假，若发现额外业务影响再明确记录。
