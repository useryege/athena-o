...
本节补充既有 45 条业务规则的可见交互，不改变后端监控、通知、配额或隔离边界。完整书面呈现见 [UI spec](../../superpowers/specs/2026-09-10-trader-sync-activity-alerts-ui-design.md)，用户已整体确认该文档；页面尚未实现。
...
数据源、金额与钱包、Profile URL 和收益口径的本轮事实见[契约核验](source-contract-verification.md)；容量及节点边界见[采集核验](collector-contract-verification.md)。完整运行样本矩阵与时效验证仍是后续实现验收工作，不要求用户选择事实真假。金额按实际来源币种准确显示是对既有“仅陈述来源事实”规则的对齐；没有加入币种转换或交易执行。若进一步核实发现业务影响再明确记录。后端与 UI 整份设计已确认，前后端联合实现正在逐项推进；运行验收不因设计确认或组件测试通过而视为完成。
