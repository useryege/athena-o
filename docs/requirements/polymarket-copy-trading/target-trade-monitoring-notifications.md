...
各项业务决定已逐项确认，包括人工选择低频目标的产品定位、UI 使用提醒与性能保障范围、按单条新成交记录独立触发并保留市场摘要、完成基线后立即生效，以及当前只做实时监控、不做历史补查。连接或服务中断期间可能遗漏成交，恢复后继续实时监控并保留中断说明。当前没有未解决的业务待确认项；整份需求仍保持 `讨论中`，等待用户最终确认后才能作为后端技术设计输入。
...
| 历史边界 | 首次订阅和手动恢复在完成基线后立即生效，展示明确生效时刻，按成交发生时间判断，不额外等待 5 分钟。当前不做历史补查，包括断线、服务重启、故障及停用期间的遗漏。仍有资格监控的订阅在故障恢复后自动继续实时监控，只识别恢复边界起的新成交；中断说明继续可见。 |
...
| 时效目标 | 面向人工选择的低频目标及其正常负载、偶发短时突发：公开可查询到站内记录形成 P95 不超过 15 秒、P99 不超过 30 秒；非突发 Telegram 提交 P95 不超过 5 秒；摘要从首条待汇总记录形成到 Telegram 确认接收不超过 60 秒。高频场景不纳入性能保障；外部延迟、监控中断及 Telegram 异常单独标识和统计，已识别记录不因频率高而丢弃。 |
...
性能保障面向低频目标及其正常负载、偶发成交分片或短时集中成交，具体承载范围由技术设计明确。持续高频交易监控和高频跟单不属于产品的性能支持目标。用户仍可按已有权限、目标解析和配额规则订阅目标，系统继续按既有活动识别、逐条保留和突发摘要规则处理；高频场景不承诺达到正常性能目标，但不能因此静默丢弃已识别活动或把异常表现为没有成交。无法可靠监控时，继续如实显示异常、延迟和可能缺失的范围；恢复后只继续实时监控，不补查遗漏。
...
首次订阅和手动恢复在完成基线后立即生效，不额外等待 5 分钟，并向用户展示明确的生效时刻。以实际成交发生时间是否处于有效监控区间决定资格，并在形成活动前遵守当前权限和订阅状态；首次发现时间不能替代成交时间。基线只确定从何时开始接收新成交，不要求扫描目标历史交易。监控中断后另建立实时恢复边界，不追溯中断期间遗漏的成交；建立可信边界的方法留给技术设计。

例如，若订阅于 12:00:00 生效，11:59:59 成交但 12:01 才发现的交易不通知；12:00:01 成交、12:01 才从实时数据源收到的交易，在期间监控连续且未暂停、取消或撤权时仍应识别。若中间发生监控中断，则按实时恢复边界处理，不以迟到记录为由回放旧区间。开始时刻纳入区间，暂停、取消或撤权的生效时刻作为区间结束，不纳入结束时刻及之后的成交；具体同秒边界的可实现性在技术设计阶段验证。
...
3. **首次订阅立即生效且不回放历史**：基线完成后立即生效并展示明确时刻，不额外等待 5 分钟，也不以扫描历史成交为前提。若于 12:00:00 生效，11:59:59 成交但 12:01 才发现的交易不通知；12:00:01 成交、12:01 才从实时数据源收到的交易，在期间监控连续且未暂停、取消或撤权时独立形成活动和提醒。
...
- 在低频目标及正常支持负载下，从交易可被公开查询到站内记录形成的 95% 不超过 15 秒、99% 不超过 30 秒；非突发摘要覆盖的活动从站内记录形成到 Telegram 成功提交发送的 95% 不超过 5 秒。偶发短时突发的摘要从首条待汇总站内记录形成到 Telegram 确认接收不超过 60 秒，仅生成内容不算完成，也不要求用户已经阅读。持续高频场景不纳入性能保障，已识别记录仍须保留。Polymarket 公开前的外部延迟单独记录；监控中断单独呈现，已接收或排队活动继续保留时间与延迟证据。
...
业务待确认问题：无。低频产品定位、UI 使用提醒与不自动限制高频目标的边界，以及独立触发与市场摘要、完成基线后立即生效、当前不做历史补查、恢复后继续实时监控并保留中断说明等条目均已确认。
