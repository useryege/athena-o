...
> 后续范围更新（2026-09-15）：用户要求不需要关注 Combo，允许按需更换公开地址。后续仅围绕普通市场及 Neg Risk 单个选项补证；下文 Combo 样本与字段差异保留为已有观察，不再继续追查或阻碍当前设计。
...
- Combo 的数据类型和市场关联差异保留为参考，不把生命周期当买卖，也不绕过执行范围限制；按 2026-09-15 的用户要求，不再安排专项补查。
