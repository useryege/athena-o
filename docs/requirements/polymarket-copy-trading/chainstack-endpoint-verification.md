...
本结论是短时端点能力验证，不是长期稳定性、吞吐或通知时效保证；端点验证本身不构成需求或技术设计确认，也不代表实现已完成。用户随后明确该端点使用免费节点；没有登录控制台读取账单设置或额度余额，没有修改业务运行配置。
...
Chainstack 已有本次实时数据和目标成交推送的正面证据，已被选为开发实时采集入口。此前 Chainnodes 端点在其验证时数据严重滞后，详见[Chainnodes 验证报告](chainnodes-endpoint-verification.md)；本次没有重新检查其是否恢复。

实时采集架构、容量预算和费用假设已随[完整后端设计](../../design/trading/trader-sync-activity-alerts.md)获得确认，历史补查不在当前范围。实际运行容量、用量费用及长期稳定性仍待实现验收；本报告的短时样本不代表这些验收已经通过。
