...
这是部署源码、公开 RPC 和实际市场查询共同支持的可行性结论。完成核心识别的难度为中等；持续运行还需要处理确认状态、重连、去重、目标集合变化与市场资料缺失。用户于 2026-09-10 明确当前不做历史补查，断线或服务恢复后仅继续实时监控并提示中断期间可能遗漏；因此补查进度和缺失区间回放不属于当前工作范围。保留下来的持续运行工作仍有工程成本，此判断不等于开发工期或性能承诺。
...
原始样本研究没有锁定主数据源、轮询间隔、组件、表结构或实现方式；后续用户明确的“不做历史补查”已同步到[关联需求](target-trade-monitoring-notifications.md#实时监控与中断恢复已确认)。研究证明链上数据不是核心能力障碍，但正式采用前仍需处理：
...
关联 Activity Alerts 整份需求已由用户正式确认为 `已确认`，[后端技术设计](../../design/trading/trader-sync-activity-alerts.md)处于 `设计中`。这些发现继续作为技术设计的核实输入；若后续方案对产品活动粒度、范围或时间定义产生实质影响，应先写回关联需求并重新确认。本报告的样本结论不构成主数据源选型、技术设计确认或实现授权。
