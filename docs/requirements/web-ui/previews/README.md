...
用户已授权先修复总体审阅问题。[v23 修订稿与证据](theme-consistency-v23/README.md)统一最终颜色、Ant 字号放大、服务表重排、选中／只读状态、数字列和管理员图标；[修订契约](../theme-consistency-contract.md)同步到全站与 Token 两份实施计划。v1–v22 和 Token v1 原件保持，正式 `ui/` 尚未改版。
