...
| Service Operations | [Sports 删除与 Worm 保留](development-runtime/sports-removal.md) | `2026-09-16 范围与数据策略已确认，尚未实施`；删除 Sports Live／History 与 World Cup Corners，专属历史数据直接删除；Worm Markets／Trading 及其数据保留 | [删除与清理配套设计](../superpowers/specs/2026-09-16-module-removal-cleanup-design.md)自查及补充已完成，[删除清理实施计划](../superpowers/plans/2026-09-16-module-removal-cleanup.md)已整理，尚未实施；[Sports 现状](../design/market-intelligence/sports-live.md)、[界面现状](../design/web-ui/market-intelligence.md) |
| Service Operations | [本地与生产服务清单核对](development-runtime/service-inventory-review.md) | `目标与现状分列`；本期访问开关不控制后台进程，原运行控制接入差距保留为历史；Token 详细接入待重构完成，旧九角色仅作差距与清理依据；恢复 Worm 后保留业务现有 16 角色、主生产 Compose 的 15 角色不是目标规模；本地独立运行并使用五个远端 Gateway 的决定继续有效，清单扩展尚未实施，Worm 配置、鉴权、就绪与停止设计已补齐 | [全栈启动配套提案](../superpowers/specs/2026-09-15-local-full-stack-design.md)已于 2026-09-16 获采用、尚未实施；[当前运行设计](../design/development-runtime/local-runtime-orchestration.md)、[Etherscan Manager](../design/blockchain-data/etherscan-manager.md) |
