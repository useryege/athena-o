...
| 业务域 | 入口 | 状态 | 关联技术设计 |
| --- | --- | --- | --- |
| Service Operations | [删除 Worm Markets，仅保留 Worm Trading](development-runtime/worm-markets-removal.md) | `完整设计已审阅确认`；按需市场目录归 Trading，Trading 功能与数据保留；尚未实施 | [内聚与退役设计](../superpowers/specs/2026-09-16-worm-trading-market-query-design.md)、[十项实施计划](../superpowers/plans/2026-09-16-worm-trading-market-query.md)已编制 |
| Service Operations | [板块访问开关：简化方案](development-runtime/business-access-control.md) | `访问范围及默认／重启规则已确认，尚未实施`；本期改为拒绝新的用户业务请求，程序、后台任务和业务通知继续运行；首次默认关闭，之后持久保存管理员设置，重启不改变开关；核心分类、BSC／Sports 删除与 Worm Markets 删除／Trading 保留决定、环境边界和 Token 详细接入延期继续有效 | [接口、存储与页面接入提案](../superpowers/specs/2026-09-15-business-access-control-design.md)方案已采用并补齐 Trading 的 `worm` 开关、全部用户入口和交易处理边界，共六个可控板块，尚未实施；[当前本地运行编排](../design/development-runtime/local-runtime-orchestration.md)、[管理员应用壳](../design/web-ui/administrator-application-shell.md)；[原整组运行需求](development-runtime/business-group-control.md)、[D01–D02](../superpowers/specs/2026-09-15-business-group-control-design.md)及 [D03](../superpowers/specs/2026-09-15-business-group-control-commands.md)已暂停，保留历史确认记录 |
| Blockchain Data | [两个 BSC 索引器删除需求](blockchain-data/bsc-indexer-removal.md) | `删除已确认，尚未实施`；移出本地及生产目标服务清单，当前只维护文档 | [删除与清理配套设计](../superpowers/specs/2026-09-16-module-removal-cleanup-design.md)自查及补充已完成，[删除清理实施计划](../superpowers/plans/2026-09-16-module-removal-cleanup.md)已整理，四个专属历史数据库直接删除已确认，尚未实施；[普通交易索引现状](../design/blockchain-data/bsc-inbound-normal-transactions.md)、[Swap 索引现状](../design/blockchain-data/bsc-v2-swap-transactions.md) |
| Service Operations | [Sports 删除与 Worm 边界](development-runtime/sports-removal.md) | `2026-09-16 范围与数据策略已确认，尚未实施`；删除 Sports Live／History 与 World Cup Corners，专属历史数据直接删除；Trading 及其数据保留，Markets 改按独立内聚设计删除 | [删除与清理配套设计](../superpowers/specs/2026-09-16-module-removal-cleanup-design.md)自查及补充已完成，[删除清理实施计划](../superpowers/plans/2026-09-16-module-removal-cleanup.md)已整理，尚未实施；[Sports 现状](../design/market-intelligence/sports-live.md)、[界面现状](../design/web-ui/market-intelligence.md) |
| Service Operations | [本地与生产服务清单核对](development-runtime/service-inventory-review.md) | `目标与现状分列`；本期访问开关不控制后台进程，原运行控制接入差距保留为历史；Token 详细接入待重构完成，旧九角色仅作差距与清理依据；当前 16／15 角色含待删除 Markets，不是目标规模；本地独立运行并使用五个远端 Gateway 的决定继续有效，清单扩展尚未实施，Trading 目标配置、鉴权、就绪与停止设计已同步 | [全栈启动配套提案](../superpowers/specs/2026-09-15-local-full-stack-design.md)已于 2026-09-16 获采用、尚未实施；[当前运行设计](../design/development-runtime/local-runtime-orchestration.md)、[Etherscan Manager](../design/blockchain-data/etherscan-manager.md) |
...
| Web UI | [市场、赛事与 Managed OO 八页集中提案](web-ui/market-intelligence-batch-proposal.md) | `已确认`（v21 八页十六张主图已整批确认，见 [v21 确认记录](web-ui/previews/theme-market-intelligence-v21-approval.json)；辅助状态沿既有规则覆盖，不追加逐图审批） | 沿用既定主题与业务契约；正式前端及真实业务验收尚未完成；[Sports 删除、Worm 保留已确认，尚未实施](development-runtime/sports-removal.md)，其余页面与共用规则保留 |
| Web UI | [Worm Trading 六页与共用界面集中提案](web-ui/worm-and-common-batch-proposal.md) | `已确认`（v22 六个业务页面／七条路由及六项共用界面差异的 24 张主图已整批确认，见 [v22 确认记录](web-ui/previews/theme-worm-and-common-v22-approval.json)；辅助状态继承覆盖与验证） | 现有 Worm 与身份权限契约保持有效；121 张合成提案／React 对照不代表正式 UI 实施或真实业务验收；[Sports 删除、Worm 保留已确认，尚未实施](development-runtime/sports-removal.md)，其余页面与共用规则保留 |
