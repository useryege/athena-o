...
| Service Operations | [本地与生产服务清单核对](development-runtime/service-inventory-review.md) | `目标与现状分列`；当前源码 15 个业务角色、生产 Compose 14 个（缺 Solana），Markets 已删除；旧 Token 九角色仍只作差距与清理依据。当前默认本地全栈仍为六进程，Trading 有独立入口但十一应用扩展尚未实施 | [全栈启动配套提案](../superpowers/specs/2026-09-15-local-full-stack-design.md)已采用、尚未实施；[当前运行设计](../design/development-runtime/local-runtime-orchestration.md)、[Etherscan Manager](../design/blockchain-data/etherscan-manager.md) |
