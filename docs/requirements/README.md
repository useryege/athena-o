...
| Trader Sync | [产品需求](polymarket-copy-trading/README.md) | Activity Alerts 与[交易板块需求](polymarket-copy-trading/copy-trading.md)均 `已确认`；用户自行到 Polymarket 入金，手动买卖及支持范围内的结算领取在 ATHENA 完成 | Activity Alerts [后端设计](../design/trading/trader-sync-activity-alerts.md)与[UI 设计](../design/web-ui/trader-sync-activity-alerts.md)已实现，见[验收记录](../testing/trader-sync-activity-alerts-acceptance.md)；[手动交易总体设计](../design/trading/polymarket-manual-trading.md)讨论中、代码未实现，[平台校验](polymarket-copy-trading/manual-trading-contract-verification.md)已有官方契约与公开样本证据，账户入金一致性、私有执行与完整数据覆盖待实测 |
