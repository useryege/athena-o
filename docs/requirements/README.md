...
| Service Operations | [Sports 删除与 Worm 边界](development-runtime/sports-removal.md) | `代码清理已实施`；现场状态见[验收记录](../testing/module-removal-cleanup-acceptance.md)；删除 Sports Live／History 与 World Cup Corners，专属历史数据直接删除；本轮保留 Worm Markets／Trading 及其数据，Markets 后续按独立内聚设计退役 | [删除与清理配套设计](../superpowers/specs/2026-09-16-module-removal-cleanup-design.md)自查及补充已完成，[删除清理实施计划](../superpowers/plans/2026-09-16-module-removal-cleanup.md)已整理，代码清理已实施；现场状态见[验收记录](../testing/module-removal-cleanup-acceptance.md)；[Sports 现状](../design/market-intelligence/sports-live.md)、[界面现状](../design/web-ui/market-intelligence.md) |
...
| Web UI | [市场、赛事与 Managed OO 八页集中提案](web-ui/market-intelligence-batch-proposal.md) | `已确认`（v21 八页十六张主图已整批确认，见 [v21 确认记录](web-ui/previews/theme-market-intelligence-v21-approval.json)；辅助状态沿既有规则覆盖，不追加逐图审批） | 沿用既定主题与业务契约；批准时正式前端及真实业务验收尚未完成；[Sports 代码删除、Worm 保留已实施](development-runtime/sports-removal.md)，其余页面与共用规则保留 |
| Web UI | [Worm Trading 六页与共用界面集中提案](web-ui/worm-and-common-batch-proposal.md) | `已确认`（v22 六个业务页面／七条路由及六项共用界面差异的 24 张主图已整批确认，见 [v22 确认记录](web-ui/previews/theme-worm-and-common-v22-approval.json)；辅助状态继承覆盖与验证） | 现有 Worm 与身份权限契约保持有效；121 张合成提案／React 对照不代表正式 UI 实施或真实业务验收；[Sports 代码删除、Worm 保留已实施](development-runtime/sports-removal.md)，其余页面与共用规则保留 |
