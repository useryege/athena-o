...
> 文档状态：目标设计草案，尚未实现。本图记录已确认的 AI 税率读取方式分析、Go 链上读取与研究资料展示范围。
...
同一份源码的 AI 读取与解释方案通过运行时字节码 `code_hash` 匹配后复用；每个部署实例的实际税率分别读取和保存。本板块只采集、解析、保存和展示资料，不增加税率阈值、风险评级或项目排除规则。源码 AI 当前只生成事实报告和依据，后续由程序按管理员维护的规则评级与筛选，见 [AI 静态质检流程](token-contract-review-flow.md)。
...
项目首次真实 Swap 为研究截止，识别不限定 V2、V3 或 V4；税率资料尚未取得不延后截止。首次 Swap 时尚未交接的项目结束研究并放弃本轮参与；结束研究不代表资料齐备、高风险或可以买入。管理员手动重检共享报告不自动恢复项目，具体衔接见 [项目研究与筛选流程](token-research-flow.md)。
...
读取与换算说明的具体数据结构、多个读取入口的选择及结果冲突处理、采集调度、失败重试、刷新和分析更新机制仍待设计。本轮不增加全部税率项必须取得的研究完成门槛。
