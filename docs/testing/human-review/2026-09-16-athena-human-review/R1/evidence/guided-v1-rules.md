...
收到已提交报告后，先用 `receiving-code-review` 核实，再用 `systematic-debugging` 定位；记录问题方案并对已确认设计范围内的缺陷连续实施。报告提交即包含这部分修复授权。用户明确的仅分析／先审方案限制继续优先；新需求、设计决定及外部操作仍按原授权边界处理。

保留原始观察，问题编号跨轮次不重置。AI 验证后的问题标为“AI 已验证，待人工复验”，不能标为人工关闭。
...
- 分支：【AI 填写】
- 提交／产物版本：【AI 填写完整 SHA、子模块 SHA 或其他不可变版本】
...
| 检查编号 | 检查内容（AI 预填） | 结果（用户填写） | 实际观察或证据 | 问题编号 |
| --- | --- | --- | --- | --- |
| CHK-001 | 【AI 预填与指南一致的具体检查】 | 未执行 |  |  |

【AI 为指南中的每个真实检查项预填一行；结果初始值均为“未执行”。】
...
3. 预期：【AI 填写应看到的完整 SHA、差异摘要或产物版本。】
...
【AI 为所有真实检查项复制该结构，并保持 CHK 编号在同一任务内稳定。】
