...
验收日期：2026-09-16。范围为 Tasks 1–10；Tasks 1–9 源码与隔离验收阶段截至 `191e4f31`，精确数据库退役工具为 `0022bed4`，最终受审产品为 `1dfcb795dfdf3da1ecb1b63e4acc9677604d90a5`。原 main default 的账户迁移、真实只读验收、五来源通知维护、专属数据库删除、正常重启和环境收尾均已完成并留证；最终产品的集中修复、scoped re-review、真实只读复验、preservation audit 和再次收尾也已完成。用户 R1 人工检查尚未执行。

| 证据层 | 状态与范围 |
| --- | --- |
| 实现 | Trading 独立目录、权限、消费者、worker、账户迁移、Markets 服务/API 清理、本地独立运行和通知维护已集成；Task 9 补成功交易回归并修复签名前后加载步骤遗漏尝试记录的问题，Task 10 增加默认报告、显式 apply 的精确数据库退役工具；最终产品为 catalog 授权、目录和 store 统一预算，并给 API 三条 catalog 调用增加 60 秒 transport deadline。 |
| 隔离副作用 | 真实 PostgreSQL、Trading SQLStore、凭据 cipher、目录校验、Solana 交易解析与 Ed25519 签名；Worm、Wallet、Solana 外部边界受控。没有真实下单、平仓或撤凭据。 |
| 真实只读 | 实际开发实例的会员/管理员、桌面/手机；shell smoke 2 项，Task 9 四场景业务 16 项，以及最终产品重建后 healthy 4 项通过，均 0 skipped/flaky。 |
| 通知与收尾 | 原 main 五来源 report/apply/report 均成功，待发和在途均为 0；没有发送测试通知。各阶段 owned 应用、替身和容器均按归属停止；最终核对 8 容器停止、8 卷保留、30 端口无监听、41 个 foreign 容器状态不变。任务结果邮件在本材料固化时尚未发送，最终状态见本机 `W/evidence/task-result-email.json`，由控制器最后写入。 |
...
首次综合单元测试另暴露 `internal/devruntime` 测试 helper 的 `/proc` 启动读取竞态；定向 20 次复现后，仅让 helper 等待本次进程的确切 run marker，带 1 秒上限。该包及 Task 9 阶段完整指定单元命令通过；未修改进程管理产品逻辑。
...
Task 9 阶段四场景每场景 4 tests 全通过（member/admin × 两尺寸），另有初始 shell smoke 2 tests。最终产品集中修复后又重建并执行 healthy 4 tests。手机成功/失败截图由控制器查看。最初三轮 locator 失败及 provider 代理环境覆盖导致的无效故障均保留，未计为通过；校准 exact 图标菜单名、Profile region 和 mobile Primary navigation 范围，没有修改产品 UI 来迎合测试。
...
该退役阶段 preservation audit 退出 0，逐项确认 16 个历史迁移源文件、28 条现行 HTTP 路径和七条 Trading 路由未因退役操作改变；Markets 的三条旧 HTTP 路径继续为 404。
...
11:27–11:28 UTC，该退役阶段先停止第二次 owned Trading borrower，再从原 main 执行 `make stop INSTANCE=full-stack`；六个应用、supervisor 和三个 owned 容器均停止，全部应用与动态基础设施端口释放，三个卷保留。通知 helper 在核对 PID 的 cwd/startTicks 后以 SIGTERM 结束，61907 释放；启动会话均正常退出，helper 的 143 符合信号终止。现场恢复任务前停止状态，没有 reset、删卷或停止归属不明资源。11:34 UTC 又核对隔离测试 PostgreSQL 的容器 ID、名称、挂载和实际 63533 映射后停止容器、释放端口并保留卷；首次 guard 因把空 HostConfig 动态端口误认作固定映射而在 mutation 前失败，改按 NetworkSettings 核对后才执行正确停止。该阶段全部临时环境均已停止。

## 最终产品复审、真实只读复验与再次收尾

整分支审阅对 `617bd6a2..d66c0c3d` 提出 catalog deadline/budget 的 I1 及 M1–M3；唯一集中修复提交 `1dfcb795dfdf3da1ecb1b63e4acc9677604d90a5`。唯一 scoped re-review 逐项确认 I1、M1–M3 全部 ADDRESSED，且修复 diff 没有新 findings。修复让 `GetOrderEventCatalog`、`CreateMarketCombination`、`UpdateMarketCombination` 从授权前共享同一 Trading catalog budget；API 三个调用使用固定 60 秒 transport deadline，并保留更早 caller deadline、取消传播和无自动写重试的边界。定向 Go、受影响包、UI TypeScript 与 e2e lint 证据见本机 `final-fix-report.md`、复审见 `final-rereview.md`。

控制器把原 main/rf4 本地快进到该完整产品版本后，从实际源码重建 full-stack 与独立 external Trading borrower。四次 bootstrap 均为 200/authenticated；首个 UI member readiness 仍耗时 5800.596ms，UI admin 为 2.522ms，直连 API member/admin 为 1.221ms/0.875ms。此前首次重启 3 passed / 1 failed 的证据继续保留；这次冷启动 UI 延迟再次出现，具体 pre-gRPC 根因仍未定位，不能写成已修复。

完整 source comparison 确认 main 当前 checkout 相对受审产品，除 `docs`、`.agents`、`.codex`、`AGENTS.md` 与另一任务明确的 `tools/task-completion-email/main.go` 外，全部 tracked 文件逐字无差异，覆盖 `common`、`pkg`、`assets`、deploy 和 config 等运行输入；证据为 `evidence/final-budget-complete-source-comparison.json`。

2026-09-16 12:20:54 UTC 的最终产品真实只读 spec 为 4 passed / 0 failed / 0 skipped / 0 flaky，耗时 17.206s。它核对精确 URL 与 h1、member/admin × 1440/390、实际目录 3 个子市场和只读价格提示；浏览器仅发同源 GET/HEAD/OPTIONS，没有保存组合或调用供应商 mutation。三个旧 Markets HTTP 路径仍为 404。Trading 空业务库仍只允许现场覆盖四条会员路由；三条历史详情和旧凭据解密没有现场数据，继续由受控 fixture 证据与明确限制承接，不冒称真实供应商下单、Close、revoke 或历史凭据验证。

最终产品 preservation audit 再次确认 16 个历史迁移源文件、28 条现行 HTTP 路径和七条 Trading 路由保持。只读 SQL 仍看到相同 server identity `7685665987292844070`、九个保留库且 `worm_markets` absent；Trading／Wallet 每表 count/fingerprint、两账户 revision 2 与 14 grants/flags、九个共享通知事实、原两 sender 指纹和 Bot offset 均保持。没有重复 DROP 或通知 apply。

12:23–12:25 UTC 先以 `make stop-instance INSTANCE=worm-retirement-field-final-20260916` 停止最终 borrower，再以 `make stop INSTANCE=full-stack` 停止 owner；拒发 Telegram helper 在核对 cwd/startTicks 后 SIGTERM 退出，143 符合信号终止。最终核对全部本任务应用、supervisor 和 helper 退出，8 个 owned 容器 stopped、8 卷保留、30 个旧／新端口无监听；41 个 original foreign 容器状态完整不变，其中 7 个仍运行。没有 reset、删卷或终止归属不明进程。完整事实与 JSON 索引见本机证据根的 `final-live-evidence.md`、`evidence/preservation-audit-product-1dfcb795.json`、`evidence/task-10-field-browser-final-budget-fix.json` 和 `evidence/environment-final-budget-verification.json`。
...
以上命令在 Task 9 阶段全部退出 0。race JSON 中 Trading 19、server 4 个 pass 事件（包括子测试，不能当作 23 个顶层测试）；补充父 batch 终态后受影响测试再次带 race 通过。首次完整 unit 命令因上述测试竞态失败，修复后的完整命令通过。集成日志最慢 devruntime 235.376s，未因运行时间截断。
...
以上在 Task 9 阶段通过。UI build 有既存 bundle size advisory。ESLint 仅对四个实际适用 e2e/config 文件执行成功；三个 Jest 文件受现有 ignore，不声称其已 lint。第一次 ShellCheck 使用不存在的猜测路径失败，随后按 Tasks 6/7 实际脚本路径检查通过。

## SDS-R1–R8 证据映射

| 规则 | 本项证据 |
| --- | --- |
| R1 边界 | Trading 为独立进程；API 不持有 Trading runtime。Task 9 阶段真实 owner/borrower 配置和二进制记录；本次 owned 新实例没有 Markets 进程/库。 |
...
| R5 所有权 | borrower 先停止，owner 保持运行；随后 owner 六进程/三容器及三个 helper 按归属停止；未 reset、未删卷。helper 停止后的首次即时端口绑定断言因释放竞态失败，随后复核端口释放成功；Task 9 阶段证据采用复核结果并保留首次失败。 |
...
- Task 9 阶段真实业务：`.tmp/athena-ui-acceptance/worm-retirement-real-{healthy,provider-down,account-down,restored}-4/`；该阶段收口目录为 `.tmp/athena-ui-acceptance/worm-retirement-real-healthy-final/`，包括报告、trace、截图、route coverage 和 GET 结果。
...
Task 9 阶段 Trading 源码 `execution_runs.go` SHA256 为 `c9073fba9495686c2a213221095198b88b8ca51ac1812959b42a6cf497cd61b1`，实际 binary SHA256 为 `5481ef5fd89dfea608bcece3883a695aa069f7d1533d063855c565331423fe84`。该阶段 PID 1305291 已退出，61906 释放。`make stop-instance INSTANCE=worm-retirement-live` exit 0，12.286 秒；health Watch 总寿命 32.65 秒包含此前等待，不是停止耗时。
...
Task 10 退役阶段证据位于同一证据根下的 `task-10-field-evidence.md`、`evidence/field-*.json`、`evidence/task-10-field-*.json` 及对应 stop log。最终产品复验另见 `final-live-evidence.md` 及其引用 JSON。原 main 的应用、supervisor、owned 容器、各 field borrower、通知 helper 和隔离测试 PG 均已按各阶段归属停止，现场及测试端口释放，所属卷保留。没有遗留本任务启动的运行实例。
