	...
	for _, reason := range []string{moduleaccess.ClosedReason, moduleaccess.UnavailableReason} {
		for _, purpose := range []string{"credentials", "execution", "cash_out", "batch"} {
			for _, binding := range []string{"bound", "account", "jti", "revision"} {
				t.Run(reason+"/"+purpose+"/"+binding, func(t *testing.T) {
					...
					expected := reason
					if binding != "bound" {
						expected = map[string]string{"credentials": "WORM_TRADING_REAUTH_REQUIRED", "execution": WormExecutionAuthorizationRequiredReason, "cash_out": WormPositionCashOutAuthorizationRequiredReason, "batch": WormPositionCashOutBatchAuthorizationRequiredReason}[purpose]
					}
					...
					case "credentials":
						...
						h.wormCredentials = &wormCredentialReauthentication{google: h, store: &wormCredentialTransactionStore{redis: client}, authenticate: authenticate, admit: admit}
					case "execution":
						...
						h.wormExecutions = &wormExecutionAuthorization{google: h, store: &wormExecutionTransactionStore{redis: client}, authenticate: authenticate, admit: admit}
					case "cash_out":
						...
						h.wormPositionCashOuts = &wormPositionCashOutAuthorization{google: h, store: &wormPositionCashOutTransactionStore{redis: client}, authenticate: authenticate, admit: admit}
					case "batch":
						...
						h.wormPositionCashOutBatches = &wormPositionCashOutBatchAuthorization{google: h, store: &wormPositionCashOutBatchTransactionStore{redis: client}, authenticate: authenticate, admit: admit}
					}
					...
					require.Contains(t, w.Header().Get("Location"), expected)
