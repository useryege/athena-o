...
func (s *SQLStore) CompleteProjectAveDataCollection(ctx context.Context, task domain.ProjectDataCollectionTask, observation domain.AveObservationV1, checkedAt time.Time) (*domain.ProjectObservation, error) {
	return s.completeTypedObservation(ctx, task, observation, nil, checkedAt, false)
}
func (s *SQLStore) CompleteProjectChainStateCollection(ctx context.Context, task domain.ProjectDataCollectionTask, observation domain.ChainStateObservationV1, blockNumber uint64, checkedAt time.Time) (*domain.ProjectObservation, error) {
	return s.completeTypedObservation(ctx, task, observation, &blockNumber, checkedAt, false)
}
func (s *SQLStore) CompleteProjectWalletAssetStateCollection(ctx context.Context, task domain.ProjectDataCollectionTask, observation domain.WalletAssetObservationV1, blockNumber uint64, checkedAt time.Time) error {
	_, e := s.completeTypedObservation(ctx, task, observation, &blockNumber, checkedAt, false)
	return e
}
func (s *SQLStore) CompleteProjectSimulationResultCollection(ctx context.Context, task domain.ProjectDataCollectionTask, observation domain.SimulationObservationV1, blockNumber uint64, checkedAt time.Time) error {
	_, e := s.completeTypedObservation(ctx, task, observation, &blockNumber, checkedAt, false)
	return e
}

func (s *SQLStore) CompleteProjectContractCodeSourceCollection(ctx context.Context, task domain.ProjectDataCollectionTask, codeHash common.Hash, sourceCode string, checkedAt time.Time) error {
	...
	payload, _ := json.Marshal(domain.ContractSourceObservationV1{CodeHash: codeHash, SourceAvailable: true})
	if _, e = recordObservation(ctx, q, task.ProjectID, task.DataType, domain.ObservationSchemaVersionV1, payload, nil, checkedAt); e != nil {
		return e
	}
...
func (s *SQLStore) completeUnavailableContractSource(ctx context.Context, task domain.ProjectDataCollectionTask, checkedAt time.Time) error {
	if checkedAt.IsZero() {
...
func (s *SQLStore) completeTypedObservation(ctx context.Context, task domain.ProjectDataCollectionTask, observation any, blockNumber *uint64, checkedAt time.Time, completeSchedule bool) (*domain.ProjectObservation, error) {
...
func (s *SQLStore) completeObservation(ctx context.Context, task domain.ProjectDataCollectionTask, payload json.RawMessage, blockNumber *uint64, checkedAt time.Time, completeSchedule bool) (*domain.ProjectObservation, error) {
	if checkedAt.IsZero() {
		checkedAt = time.Now().UTC()
	}
	tx, e := s.pool.Begin(ctx)
	...
	defer func() { _ = tx.Rollback(ctx) }()
	q := tokensqlc.New(tx)
	obs, e := recordObservation(ctx, q, task.ProjectID, task.DataType, domain.ObservationSchemaVersionV1, payload, blockNumber, checkedAt)
	if e != nil {
		return nil, e
	}
	n, e := q.MarkProjectDataCollectionTaskSucceeded(ctx, task.ID)
...
func recordObservation(ctx context.Context, q *tokensqlc.Queries, projectID int64, dataType domain.DataCollectionType, schemaVersion int32, payload json.RawMessage, blockNumber *uint64, checkedAt time.Time) (*domain.ProjectObservation, error) {
