...
func (s *SQLStore) ListDueProjectDataCollectionTasks(ctx context.Context, dataType domain.DataCollectionType, chainIDs []int64, limit int32) ([]domain.ProjectDataCollectionTaskWithProject, error) {
	if len(chainIDs) == 0 {
		return nil, nil
	}
	q, e := s.querier()
	...
	out := make([]domain.ProjectDataCollectionTaskWithProject, 0, len(rows))
	for _, row := range rows {
		...
		out = append(out, domain.ProjectDataCollectionTaskWithProject{Task: mapProjectDataCollectionTask(row), Project: *project})
	}
	return out, nil
}
func (s *SQLStore) GetProjectDataCollectionTask(ctx context.Context, id int64) (*domain.ProjectDataCollectionTask, error) {
	q, e := s.querier()
	if e != nil {
		return nil, e
	}
...
func (s *SQLStore) ListProjectDataCollectionTasks(ctx context.Context, projectID int64, dataType, status string, page, pageSize int32) (*ProjectDataCollectionTaskPage, error) {
	...
	items := make([]domain.ProjectDataCollectionTask, 0, len(rows))
	for _, r := range rows {
...
func (s *SQLStore) MarkProjectDataCollectionTaskSucceeded(ctx context.Context, task domain.ProjectDataCollectionTask) (bool, error) {
	if s == nil || s.pool == nil {
	...
	now := time.Now().UTC()
	if task.DataType == domain.DataCollectionTypeContractCodeSource {
...
func (s *SQLStore) MarkProjectDataCollectionTaskFailed(ctx context.Context, task domain.ProjectDataCollectionTask, lastError string) (*domain.ProjectDataCollectionTask, bool, error) {
	if s == nil || s.pool == nil {
