...
type Store interface {
	ClaimProjectSelectionEvaluationTasks(context.Context, int32) ([]selection.ProjectSelectionEvaluationTask, error)
	GetProjectReportRevision(context.Context, int64, int64) (*reporting.ProjectReportRevision, error)
	CompleteProjectSelectionEvaluation(context.Context, selection.ProjectSelectionEvaluationTask, selection.ProjectSelection) (*selection.ProjectSelection, bool, error)
	MarkProjectSelectionEvaluationTaskFailed(context.Context, selection.ProjectSelectionEvaluationTask, string) error
}

type Strategy interface {
	Key() string
	Version() string
	Evaluate(context.Context, selection.StrategyInput) (selection.SelectionDecision, error)
}
...
func (NotConfiguredStrategy) Evaluate(context.Context, selection.StrategyInput) (selection.SelectionDecision, error) {
	return selection.SelectionDecision{Outcome: selection.SelectionOutcomeDeferred, ReasonCodes: []string{"strategy_not_configured"}}, nil
}
...
func (w *Worker) process(ctx context.Context, task selection.ProjectSelectionEvaluationTask) {
	...
	decision, err := strategy.Evaluate(ctx, selection.StrategyInput{ProjectID: report.ProjectID, ReportRevision: report.Revision, Report: report.Report})
	if err != nil {
		_ = w.opts.Store.MarkProjectSelectionEvaluationTaskFailed(ctx, task, err.Error())
		return
	}
	...
	selection := selection.ProjectSelection{ProjectID: task.ProjectID, Outcome: decision.Outcome, StrategyKey: strategy.Key(), StrategyVersion: strategy.Version(), ReportRevision: task.ReportRevision, ReasonCodes: decision.ReasonCodes, ReasonDetail: decision.ReasonDetail, DecidedAt: time.Now().UTC()}
...
func normalizeSelectionDecision(decision selection.SelectionDecision) (selection.SelectionDecision, error) {
	switch decision.Outcome {
	case selection.SelectionOutcomeSelected, selection.SelectionOutcomeRejected, selection.SelectionOutcomeDeferred:
	default:
		return selection.SelectionDecision{}, fmt.Errorf("token selection strategy returned invalid outcome %q", decision.Outcome)
	}
	...
	for _, reasonCode := range decision.ReasonCodes {
		...
		if !reasonCodePattern.MatchString(reasonCode) {
			return selection.SelectionDecision{}, fmt.Errorf("token selection strategy returned invalid reason code %q", reasonCode)
		}
	...
	if len(reasonCodes) == 0 {
		return selection.SelectionDecision{}, fmt.Errorf("token selection strategy must return at least one reason code")
	}
