...
func (s *Database) ScheduleDueProjectDataCollectionTasks(ctx context.Context, limit int32) (int, error) {
	q, err := s.querier()
	if err != nil {
		return 0, err
	}
	...
	for _, candidate := range schedules {
		...
		if locked.Status != string(research.DataCollectionScheduleStatusActive) || timeValue(locked.NextRunAt).After(time.Now().UTC()) {
			_ = tx.Rollback(ctx)
			continue
		}
...
func (s *Database) ApplyResearchPolicy(ctx context.Context, intervals map[research.DataCollectionType]time.Duration, ttl time.Duration) error {
	q, err := s.querier()
	if err != nil {
		return err
	}
...
func (s *Database) MaintainResearchLifecycle(ctx context.Context) error {
	q, e := s.querier()
	if e != nil {
		return e
	}
...
func markScheduleSucceeded(ctx context.Context, q *tokensqlc.Queries, projectID int64, dataType research.DataCollectionType, checkedAt time.Time) error {
...
func markScheduleFailed(ctx context.Context, q *tokensqlc.Queries, projectID int64, dataType research.DataCollectionType, lastError string, failedAt time.Time) error {
	schedule, e := q.GetProjectDataCollectionSchedule(ctx, tokensqlc.GetProjectDataCollectionScheduleParams{ProjectID: projectID, DataType: string(dataType)})
	if e != nil {
		return e
	}
