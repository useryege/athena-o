...
var defaultCollectionIntervals = map[research.DataCollectionType]time.Duration{
	research.DataCollectionTypeChainState:         15 * time.Second,
	research.DataCollectionTypeWalletAssetState:   60 * time.Second,
	research.DataCollectionTypeSimulationResult:   60 * time.Second,
	research.DataCollectionTypeAve:                5 * time.Minute,
	research.DataCollectionTypeContractCodeSource: 10 * time.Minute,
}

func (s *Database) ValidateProjectCandidate(ctx context.Context, candidate discovery.ProjectCandidate, codeHash common.Hash, token catalog.ProjectTokenMetadata, wethPair, usdtPair common.Address, relatedWallets []catalog.ProjectRelatedWallet, initialRecipients []catalog.ProjectInitialRecipient) (*catalog.Project, error) {
	if s == nil || s.pool == nil {
	...
	if _, err = q.CompleteProjectCandidateValidation(ctx, tokensqlc.CompleteProjectCandidateValidationParams{ID: candidate.ID, Status: string(discovery.ProjectCandidateStatusValidated), ValidationLockToken: uuidParam(candidate.ValidationLockToken)}); err != nil {
