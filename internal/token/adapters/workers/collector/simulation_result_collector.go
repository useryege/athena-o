...
func (r *dataCollectorRunner) processSimulationResultTasks(ctx context.Context) error {
	tasks, err := r.opts.store.ListDueProjectDataCollectionTasks(ctx, research.DataCollectionTypeSimulationResult, r.opts.chainIDs, simulationTaskLimit)
	if err != nil {
		return err
	}
	if len(tasks) == 0 {
...
func (r *dataCollectorRunner) processSimulationResultTask(ctx context.Context, task research.ProjectDataCollectionTaskWithProject) {
	client, err := r.ensureClient(ctx, research.DataCollectionTypeSimulationResult, task.Project.ChainID)
	if err != nil {
	...
	caller, err := r.ensureCaller(ctx, research.DataCollectionTypeSimulationResult, task.Project.ChainID)
	if err != nil {
		r.markTaskFailed(ctx, task.Task, err)
		return
	}
	...
	if err != nil {
		r.resetChain(research.DataCollectionTypeSimulationResult, task.Project.ChainID)
	...
	if err := r.opts.store.CompleteProjectSimulationResultCollection(ctx, task.Task, research.SimulationObservationV1{Items: results}, blockNumber, time.Now().UTC()); err != nil {
		r.markTaskFailed(ctx, task.Task, err)
		return
	}
...
func (r *dataCollectorRunner) completeEmptySimulationResultTask(ctx context.Context, task research.ProjectDataCollectionTaskWithProject, blockNumber uint64, reason string) {
	if err := r.opts.store.CompleteProjectSimulationResultCollection(ctx, task.Task, research.SimulationObservationV1{}, blockNumber, time.Now().UTC()); err != nil {
		r.markTaskFailed(ctx, task.Task, err)
		return
	}
	log.WithFields(log.Fields{
