...
func (r *validatorRunner) processPendingCandidates(ctx context.Context) error {
	...
	for _, chainID := range r.opts.chainIDs {
		...
		if processErr != nil {
			if releaseErr != nil {
				log.WithError(releaseErr).WithField("validation_lock_token", lockToken).Error("token project validator failed to release claims")
			}
			return processErr
		}
		if releaseErr != nil {
...
func (r *validatorRunner) renewCandidateValidationClaims(ctx context.Context, lockToken string) func() {
		...
		for {
			...
			case <-ticker.C:
				if err := r.opts.store.RenewProjectCandidateValidationClaims(renewCtx, lockToken, candidateLease); err != nil && renewCtx.Err() == nil {
					log.WithError(err).WithField("validation_lock_token", lockToken).Error("token project validator failed to renew claims")
				}
			}
		}
	}()
...
func (r *validatorRunner) processChainCandidates(ctx context.Context, chainID int64, candidates []discovery.ProjectCandidate) error {
...
func (r *validatorRunner) processValidatedCandidate(ctx context.Context, client *ethclient.Client, candidate discovery.ProjectCandidate, validation tokenValidation) error {
	...
	token := catalog.ProjectTokenMetadata{
...
func (r *validatorRunner) rejectCandidate(ctx context.Context, candidate discovery.ProjectCandidate, message string) error {
