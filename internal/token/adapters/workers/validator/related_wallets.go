...
func fetchProjectCreationReceipt(ctx context.Context, client *ethclient.Client, candidate discovery.ProjectCandidate) (*types.Receipt, error) {
...
func buildProjectRelatedWallets(candidate discovery.ProjectCandidate, initialRecipients []initialRecipientWallet) []catalog.ProjectRelatedWallet {
	wallets := make([]catalog.ProjectRelatedWallet, 0, 1+len(initialRecipients))
	if candidate.TxSender != (common.Address{}) {
		wallets = append(wallets, catalog.ProjectRelatedWallet{
			Wallet: candidate.TxSender,
			Role:   catalog.RelatedWalletRoleCreator,
		})
	}
	for _, recipient := range initialRecipients {
		...
		wallets = append(wallets, catalog.ProjectRelatedWallet{
			Wallet: wallet,
			Role:   catalog.RelatedWalletRoleInitialRecipient,
		})
	}
	return wallets
}

func buildProjectInitialRecipients(candidate discovery.ProjectCandidate, initialRecipients []initialRecipientWallet, totalSupply *big.Int) []catalog.ProjectInitialRecipient {
	if len(initialRecipients) == 0 || totalSupply == nil || totalSupply.Sign() <= 0 {
		return nil
	}
	items := make([]catalog.ProjectInitialRecipient, 0, len(initialRecipients))
	for i, recipient := range initialRecipients {
		...
		items = append(items, catalog.ProjectInitialRecipient{
			Wallet:            recipient.wallet,
