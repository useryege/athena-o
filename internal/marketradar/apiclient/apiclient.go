...
type Clientset interface {
	MarketRadar() MarketRadarServiceClient
...
type clientSet struct {
	...
	client     MarketRadarServiceClient
}

func (c *clientSet) MarketRadar() MarketRadarServiceClient {
	return c.client
}

func NewMarketRadarClientset(address string) (Clientset, error) {
	connection, err := utilgrpc.NewClientConnection(address)
	if err != nil {
		return nil, err
	}
	return &clientSet{connection: connection, client: NewMarketRadarServiceClient(connection.ClientConn())}, nil
}
