...
func (s *Service) ListRealtimeMarkets(ctx context.Context, req *apiclient.ListRealtimeMarketsRequest) (*apiclient.ListRealtimeMarketsResponse, error) {
	s.startStopMu.Lock()
	started := s.started
...
func (s *Service) currentRealtimeMarketResponse(limit int) *apiclient.ListRealtimeMarketsResponse {
	s.cacheMu.Lock()
	...
	now := s.now()
	nowUnix := now.Unix()
	items := make([]*v1alpha1.MarketRadarRealtimeMarketItem, 0, limit)
	for i := 0; i < limit; i++ {
		market := markets[i]
		item := &v1alpha1.MarketRadarRealtimeMarketItem{
			...
			Tokens:       make([]*v1alpha1.MarketRadarRealtimeTokenItem, 0, len(market.Tokens)),
		}
	...
	return &apiclient.ListRealtimeMarketsResponse{
		Items:             items,
		FetchedAt:         fetchedAt,
...
func (s *Service) realtimeTokenItemLocked(token *v1alpha1.MarketRadarHotMarketTokenItem, state *realtimeTokenState, nowUnix int64) *v1alpha1.MarketRadarRealtimeTokenItem {
	price := token.Price
	outcome := token.Outcome
	item := &v1alpha1.MarketRadarRealtimeTokenItem{
		TokenID: token.TokenID,
		Outcome: outcome,
		Price:   price,
		Warmup:  true,
		Windows: []*v1alpha1.MarketRadarRealtimeWindowItem{
	...
	if state != nil {
		...
		item.Windows = []*v1alpha1.MarketRadarRealtimeWindowItem{
...
func (s *Service) realtimeWindowItemLocked(tokenID string, nowUnix int64, window time.Duration, currentPrice float64) *v1alpha1.MarketRadarRealtimeWindowItem {
	samples := s.realtimeSamples[tokenID]
	windowSeconds := int64(window.Seconds())
	label := realtimeWindowLabel(window)
	out := &v1alpha1.MarketRadarRealtimeWindowItem{
		Window:      label,
		Warmup:      true,
...
func (s *Service) ensureRealtimeStatesForMarketsLocked(markets []*v1alpha1.MarketRadarHotMarketItem) {
...
func (s *Service) cleanupRealtimeStatesForMarketsLocked(markets []*v1alpha1.MarketRadarHotMarketItem) {
...
func realtimeTopHotMarketsLocked(items []*v1alpha1.MarketRadarHotMarketItem) []*v1alpha1.MarketRadarHotMarketItem {
	limit := hotMarketTargetLimit
	if len(items) < limit {
		limit = len(items)
	}
	out := make([]*v1alpha1.MarketRadarHotMarketItem, 0, limit)
	for i := 0; i < limit; i++ {
