...
type hotMarketCandidate struct {
	item *v1alpha1.MarketRadarHotMarketItem
	rank int
}
...
func (s *Service) ListHotMarkets(ctx context.Context, req *apiclient.ListHotMarketsRequest) (*apiclient.ListHotMarketsResponse, error) {
	s.startStopMu.Lock()
	started := s.started
...
func (s *Service) fetchHotMarketCandidates(ctx context.Context) ([]hotMarketCandidate, error) {
	...
	items := make([]*v1alpha1.MarketRadarHotMarketItem, 0, hotMarketCandidateLimit)
	cursor := ""
	closed := false
	ascending := false
	includeTag := true
...
func (s *Service) applyHotMarketCandidatesLocked(candidates []hotMarketCandidate) {
	...
	out := make([]*v1alpha1.MarketRadarHotMarketItem, 0, hotMarketTargetLimit)
...
func mapHotMarket(market utilpolymarket.Market) (*v1alpha1.MarketRadarHotMarketItem, bool) {
	...
	tokens := make([]*v1alpha1.MarketRadarHotMarketTokenItem, 0, len(tokenIDs))
	...
	for i := range tokenIDs {
		...
		tokens = append(tokens, &v1alpha1.MarketRadarHotMarketTokenItem{
			TokenID: tokenID,
			Outcome: outcome,
			Price:   price,
		})
	...
	return &v1alpha1.MarketRadarHotMarketItem{
...
func sortHotMarkets(items []*v1alpha1.MarketRadarHotMarketItem) {
...
func (s *Service) currentHotMarketResponse(limit int) *apiclient.ListHotMarketsResponse {
	s.cacheMu.RLock()
	defer s.cacheMu.RUnlock()
	if len(s.hotMarketItems) == 0 && s.hotMarketFetched == 0 {
		return nil
	...
	items := make([]*v1alpha1.MarketRadarHotMarketItem, 0, limit)
	for i := 0; i < limit; i++ {
		items = append(items, cloneHotMarketItem(s.hotMarketItems[i]))
	}
	return &apiclient.ListHotMarketsResponse{
		Items:            items,
...
func cloneHotMarketItem(in *v1alpha1.MarketRadarHotMarketItem) *v1alpha1.MarketRadarHotMarketItem {
	if in == nil {
		return nil
	}
	out := *in
	out.Tokens = make([]*v1alpha1.MarketRadarHotMarketTokenItem, 0, len(in.Tokens))
...
func countHotMarketTokens(items []*v1alpha1.MarketRadarHotMarketItem) int {
	total := 0
	for i := range items {
		if items[i] == nil {
			continue
