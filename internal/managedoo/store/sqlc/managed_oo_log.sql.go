...
const getManagedOOChainLogCursor = `-- name: GetManagedOOChainLogCursor :one
SELECT
  sync_name,
  contract_address,
  topic,
...
func (q *Queries) GetManagedOOChainLogCursor(ctx context.Context, syncName string) (ManagedOoChainLogCursor, error) {
	row := q.db.QueryRow(ctx, getManagedOOChainLogCursor, syncName)
	var i ManagedOoChainLogCursor
	err := row.Scan(
		&i.SyncName,
		&i.ContractAddress,
		&i.Topic,
...
const upsertManagedOOChainLogCursor = `-- name: UpsertManagedOOChainLogCursor :one
...
type UpsertManagedOOChainLogCursorParams struct {
	SyncName        string
	ContractAddress string
	Topic           string
...
func (q *Queries) UpsertManagedOOChainLogCursor(ctx context.Context, arg UpsertManagedOOChainLogCursorParams) (ManagedOoChainLogCursor, error) {
	row := q.db.QueryRow(ctx, upsertManagedOOChainLogCursor,
		arg.SyncName,
		arg.ContractAddress,
		arg.Topic,
		arg.LastBlockNumber,
