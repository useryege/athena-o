...
INSERT INTO worm_markets_market (
  condition_id,
  title,
  description,
  logo,
...
SET title = EXCLUDED.title,
  ...
  live_state = worm_markets_market.live_state,
  live_checked_at = worm_markets_market.live_checked_at,
  live_price_change = worm_markets_market.live_price_change,
  raw = EXCLUDED.raw,
...
INSERT INTO worm_markets_price_history (
  condition_id,
  price,
  sampled_at
)
...
DELETE FROM worm_markets_price_history
...
SELECT
  ...
  EXISTS (
    SELECT 1
    FROM worm_markets_market AS live_market
    WHERE live_market.event_condition_id = market.event_condition_id
      AND live_market.live_state = 'live'
  )::boolean AS event_has_live
FROM worm_markets_market AS market
LEFT JOIN worm_markets_price_history AS history
...
DELETE FROM worm_markets_market
WHERE condition_id = @condition_id;
