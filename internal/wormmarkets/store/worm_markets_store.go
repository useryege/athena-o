...
func (s *SQLStore) BatchInsertWormMarketPriceHistory(ctx context.Context, samples []WormMarketPriceSample) error {
	...
	params := wormmarketssqlc.BatchInsertWormMarketPriceHistoryParams{
...
func (s *SQLStore) UpdateWormMarketPriceAlertBand(ctx context.Context, conditionID, expectedBand, band string) (bool, error) {
	...
	rowsAffected, err := q.UpdateWormMarketPriceAlertBand(ctx, wormmarketssqlc.UpdateWormMarketPriceAlertBandParams{
...
func (s *SQLStore) SetWormMarketRulesIfMissing(ctx context.Context, conditionID string, rules json.RawMessage) (int64, error) {
	...
	rowsAffected, err := q.SetWormMarketRulesIfMissing(ctx, wormmarketssqlc.SetWormMarketRulesIfMissingParams{
		ConditionID: conditionID,
		Rules:       []byte(rules),
	})
	if err != nil {
...
func (s *SQLStore) ListWormEventsPage(ctx context.Context, page, pageSize int32) (*WormEventPage, error) {
	...
	eventRows, err := q.ListWormEventsPage(ctx, wormmarketssqlc.ListWormEventsPageParams{
		Offset: offset,
		Limit:  pageSize,
	})
	if err != nil {
...
func (s *SQLStore) UpdateWormMarketLiveState(ctx context.Context, item WormMarket) (*WormMarket, error) {
	...
	row, err := q.UpdateWormMarketLiveState(ctx, wormmarketssqlc.UpdateWormMarketLiveStateParams{
...
func upsertWormMarketParams(item WormMarket) wormmarketssqlc.UpsertWormMarketParams {
	...
	return wormmarketssqlc.UpsertWormMarketParams{
		ConditionID:      item.ConditionID,
		Title:            item.Title,
...
func updateWormMarketParams(item WormMarket) wormmarketssqlc.UpdateWormMarketParams {
	params := upsertWormMarketParams(item)
	return wormmarketssqlc.UpdateWormMarketParams{
...
func batchUpsertWormMarketsParams(items []WormMarket) wormmarketssqlc.BatchUpsertWormMarketsParams {
	now := time.Now().UTC()
	params := wormmarketssqlc.BatchUpsertWormMarketsParams{
