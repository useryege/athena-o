...
type Service struct {
	apiclient.UnimplementedWormMarketsServiceServer
	store                 *wormmarketsstore.SQLStore
...
func (s *Service) Start() error {
	...
	if s.store == nil {
		return status.Error(codes.FailedPrecondition, "Worm Markets store is required")
	}
	if s.wormClient == nil {
...
func (s *Service) GetWormMarketsStatus(context.Context, *apiclient.GetWormMarketsStatusRequest) (*apiclient.GetWormMarketsStatusResponse, error) {
	s.startStopMu.Lock()
	started := s.started
	s.startStopMu.Unlock()
	...
	return &apiclient.GetWormMarketsStatusResponse{
		Started: started,
		Status:  statusText,
	}, nil
}
...
func (s *Service) ListWormEvents(ctx context.Context, req *apiclient.ListWormEventsRequest) (*apiclient.ListWormEventsResponse, error) {
	if s.store == nil {
		return nil, status.Error(codes.FailedPrecondition, "Worm Markets store is required")
	}
	...
	resp := &apiclient.ListWormEventsResponse{
		Events: make([]*v1alpha1.WormMarketsEventItem, 0, len(result.Items)),
	}
...
func (s *Service) wormEventToAPIItem(event wormmarketsstore.WormEvent) *v1alpha1.WormMarketsEventItem {
	item := &v1alpha1.WormMarketsEventItem{
		ConditionID: event.ConditionID,
		Title:       event.Title,
		Logo:        event.Logo,
		Live:        event.Live,
		MarketCount: event.MarketCount,
		Markets:     make([]*v1alpha1.WormMarketsMarketItem, 0, len(event.Markets)),
	}
...
func (s *Service) toAPIMarketSummary(market utilworm.MarketSummary) *v1alpha1.WormMarketsMarketItem {
	item := &v1alpha1.WormMarketsMarketItem{
...
func (s *Service) toAPIEvent(event *utilworm.Event) *v1alpha1.WormMarketsEventItem {
	if event == nil {
		return nil
	}
	item := &v1alpha1.WormMarketsEventItem{
...
func (s *Service) enrichWormEventTradingData(ctx context.Context, event *v1alpha1.WormMarketsEventItem) {
	...
	for _, market := range event.Markets {
		...
		wg.Add(1)
		go func(market *v1alpha1.WormMarketsMarketItem) {
			defer wg.Done()
...
func (s *Service) enrichWormMarketTradingData(ctx context.Context, item *v1alpha1.WormMarketsMarketItem) {
...
func toAPIWormMarginPositionEstimate(estimate *utilworm.MarginPositionEstimate, funds string, isYes bool, leverage string) *v1alpha1.WormMarketsMarginPositionEstimateItem {
	item := &v1alpha1.WormMarketsMarginPositionEstimateItem{
		Funds:    funds,
		IsYes:    isYes,
		Leverage: leverage,
	}
...
func (s *Service) wormMarketToAPIItem(market wormmarketsstore.WormMarket) *v1alpha1.WormMarketsMarketItem {
	return &v1alpha1.WormMarketsMarketItem{
		ConditionID:      market.ConditionID,
		Title:            market.Title,
...
func (s *Service) syncWormMarketsOnce(ctx context.Context) error {
	...
	newEvents := make(map[string]wormmarketsstore.WormMarket)
	...
	for {
		...
		items := make([]wormmarketsstore.WormMarket, 0, len(markets.Markets))
		priceSamples := make([]wormmarketsstore.WormMarketPriceSample, 0, len(markets.Markets))
		...
		for i := range markets.Markets {
			...
			items = append(items, wormmarketsstore.WormMarket{
			...
			if price, ok := parseWormMarketPrice(item.LastTradePrice); ok {
				priceSamples = append(priceSamples, wormmarketsstore.WormMarketPriceSample{
					ConditionID: item.ConditionID,
					Price:       price,
					SampledAt:   fetchedAt,
				})
...
func (s *Service) updateWormMarketLiveStates(ctx context.Context) {
	...
	for _, priceChange := range priceChanges {
		...
		if _, err := s.store.UpdateWormMarketLiveState(ctx, wormmarketsstore.WormMarket{
