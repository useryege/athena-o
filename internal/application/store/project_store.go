...
func (s *SQLStore) UpdateProjectAveLogo(ctx context.Context, contract common.Address, logo string) error {
	_, err := s.db.ExecContext(ctx, `
UPDATE project
SET ave_logo = $2, ave_logo_fetched_at = now()
WHERE contract = $1
`, contract.Bytes(), logo)
	if err != nil {
		return fmt.Errorf("update project ave logo: %w", err)
	}
	return nil
}
...
func scanProjectMetaRow(scanner rowScanner) (ProjectMeta, error) {
	...
	meta.AveLogo = aveLogo.String
	if aveLogoFetchedAt.Valid {
		meta.AveLogoFetchedAt = aveLogoFetchedAt.Time
	}
