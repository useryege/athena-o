...
func TestApplicationServerHealthStatusTransitions(t *testing.T) {
	healthServer := health.NewServer()
	healthServer.SetServingStatus("", grpc_health_v1.HealthCheckResponse_NOT_SERVING)
	server := &ApplicationServer{healthService: healthServer}

	address, stop := serveApplicationGRPC(t, server.CreateGRPC())
	defer stop()
...
func serveApplicationGRPC(t *testing.T, grpcServer *grpc.Server) (string, func()) {
	t.Helper()
	...
	go func() {
		...
		if err := grpcServer.Serve(listener); err != nil {
			t.Logf("application health test server stopped: %v", err)
		}
	}()
...
func checkHealthStatus(t *testing.T, address string) grpc_health_v1.HealthCheckResponse_ServingStatus {
	...
	conn, err := applicationpkg.NewConnection(address)
	if err != nil {
