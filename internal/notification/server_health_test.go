...
import (
	...
	notificationpkg "github.com/useryege/athena/internal/notification/apiclient"
	"google.golang.org/grpc"
	"google.golang.org/grpc/health"
	"google.golang.org/grpc/health/grpc_health_v1"
)

func TestNotificationServerHealthStatusTransitions(t *testing.T) {
	healthServer := health.NewServer()
	healthServer.SetServingStatus("", grpc_health_v1.HealthCheckResponse_NOT_SERVING)
	server := &Server{healthService: healthServer}

	address, stop := serveNotificationGRPC(t, server.CreateGRPC())
	defer stop()
...
func serveNotificationGRPC(t *testing.T, grpcServer *grpc.Server) (string, func()) {
	t.Helper()
	...
	go func() {
		...
		if err := grpcServer.Serve(listener); err != nil {
			t.Logf("notification health test server stopped: %v", err)
		}
	}()
...
func checkHealthStatus(t *testing.T, address string) grpc_health_v1.HealthCheckResponse_ServingStatus {
	...
	conn, err := notificationpkg.NewConnection(address)
	if err != nil {
