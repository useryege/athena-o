...
func (c *clientSet) Notification() NotificationServiceClient {
	return c.client
}

// NewNotificationClientset creates a new instance of notification Clientset.
func NewNotificationClientset(address string) (Clientset, error) {
	connection, err := utilgrpc.NewClientConnection(address)
	if err != nil {
		return nil, err
	}
	return &clientSet{connection: connection, client: NewNotificationServiceClient(connection.ClientConn())}, nil
}
