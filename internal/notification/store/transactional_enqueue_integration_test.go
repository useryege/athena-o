...
func frozenActivityFixture(t *testing.T, pool *pgxpool.Pool, owner string, index int, form string) int64 {
 ...
 SELECT c.owner_id,c.subscription_id,c.source_record_id,i.id,1,'{}','fixture','{"DisplayName":{"Availability":"unavailable","ReasonCode":"fixture"}}','',$3,'',clock_timestamp(),clock_timestamp(),clock_timestamp(),'{"rule":"fixture_only","cohort":"ordinary_unclassified","reason":"transactional_enqueue_fixture"}'::jsonb FROM candidate c JOIN interval i ON c.subscription_id=i.subscription_id RETURNING id`, owner, index, form).Scan(&id)
	})
	if e != nil {
		t.Fatal(e)
	}
