...
func (s *Service) fetchSportsLiveEvents(ctx context.Context, fetchedAt time.Time) ([]sportslivestore.SportsLiveEvent, []sportslivestore.SportsLiveMarket, error) {
	...
	eventsByKey := make(map[string]sportslivestore.SportsLiveEvent)
	marketsByKey := make(map[string]sportslivestore.SportsLiveMarket)
	...
	events := make([]sportslivestore.SportsLiveEvent, 0, len(eventsByKey))
	for _, item := range eventsByKey {
		events = append(events, item)
	}
	markets := make([]sportslivestore.SportsLiveMarket, 0, len(marketsByKey))
...
func sportsLiveEventFromGamma(eventKey string, event utilpolymarket.Event, fetchedAt time.Time) sportslivestore.SportsLiveEvent {
	return sportslivestore.SportsLiveEvent{
		EventKey:          eventKey,
...
func sportsLiveMarketFromGamma(eventKey, eventID, eventSlug, marketKey string, market utilpolymarket.Market, fetchedAt time.Time) sportslivestore.SportsLiveMarket {
	return sportslivestore.SportsLiveMarket{
		MarketKey:        marketKey,
		EventKey:         eventKey,
...
func (s *Service) ListSportsLiveEvents(ctx context.Context, req *apiclient.ListSportsLiveEventsRequest) (*apiclient.ListSportsLiveEventsResponse, error) {
	...
	if s.store == nil {
		return nil, status.Error(codes.FailedPrecondition, "sports live store is required")
	}
	...
	resp := &apiclient.ListSportsLiveEventsResponse{
		Items: make([]*v1alpha1.SportsLiveEventCardItem, 0, len(events)),
	...
	for _, event := range events {
		item := &v1alpha1.SportsLiveEventCardItem{
...
func (s *Service) BatchGetSportsLivePriceHistories(ctx context.Context, req *apiclient.BatchGetSportsLivePriceHistoriesRequest) (*apiclient.BatchGetSportsLivePriceHistoriesResponse, error) {
	...
	if len(marketKeys) == 0 {
		return &apiclient.BatchGetSportsLivePriceHistoriesResponse{}, nil
	}
	...
	resp := &apiclient.BatchGetSportsLivePriceHistoriesResponse{
		Items: make([]*v1alpha1.SportsLivePriceHistorySeriesItem, 0, len(series)),
	}
	for _, item := range series {
		resp.Items = append(resp.Items, &v1alpha1.SportsLivePriceHistorySeriesItem{
...
func sportsLiveTeamsFromRaw(raw json.RawMessage) []*v1alpha1.SportsTeamItem {
