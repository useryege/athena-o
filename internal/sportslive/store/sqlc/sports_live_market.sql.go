...
const getSportsLiveSyncState = `-- name: GetSportsLiveSyncState :one
SELECT last_success_at
FROM sports_live_sync_state
WHERE sync_name = $1
`

func (q *Queries) GetSportsLiveSyncState(ctx context.Context, syncName string) (pgtype.Timestamptz, error) {
	row := q.db.QueryRow(ctx, getSportsLiveSyncState, syncName)
...
type UpsertSportsLiveSyncStateParams struct {
	SyncName      string
	LastSuccessAt pgtype.Timestamptz
}

func (q *Queries) UpsertSportsLiveSyncState(ctx context.Context, arg UpsertSportsLiveSyncStateParams) error {
	_, err := q.db.Exec(ctx, upsertSportsLiveSyncState, arg.SyncName, arg.LastSuccessAt)
	return err
}
