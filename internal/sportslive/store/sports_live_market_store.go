...
import (
	...
	sportslivesqlc "github.com/useryege/athena/internal/sportslive/store/sqlc"
)
...
func (s *SQLStore) SyncSportsLiveEvents(ctx context.Context, events []SportsLiveEvent, markets []SportsLiveMarket, syncStartedAt, fetchedAt time.Time) error {
	...
	if err := queries.UpsertSportsLiveSyncState(ctx, sportslivesqlc.UpsertSportsLiveSyncStateParams{
...
func (s *SQLStore) ListSportsLiveEventCards(ctx context.Context, limit int32) ([]SportsLiveEventCard, error) {
	if s == nil || s.queries == nil {
		return nil, fmt.Errorf("sports live postgres database is not configured")
	}
...
func (s *SQLStore) GetSportsLiveLastSuccessAt(ctx context.Context) (time.Time, error) {
	if s == nil || s.queries == nil {
		return time.Time{}, fmt.Errorf("sports live postgres database is not configured")
	}
	value, err := s.queries.GetSportsLiveSyncState(ctx, sportsLiveSyncName)
...
func (s *SQLStore) UpdateSportsLiveScoreAlertState(ctx context.Context, state SportsLiveScoreAlertState) error {
	...
	if err := s.queries.UpdateSportsLiveScoreAlertState(ctx, sportslivesqlc.UpdateSportsLiveScoreAlertStateParams{
...
func (s *SQLStore) ListSportsLivePriceHistorySeries(ctx context.Context, marketKeys []string, limitPerToken int32) ([]SportsLivePriceHistorySeries, error) {
	...
	rows, err := s.queries.ListSportsLivePriceHistoryByMarketKeys(ctx, sportslivesqlc.ListSportsLivePriceHistoryByMarketKeysParams{
...
func batchUpsertSportsLiveEventsParams(items []SportsLiveEvent) sportslivesqlc.BatchUpsertSportsLiveEventsParams {
	params := sportslivesqlc.BatchUpsertSportsLiveEventsParams{
...
func batchUpsertSportsLiveMarketsParams(items []SportsLiveMarket) sportslivesqlc.BatchUpsertSportsLiveMarketsParams {
	params := sportslivesqlc.BatchUpsertSportsLiveMarketsParams{
...
func batchUpsertSportsLivePricePointsParams(items []SportsLivePricePoint) sportslivesqlc.BatchUpsertSportsLivePricePointsParams {
