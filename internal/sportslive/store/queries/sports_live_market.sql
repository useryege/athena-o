...
INSERT INTO sports_live_score_alert_state (
  event_key,
  last_score
)
SELECT
  event.event_key,
  btrim(event.score)
FROM sports_live_event AS event
...
INSERT INTO sports_live_market (
  market_key,
  event_key,
  event_id,
  event_slug,
...
SELECT
  ...
  event.last_seen_at
FROM sports_live_event AS event
JOIN sports_live_market AS market ON market.event_key = event.event_key
...
FROM sports_live_market AS market
JOIN sports_live_event AS event ON event.event_key = market.event_key
...
WITH ranked_points AS (
  ...
  FROM sports_live_price_point AS point
  JOIN sports_live_market AS market ON market.market_key = point.market_key
...
WITH latest_points AS (
  ...
  FROM sports_live_price_point AS point
  JOIN sports_live_market AS market ON market.market_key = point.market_key
  JOIN sports_live_event AS event ON event.event_key = point.event_key
  WHERE event.live = true
    AND event.ended = false
    AND market.closed = false
    AND lower(market.sports_market_type) = 'moneyline'
...
JOIN sports_live_market AS market ON market.market_key = latest.market_key
JOIN sports_live_event AS event ON event.event_key = latest.event_key
...
DELETE FROM sports_live_price_alert_state
...
SELECT
  ...
  event.fetched_at
FROM sports_live_event AS event
JOIN sports_live_score_alert_state AS state ON state.event_key = event.event_key
WHERE lower(split_part(btrim(event.slug), '-', 1)) IN ('fifwc', 'mlb', 'nhl')
  AND event.live = true
  AND event.ended = false
  AND btrim(event.score) <> ''
...
UPDATE sports_live_score_alert_state
...
-- name: GetSportsLiveSyncState :one
SELECT last_success_at
FROM sports_live_sync_state
WHERE sync_name = @sync_name;

-- name: UpsertSportsLiveSyncState :exec
INSERT INTO sports_live_sync_state (sync_name, last_success_at)
