...
func (s *Service) updateSportsLivePriceAlerts(ctx context.Context) {
	...
	candidates := make([]struct {
		token sportslivestore.SportsLivePriceAlertToken
		band  string
	}, 0)
...
func (s *Service) clearSportsLivePriceAlertState(ctx context.Context, token sportslivestore.SportsLivePriceAlertToken) {
...
func shouldSendSportsLivePriceAlert(token sportslivestore.SportsLivePriceAlertToken, band string, config SportsLivePriceAlertsConfig, now time.Time) bool {
...
func sportsLivePriceAlertGameStatus(token sportslivestore.SportsLivePriceAlertToken) string {
	parts := []string{
