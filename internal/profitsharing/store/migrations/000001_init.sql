...
CREATE TABLE profit_sharing_participant (
  ...
  CONSTRAINT profit_sharing_participant_account_check
    CHECK (BTRIM(account) <> ''),
...
CREATE TABLE profit_sharing_proposal_item (
  ...
  CONSTRAINT profit_sharing_proposal_item_pk
    PRIMARY KEY (proposal_id, participant_account),
  ...
  CONSTRAINT profit_sharing_proposal_item_participant_fk
    FOREIGN KEY (round_id, participant_account)
    REFERENCES profit_sharing_participant (round_id, account)
    ON DELETE CASCADE,
...
CREATE TABLE profit_sharing_vote (
  ...
  proposal_id TEXT NOT NULL,
  proposal_author_account TEXT NOT NULL,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  CONSTRAINT profit_sharing_vote_pk
    PRIMARY KEY (round_id, ballot_number, voter_account),
  CONSTRAINT profit_sharing_vote_voter_fk
    FOREIGN KEY (round_id, voter_account)
    REFERENCES profit_sharing_participant (round_id, account)
    ON DELETE CASCADE,
  ...
  CONSTRAINT profit_sharing_vote_proposal_author_fk
    FOREIGN KEY (proposal_id, round_id, proposal_author_account)
    REFERENCES profit_sharing_proposal (id, round_id, author_account)
    ON DELETE CASCADE,
  CONSTRAINT profit_sharing_vote_no_self_vote_check
    CHECK (voter_account <> proposal_author_account)
);
