...
import (
	...
	"github.com/useryege/athena/internal/sportshistory/store/sqlc"
...
func NewSQLStoreSource() func(context.Context) (*SQLStore, error) {
	return func(ctx context.Context) (*SQLStore, error) {
		pool, err := postgres.ConnectAndMigrate(ctx, postgres.Options{
			...
			DSNEnv:       "ATHENA_SPORTS_HISTORY_POSTGRES_DSN",
			Database:     "sports_history",
			Migrations:   migrations,
			MigrationDir: "migrations",
		})
		if err != nil {
			return nil, fmt.Errorf("connect sports history postgres: %w", err)
		}
		log.Info("sports history postgres migrations are up to date")
