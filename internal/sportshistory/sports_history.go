...
func (s *Service) refreshSportsHistory(ctx context.Context) (*v1alpha1.SportsHistorySyncStatus, error) {
	...
	return value.(*v1alpha1.SportsHistorySyncStatus), err
}

func (s *Service) syncSportsHistory(ctx context.Context) (time.Time, error) {
	if s.store == nil {
		return time.Time{}, status.Error(codes.FailedPrecondition, "sports history store is required")
	}
...
func (s *Service) markSportsHistorySyncStarted() {
	...
	s.sportsHistorySyncStatus = &v1alpha1.SportsHistorySyncStatus{
...
func (s *Service) markSportsHistorySyncCompleted(lastSuccessAt time.Time, syncErr error) *v1alpha1.SportsHistorySyncStatus {
	completedAt := s.now().UTC().Unix()
	s.cacheMu.Lock()
	statusValue := &v1alpha1.SportsHistorySyncStatus{
...
func (s *Service) currentSportsHistorySyncStatus() *v1alpha1.SportsHistorySyncStatus {
	s.cacheMu.RLock()
	result := cloneSportsHistorySyncStatus(s.sportsHistorySyncStatus)
	s.cacheMu.RUnlock()
	if result == nil {
		return &v1alpha1.SportsHistorySyncStatus{State: sportsHistorySyncStateIdle}
	}
	return result
}

func cloneSportsHistorySyncStatus(value *v1alpha1.SportsHistorySyncStatus) *v1alpha1.SportsHistorySyncStatus {
	if value == nil {
		return nil
	}
	result := *value
...
func (s *Service) fetchSportsHistoryEvents(ctx context.Context, fetchedAt time.Time) ([]sportshistorystore.SportsHistoryEvent, []sportshistorystore.SportsHistoryMarket, error) {
	if s.gammaClient == nil {
		return nil, nil, status.Error(codes.FailedPrecondition, "polymarket gamma client is required")
	}

	limit := s.sportsHistoryPageLimit
	if limit <= 0 {
		limit = defaultSportsHistoryEventPageLimit
	}
	windowStart := fetchedAt.Add(-sportsHistoryWindow)
	windowEnd := fetchedAt
	eventsByKey := make(map[string]sportshistorystore.SportsHistoryEvent)
	marketsByKey := make(map[string]sportshistorystore.SportsHistoryMarket)
	for _, closed := range []bool{true, false} {
		...
		for {
			...
			for i := range resp.Events {
				...
				if league == "" || !ok || !sportsHistoryEventInWindow(event, windowStart, windowEnd) || isSportsHistoryDerivativeEvent(event) {
					continue
				}
	...
	events := make([]sportshistorystore.SportsHistoryEvent, 0, len(eventsByKey))
	for _, event := range eventsByKey {
		events = append(events, event)
	}
	markets := make([]sportshistorystore.SportsHistoryMarket, 0, len(marketsByKey))
...
func sportsHistoryEventFromGamma(eventKey, league string, event utilpolymarket.Event, finishedAt, fetchedAt time.Time) sportshistorystore.SportsHistoryEvent {
	return sportshistorystore.SportsHistoryEvent{
		EventKey:       eventKey,
...
func sportsHistoryMarketsFromGamma(eventKey string, event utilpolymarket.Event, fetchedAt time.Time) []sportshistorystore.SportsHistoryMarket {
	items := make([]sportshistorystore.SportsHistoryMarket, 0, len(event.Markets))
	for i := range event.Markets {
		...
		items = append(items, sportshistorystore.SportsHistoryMarket{
			MarketKey:        marketKey,
			EventKey:         eventKey,
...
func (s *Service) GetSportsHistorySyncStatus(ctx context.Context, _ *apiclient.GetSportsHistorySyncStatusRequest) (*apiclient.GetSportsHistorySyncStatusResponse, error) {
	if s.store == nil {
		return nil, status.Error(codes.FailedPrecondition, "sports history store is required")
	}
	...
	return &apiclient.GetSportsHistorySyncStatusResponse{Status: statusValue}, nil
}

func (s *Service) RefreshSportsHistory(ctx context.Context, _ *apiclient.RefreshSportsHistoryRequest) (*apiclient.RefreshSportsHistoryResponse, error) {
	statusValue, err := s.refreshSportsHistory(ctx)
	if err != nil {
		return nil, status.Errorf(codes.Unavailable, "refresh sports history: %v", err)
	}
	return &apiclient.RefreshSportsHistoryResponse{Status: statusValue}, nil
}

func (s *Service) ListSportsHistoryEvents(ctx context.Context, req *apiclient.ListSportsHistoryEventsRequest) (*apiclient.ListSportsHistoryEventsResponse, error) {
	...
	s.cacheMu.RUnlock()
	resp := &apiclient.ListSportsHistoryEventsResponse{
		Items: make([]*v1alpha1.SportsHistoryEventCardItem, 0, len(events)),
	...
	for _, event := range events {
		item := &v1alpha1.SportsHistoryEventCardItem{
			...
			Markets:     make([]*v1alpha1.SportsLiveMarketCardItem, 0, len(event.Markets)),
			Teams:       sportsHistoryTeamsFromRaw(event.Teams),
		}
		for _, market := range event.Markets {
			item.Markets = append(item.Markets, &v1alpha1.SportsLiveMarketCardItem{
...
func (s *Service) BatchGetSportsHistoryPriceHistories(ctx context.Context, req *apiclient.BatchGetSportsHistoryPriceHistoriesRequest) (*apiclient.BatchGetSportsHistoryPriceHistoriesResponse, error) {
	...
	if len(marketKeys) == 0 {
		return &apiclient.BatchGetSportsHistoryPriceHistoriesResponse{}, nil
	}
	...
	resp := &apiclient.BatchGetSportsHistoryPriceHistoriesResponse{
		Items: make([]*v1alpha1.SportsHistoryPriceHistorySeriesItem, 0, len(series)),
	}
	for _, item := range series {
		resp.Items = append(resp.Items, &v1alpha1.SportsHistoryPriceHistorySeriesItem{
...
func isSportsHistoryDerivativeEvent(event utilpolymarket.Event) bool {
