...
type sportsHistoryPriceHistoryPointKey struct {
	tokenID string
	priceTs int64
}
...
func (s *Service) syncSportsHistoryPriceHistory(ctx context.Context) error {
	...
	if s.store == nil {
		return fmt.Errorf("sports history store is required")
	}
	...
	for _, key := range keys {
		...
		for offset := 0; offset < len(tokenIDs); offset += sportsHistoryPriceHistoryBatchLimit {
			end := offset + sportsHistoryPriceHistoryBatchLimit
			if end > len(tokenIDs) {
				end = len(tokenIDs)
			}
...
func sportsHistoryPriceHistoryTokens(markets []sportshistorystore.SportsHistoryPriceHistoryMarket) map[string]sportsHistoryPriceHistoryToken {
	out := make(map[string]sportsHistoryPriceHistoryToken)
	for _, market := range markets {
		tokenIDs := parseSportsHistoryStringList(market.ClobTokenIDs)
		outcomes := parseSportsHistoryStringList(market.Outcomes)
...
func (s *Service) syncSportsHistoryPriceHistoryBatch(ctx context.Context, tokenIDs []string, tokenByID map[string]sportsHistoryPriceHistoryToken, start, end int64) error {
	...
	endTs := float64(end)
	fidelity := sportsHistoryPriceHistoryFidelity
	...
	fetchedAt := s.now().UTC()
	pointsByKey := make(map[sportsHistoryPriceHistoryPointKey]sportshistorystore.SportsHistoryPricePoint)
	for _, tokenID := range tokenIDs {
		...
		for _, point := range resp.History[tokenID] {
			...
			key := sportsHistoryPriceHistoryPointKey{tokenID: tokenID, priceTs: point.T}
			pointsByKey[key] = sportshistorystore.SportsHistoryPricePoint{
				TokenID:     tokenID,
	...
	points := make([]sportshistorystore.SportsHistoryPricePoint, 0, len(pointsByKey))
