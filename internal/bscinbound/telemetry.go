...
func NewMetrics() *Metrics {
	metrics := &Metrics{
		...
		finalizedBlock: prometheus.NewGauge(prometheus.GaugeOpts{
			Name: "athena_bsc_inbound_finalized_block", Help: "Latest finalized BSC block observed by the scanner.",
		}),
		indexedBlock: prometheus.NewGauge(prometheus.GaugeOpts{
			Name: "athena_bsc_inbound_indexed_block", Help: "Highest finalized BSC block committed to PostgreSQL.",
		}),
		lagBlocks: prometheus.NewGauge(prometheus.GaugeOpts{
			Name: "athena_bsc_inbound_lag_blocks", Help: "Finalized blocks not yet committed to PostgreSQL.",
		}),
		processedBlocks: prometheus.NewCounter(prometheus.CounterOpts{
			Name: "athena_bsc_inbound_processed_blocks_total", Help: "Finalized BSC blocks committed by the scanner.",
		}),
		storedTransactions: prometheus.NewCounter(prometheus.CounterOpts{
			Name: "athena_bsc_inbound_stored_transactions_total", Help: "Qualifying inbound BNB transfers committed by the scanner.",
		}),
		scanFailures: prometheus.NewCounter(prometheus.CounterOpts{
			Name: "athena_bsc_inbound_scan_failures_total", Help: "Failed BSC scanner iterations.",
		}),
		queryDuration: prometheus.NewHistogram(prometheus.HistogramOpts{
			Name: "athena_bsc_inbound_query_duration_seconds", Help: "ListInboundNormalTransactions request duration.",
		}),
	}
	metrics.registry.MustRegister(
		metrics.finalizedBlock, metrics.indexedBlock, metrics.lagBlocks,
		metrics.processedBlocks, metrics.storedTransactions, metrics.scanFailures,
...
func (m *Metrics) RecordScanSuccess(blocks, transactions uint64) {
...
func (s *TelemetryServer) Start() error {
	...
	if err != nil {
		return fmt.Errorf("listen BSC inbound telemetry on %s: %w", s.address, err)
	}
