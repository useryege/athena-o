...
import (
	...
	polymarketpkg "github.com/useryege/athena/internal/polymarket/apiclient"
	"google.golang.org/grpc"
	"google.golang.org/grpc/health"
	"google.golang.org/grpc/health/grpc_health_v1"
)

func TestPolymarketServerHealthStatusTransitions(t *testing.T) {
	healthServer := health.NewServer()
	healthServer.SetServingStatus("", grpc_health_v1.HealthCheckResponse_NOT_SERVING)
	server := &Server{healthService: healthServer}

	address, stop := servePolymarketGRPC(t, server.CreateGRPC())
	defer stop()
...
func servePolymarketGRPC(t *testing.T, grpcServer *grpc.Server) (string, func()) {
	t.Helper()
	...
	go func() {
		...
		if err := grpcServer.Serve(listener); err != nil {
			t.Logf("polymarket health test server stopped: %v", err)
		}
	}()
...
func checkHealthStatus(t *testing.T, address string) grpc_health_v1.HealthCheckResponse_ServingStatus {
	...
	conn, err := polymarketpkg.NewConnection(address)
	if err != nil {
