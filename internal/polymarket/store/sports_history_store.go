...
func (s *SQLStore) GetSportsHistoryLastSuccessAt(ctx context.Context) (time.Time, error) {
	...
	value, err := s.queries.GetPolymarketSyncState(ctx, sportsHistorySyncName)
...
func (s *SQLStore) BatchUpsertSportsHistoryPricePoints(ctx context.Context, points []SportsLivePricePoint) error {
	if s == nil || s.queries == nil {
		return fmt.Errorf("polymarket postgres database is not configured")
	}
	if len(points) == 0 {
	...
	params := polymarketsqlc.BatchUpsertSportsHistoryPricePointsParams{
...
func (s *SQLStore) ListSportsHistoryPriceHistorySeries(ctx context.Context, marketKeys []string, limitPerToken int32) ([]SportsLivePriceHistorySeries, error) {
	if s == nil || s.queries == nil {
		return nil, fmt.Errorf("polymarket postgres database is not configured")
	}
	...
	rows, err := s.queries.ListSportsHistoryPriceHistoryByMarketKeys(ctx, polymarketsqlc.ListSportsHistoryPriceHistoryByMarketKeysParams{
...
func batchUpsertSportsHistoryEventsParams(items []SportsHistoryEvent) polymarketsqlc.BatchUpsertSportsHistoryEventsParams {
	params := polymarketsqlc.BatchUpsertSportsHistoryEventsParams{
...
func batchUpsertSportsHistoryMarketsParams(items []SportsHistoryMarket) polymarketsqlc.BatchUpsertSportsHistoryMarketsParams {
	params := polymarketsqlc.BatchUpsertSportsHistoryMarketsParams{
