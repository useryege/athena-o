...
func (s *Service) ListPolymarketMovers(ctx context.Context, req *apiclient.ListPolymarketMoversRequest) (*apiclient.ListPolymarketMoversResponse, error) {
	s.startStopMu.Lock()
	started := s.started
...
func (s *Service) currentMoverMarketResponse(limit int) *apiclient.ListPolymarketMoversResponse {
	s.cacheMu.Lock()
	...
	now := s.now()
	nowUnix := now.Unix()
	items := make([]*v1alpha1.PolymarketMoverMarketItem, 0, len(markets))
	...
	return &apiclient.ListPolymarketMoversResponse{
		Items:            items,
		FetchedAt:        fetchedAt,
...
func (s *Service) moverMarketItemLocked(market *v1alpha1.PolymarketHotMarketItem, nowUnix int64) *v1alpha1.PolymarketMoverMarketItem {
	if market == nil {
		return nil
	}
	item := &v1alpha1.PolymarketMoverMarketItem{
		...
		Tokens:       make([]*v1alpha1.PolymarketMoverTokenItem, 0, len(market.Tokens)),
	}
...
func (s *Service) moverTokenItemLocked(token *v1alpha1.PolymarketHotMarketTokenItem, state *realtimeTokenState, nowUnix int64) *v1alpha1.PolymarketMoverTokenItem {
	...
	windows := []*v1alpha1.PolymarketMoverWindowItem{
	...
	item := &v1alpha1.PolymarketMoverTokenItem{
...
func (s *Service) moverWindowItemLocked(tokenID string, nowUnix int64, window time.Duration, currentPrice float64) *v1alpha1.PolymarketMoverWindowItem {
	realtimeWindow := s.realtimeWindowItemLocked(tokenID, nowUnix, window, currentPrice)
	return &v1alpha1.PolymarketMoverWindowItem{
...
func scoreMoverWindows(windows []*v1alpha1.PolymarketMoverWindowItem) (float64, string) {
	score := 0.0
...
func moverWindowAbsChange(token *v1alpha1.PolymarketMoverTokenItem, label string) float64 {
	if token == nil {
		return 0
	}
