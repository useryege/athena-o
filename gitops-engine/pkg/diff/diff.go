...
import (
	...
	"github.com/useryege/athena/gitops-engine/internal/kubernetes_vendor/pkg/api/v1/endpoints"
	"github.com/useryege/athena/gitops-engine/pkg/diff/internal/fieldmanager"
	"github.com/useryege/athena/gitops-engine/pkg/sync/resource"
	jsonutil "github.com/useryege/athena/gitops-engine/pkg/utils/json"
	gescheme "github.com/useryege/athena/gitops-engine/pkg/utils/kube/scheme"
)
...
func removeWebhookMutation(predictedLive, live *unstructured.Unstructured, gvkParser *managedfields.GvkParser, manager string) (*unstructured.Unstructured, error) {
	...
	nonAthenaFieldsSet := predictedLiveFieldSet.Difference(managerFieldsSet)
	...
	if comparison.Removed != nil && !comparison.Removed.Empty() {
		...
		comparison.Removed = comparison.Removed.Difference(nonAthenaFieldsSet)
	}

	// In case any of the removed fields cause schema violations, we will keep those fields
	nonAthenaFieldsSet = filterOutCompositeKeyFields(typedPredictedLive, nonAthenaFieldsSet)
	typedPredictedLive = typedPredictedLive.RemoveItems(nonAthenaFieldsSet)
