...
export interface PolymarketFIFATeamItem {
    name: string;
    logo?: string;
    abbreviation?: string;
    alias?: string;
...
export interface PolymarketFIFAMoneylineEventItem {
    ...
    live?: boolean;
    ended?: boolean;
    teams: PolymarketFIFATeamItem[];
    options: PolymarketFIFAMoneylineOptionItem[];
}

export interface FIFAMarketDashboardWalletBalanceItem {
    chain: string;
    label: string;
...
export interface FIFAMarketDashboardWalletHoldingItem {
    walletId?: number;
    chain: string;
    type: string;
    alias: string;
...
export interface FIFAMarketDashboard {
    config?: FIFAMarketDashboardEventConfig;
    wormEvent?: WormMarketsEventItem;
    wormFetchedAt?: number;
    wormError?: string;
    polymarketEvent?: PolymarketFIFAMoneylineEventItem;
    polymarketFetchedAt?: number;
    polymarketError?: string;
    walletBalances: FIFAMarketDashboardWalletBalanceItem[];
    walletBalancesFetchedAt?: number;
    walletBalancesError?: string;
    walletHoldings: FIFAMarketDashboardWalletHoldingItem[];
...
const normalizeFIFAEventConfig = (item: any): FIFAMarketDashboardEventConfig => ({
...
const normalizeWormMarket = (item: any): WormMarketsMarketItem => ({
    ...
    liveState: readString(item, 'liveState', 'live_state') as WormMarketsMarketItem['liveState'],
...
const normalizeWormEvent = (item: any): WormMarketsEventItem => {
...
const normalizeFIFATeam = (item: any): PolymarketFIFATeamItem => ({
...
const normalizeFIFAMoneylineEvent = (item: any): PolymarketFIFAMoneylineEventItem => {
    ...
    return {
        ...
        teams: Array.isArray(teams) ? teams.map(normalizeFIFATeam) : [],
        options: Array.isArray(options) ? options.map(normalizeFIFAMoneylineOption) : []
    };
};

const normalizeFIFAWalletBalance = (item: any): FIFAMarketDashboardWalletBalanceItem => ({
...
const normalizeFIFAWalletHolding = (item: any): FIFAMarketDashboardWalletHoldingItem => ({
...
const normalizeDashboard = (item: any): FIFAMarketDashboard => {
