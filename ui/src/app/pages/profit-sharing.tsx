import {ArrowLeftOutlined, EditOutlined, SafetyCertificateOutlined, SendOutlined, SettingOutlined, TrophyOutlined} from '@ant-design/icons';
...
import {ProfitSharingProposalItem, ProfitSharingProposalStatus, ProfitSharingRound, ProfitSharingRoundPhase, services} from '../shared/services';
        ...
        <AppPage
            ...
            extra={
                authorization.isAdmin ? (
                    <Button icon={<SettingOutlined />} onClick={() => navigate('/admin/profit-sharing')}>
                        Manage rounds
                    </Button>
                ) : undefined
            }>
    ...
    const submitVote = () => {
        ...
        ctx.modal.confirm({
            ...
            onOk: async () => {
                setSubmitting(true);
                try {
                    await services.profitSharing.submitVote(props.round.slug, selectedProposal.id);
    ...
    const submitProposal = () => {
        ...
        ctx.modal.confirm({
            ...
            onOk: async () => {
                ...
                setSaving(true);
                try {
                    await services.profitSharing.submitProposal(round.slug, revision);
    ...
    const reopenProposal = () => {
        ...
        ctx.modal.confirm({
            ...
            onOk: async () => {
                setSaving(true);
                try {
                    await services.profitSharing.reopenProposal(round.slug, proposalRevision);
