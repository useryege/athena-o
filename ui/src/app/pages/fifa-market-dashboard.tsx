...
import {
    ...
    FIFAMarketDashboardWalletBalanceItem,
    FIFAMarketDashboardWalletHoldingItem,
    FIFAMarketDashboard,
    FIFAMarketDashboardEventConfig
} from '../shared/services/fifa-market-dashboard-service';
import {GetWormMarketsEventResult, WormMarketsMarketItem} from '../shared/services/worm-markets-models';
import {boolTag, fmt} from './shared';

const dashboardRefreshIntervalMs = 1000;

const walletBalancePlaceholders: FIFAMarketDashboardWalletBalanceItem[] = [
    {
        chain: 'polygon',
        label: 'Polygon pUSD',
...
const balanceValue = (item: FIFAMarketDashboardWalletBalanceItem) =>
...
const orderedWormMarkets = (eventTitle: string, markets: WormMarketsMarketItem[]) => {
...
const FIFAWalletBalanceCard = (props: {item: FIFAMarketDashboardWalletBalanceItem; loading?: boolean; detailsVisible?: boolean}) => {
...
const FIFAWalletBalancesPanel = (props: {items?: FIFAMarketDashboardWalletBalanceItem[]; fetchedAt?: number; loading?: boolean; error?: Error}) => {
...
const FIFAWalletHoldingCard = (props: {item: FIFAMarketDashboardWalletHoldingItem}) => {
    const item = props.item;
...
const FIFAWalletHoldingsPanel = (props: {items?: FIFAMarketDashboardWalletHoldingItem[]; fetchedAt?: number; loading?: boolean; error?: Error}) => {
...
const WormMarketCard = (props: {item: WormMarketsMarketItem; selected?: boolean; onSelect?: () => void}) => {
    const item = props.item;
...
const WormEventPanel = (props: {
    data?: GetWormMarketsEventResult;
    loading?: boolean;
    error?: Error;
...
const selectedWormOutcomeKey = (data?: GetWormMarketsEventResult, selectedConditionId?: string): FIFAOutcomeKey | undefined => {
...
const selectedWormMarket = (data?: GetWormMarketsEventResult, selectedConditionId?: string) => data?.item?.markets.find(market => market.conditionId === selectedConditionId);
...
const HedgeCalculatorPanel = (props: {wormMarket?: WormMarketsMarketItem; outcomeKey?: FIFAOutcomeKey; polymarketOption?: PolymarketFIFAMoneylineOptionItem}) => {
...
export const FIFAMarketDashboardPage = () => {
    ...
    const dashboardRequestRef = React.useRef<(Promise<FIFAMarketDashboard> & {abort?: () => void}) | null>(null);

    const applyConfig = React.useCallback(
        (nextConfig: FIFAMarketDashboardEventConfig) => {
    ...
    const applyDashboard = React.useCallback(
        (nextDashboard: FIFAMarketDashboard) => {
    ...
    const loadDashboard = React.useCallback(() => {
        ...
        setConfigError(null);
        setError(null);
        const req = services.fifaMarketDashboard.getDashboard();
    ...
    const saveConfig = async () => {
        ...
        if (!wormEventId || !eventRef) {
            ctx.notifications.error('Invalid FIFA market dashboard event config', 'Worm Event ID and Polymarket Event Ref are required.');
            return;
        }
        setSaving(true);
        setConfigError(null);
        try {
            const nextConfig = await services.fifaMarketDashboard.updateEventConfig({wormEventId, eventRef});
    ...
    const wormData: GetWormMarketsEventResult = dashboard?.wormEvent || dashboard?.wormFetchedAt ? {item: dashboard?.wormEvent, fetchedAt: dashboard?.wormFetchedAt} : null;
    ...
    const balancesData: {items?: FIFAMarketDashboardWalletBalanceItem[]; fetchedAt?: number} = dashboard
        ? {items: dashboard.walletBalances, fetchedAt: dashboard.walletBalancesFetchedAt}
        : null;
    const holdingsData: {items?: FIFAMarketDashboardWalletHoldingItem[]; fetchedAt?: number} = dashboard
