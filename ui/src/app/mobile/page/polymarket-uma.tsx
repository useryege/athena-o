...
import {Button, Drawer, InputNumber, Space, Tag, Typography} from 'antd';
import type {ColumnsType} from 'antd/es/table';
import * as React from 'react';
import {AppPage, CardTitle, KeyValueGrid, MetricRow, ResponsiveResourceList, Section, TruncatedText, useAsyncData} from '../components';
...
const UMADetailDrawer = (props: {item?: UMAItem; kind: UMAKind; onClose: () => void}) => {
    ...
    return (
        <Drawer
            title={props.kind === 'proposal' ? 'UMA Proposed details' : 'UMA Disputed details'}
            open={Boolean(item)}
...
const PolymarketUMAPage = (props: {kind: UMAKind; canScan: boolean}) => {
    ...
    return (
        <AppPage
            ...
            <ResponsiveResourceList<UMAItem>
                rowKey={item => `${item.txHash}:${item.logIndex}`}
                items={data.data?.items || []}
                columns={columns}
                loading={data.loading}
                emptyText={blockNumber ? `No ${title} records in block ${blockNumber}` : `No ${title} records`}
                ...
                card={item => (
                    <div>
                        <CardTitle
                            title={item.question || item.marketId || 'UMA event'}
                            subtitle={`Block ${item.blockNumber} · Log ${item.logIndex}`}
                            tags={<Tag>{props.kind === 'proposal' ? 'Proposed' : 'Disputed'}</Tag>}
                        />
                        <MetricRow
                            items={[
                                {
                                    label: props.kind === 'proposal' ? 'Proposer' : 'Disputer',
                                    value: short(props.kind === 'proposal' ? item.proposer : 'disputer' in item ? item.disputer : '')
                                },
                                {label: 'Price', value: item.proposedPrice || '-'},
                                {label: 'Request time', value: formatTimestamp(item.requestTimestamp)}
                            ]}
                        />
                    </div>
