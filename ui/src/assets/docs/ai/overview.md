...
Athena is a permissioned platform for market intelligence, token project profiling, operational wallets, notifications, and governed collaboration. Its browser application and HTTP API are served from the same origin.
...
Product access is split across nine independently granted modules. Profit Sharing, interactive Telegram binding, and administrator operations are separate authorization boundaries. See [Modules and Permissions](/docs/ai/modules.md).
