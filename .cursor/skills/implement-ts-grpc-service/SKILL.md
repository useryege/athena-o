...
Do not implement handlers, codegen, or wiring until the **`.proto`** is readable and the **TypeScript package root** is settled (explicit path or successfully inferred—see below).
...
2. **TS project directory (optional if inferable):** Prefer a **repository-relative path** to the package root (`package.json`, `tsconfig`, `src/`, etc.) when the user names it. If they omit it, **derive the default root** by walking **upward from the `.proto` file’s directory** until you find the **nearest ancestor directory that contains `package.json`**—that directory is the TS package root for `proto:gen`, imports, and handler placement. If **no** `package.json` is found on that path, or **multiple plausible roots** conflict (ambiguous monorepo layout), stop and ask the user to specify the package directory using [assets/intake-reply-template.md](assets/intake-reply-template.md).
