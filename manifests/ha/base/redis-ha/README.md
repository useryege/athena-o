...
Athena labels.  We also add roles to redis-ha service accounts to enable run-as non-root users
in OpenShift.
...
* `overlays` is a directory containing kustomize overlays for Athena, namely label modifications and
  role additions.
